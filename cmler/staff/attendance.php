...
class Pagecustomer extends admin {
	...
	function savetohistory(){
			...
			$inrs = &$this -> dbObj -> Execute($sql);
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				if($inrrs['clearot']==1){
				echo 'UPDATE '.WEB_ADMIN_TABPOX."attendance SET planattendance=0, actualattendance=0, othours=0,leavehours=0,timeoff=0,lastremainot=0,addupot=0,`auditstatus` = 0  WHERE attendance_id =".$inrrs['attendance_id'];
				$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."attendance SET planattendance=0, actualattendance=0, othours=0,leavehours=0,timeoff=0,lastremainot=0,addupot=0,`auditstatus` = 0  WHERE attendance_id =".$inrrs['attendance_id']);
				}else {
				echo 'UPDATE '.WEB_ADMIN_TABPOX."attendance SET planattendance=0, actualattendance=0, othours=0,leavehours=0,timeoff=0,`auditstatus` = 0,lastremainot=".$inrrs['addupot']."  WHERE attendance_id =".$inrrs['attendance_id'];
				$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."attendance SET planattendance=0, actualattendance=0, othours=0,leavehours=0,timeoff=0,`auditstatus` = 0,lastremainot=".$inrrs['addupot']."  WHERE attendance_id =".$inrrs['attendance_id']);				
				}
				

			}
			$inrs -> Close();	
		    $this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."salary SET `auditstatus` = 0 WHERE agencyid =".$_SESSION["currentorgan"]);	
			 $info='归档';	
			$this -> quit($info.'成功！');
	}
	function audit(){
	 $this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."attendance SET `auditstatus` = 1 WHERE agencyid =".$_SESSION["currentorgan"]);
	 $this -> quit('提交成功！');
	}
	function disp(){
		//定义模板
		...
		if($ftable==''){$condition=$category.'="'.$keywords.'"';}else{$condition=$category.'="'.$keywords.'"';}
			...
			$inrs -> Close();	
            $auditstatus=$this -> dbObj ->GetOne("select auditstatus from ".WEB_ADMIN_TABPOX."attendance where  agencyid =".$_SESSION["currentorgan"]);
			if($auditstatus=='1'){
			$t ->set_var('auditdisabled','disabled="disabled"');
			$t ->set_var('tips','>>数据已提交，请耐心等待人事审核');
			$t ->set_var('savetohistorydisabled','disabled="disabled"');
			}else if($auditstatus=='2'){
			$t ->set_var('auditdisabled','disabled="disabled"');
			$t ->set_var('tips','>>考勤数据已通过审核，请等待工资计算！');
			$t ->set_var('savetohistorydisabled','disabled="disabled"');
			...
			$t ->set_var('auditdisabled','disabled="disabled"');
			$t ->set_var('tips','>>工资已经计算，归档到历史后开始新的月份的操作！');
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			if($leavehours>0){
			$basic=$this -> dbObj ->GetOne("select basic from ".WEB_ADMIN_TABPOX."salary where  agencyid =".$_SESSION["currentorgan"]." and employee_id =".$employee_id);
			$hourpd=$this->getValue('hourpd')?$this->getValue('hourpd'):15;
			$payph=$this->getValue('dayspm')?($basic/($hourpd*$this->getValue('dayspm'))):15;
			
			echo 'UPDATE '.WEB_ADMIN_TABPOX."salary  SET `fullattendance` = 0,`leavepay` = '".($leavehours*$payph)."' WHERE employee_id =".$employee_id;
			$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."salary  SET `fullattendance` = 0,`leavepay` = '".($leavehours*$payph)."' WHERE employee_id =".$employee_id);
			...
			$otsalaryph=$this->getValue('otsalaryph')?$this->getValue('otsalaryph'):15;
			$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."salary  SET `otpay` = ".($addupot*$otsalaryph)." WHERE employee_id =".$employee_id);
...
//$this -> quit($info.'成功！');
	...
	function quit($info){
		exit("<script>alert('$info');location.href='attendance.php';</script>");
	}
