...
function  addcard(){
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  marketingcard_id DESC  LIMIT 0 ,1");
			...
			//插入进货记录
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."card` (`card_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["card_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')");
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."card` (`item_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["item_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')";
			$id = $this -> dbObj -> Insert_ID();
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					...
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."carddetail` (card_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')");
					echo "INSERT INTO `".WEB_ADMIN_TABPOX."carddetail` (card_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')";
					
					}
			
				}
	  exit("<script>window.location.href='card.php?action=upd&updid=".$id."';</script>");
}	
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql="select *,C.customer_id AS  customer_id,S.status AS  status  from ".WEB_ADMIN_TABPOX."card  S LEFT JOIN ".WEB_ADMIN_TABPOX."customer C on C.customer_id =S.customer_id LEFT JOIN ".WEB_ADMIN_TABPOX."membercard M on M.customer_id =C.customer_id  WHERE  S.status in (0,1,2,3) AND S.agencyid =".$_SESSION["currentorgan"].' and '.$condition.' GROUP BY card_id' ;
			}else if($ftable<>''){
	    	...
	    	$t -> set_var('recordcount',$count);	
	     	...
	     	while ($inrrs = $inrs -> FetchRow()) {
				...
				$t -> set_var('card_name',$this -> dbObj -> getone('select marketingcard_name from '.WEB_ADMIN_TABPOX.'marketingcard  where marketingcard_id ='.$inrrs["marketingcard_id"]));
				...
				//$t -> set_var('category_name',$this -> dbObj -> getone('select category_name from '.WEB_ADMIN_TABPOX.'servicecategory  where category_id ='.$inrrs["categoryid"]));
			  	...
			  	$t -> set_var('delete',$this -> getDelStr('',$inrrs['card_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['card_id']));		
				$t -> parse('ml','mainlist',true);
			}
			$inrs -> Close();	
		$t -> set_var('marketingcardtypelist',$this ->selectlist('marketingcard','marketingcard_id','marketingcard_name',''));		 
	...
	function goDispAppend(){
		...
		$table=WEB_ADMIN_TABPOX.'card';
		$column='card_no';
		$number=5;
		$id='card_id';	
		$t -> set_var('card_no',$this->makeno($Prefix,$agency_no,$table,$column,$number,$id));				
		$t -> set_var('value',"");	
		$t -> set_var('overdate',"");	
		$t -> set_var('card_name',"");	
		...
		$t -> set_var('userid',$this->getUid());		
			...
			$date=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'card WHERE card_id = '.$updid.' and agencyid ='.$_SESSION["currentorgan"]);
		...
		$marketingcardtimeslist=explode("||",$date['marketingcardtimeslist']);
		
		$t -> set_var('recordcount',count($remaintimes));
		if($date['remaintimeslist']){
				
		//$remaintimes=explode("||",$date['remaintimeslist']);
		count($item_id)<count($remaintimes)?$k=count($item_id):$k=count($remaintimes);
		for ($i=0;$i<$k;$i++)	{
			...
			$t -> set_var('services_times',$marketingcardtimeslist[$i]);
		...
		$t -> set_var('remaintimes',$date['remaintimes']);
		}
...
function statuslist($status_name,$arraselectid=0){
		...
		function selectlist($table,$id,$name,$selectid=0,$marketingcardtypeid=8){
            $inrs= &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.$table .' where marketingcardtype_id ='.$marketingcardtypeid.' and agencyid ='.$_SESSION["currentorgan"]);
			$str='';
	     	while ($inrrs = &$inrs -> FetchRow()) {
	...
	function goDelete(){
	  if($_GET['consumables_id']==''){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'card WHERE card_id in('.$delid.')');
		}else{
		//echo 'DELETE FROM '.WEB_ADMIN_TABPOX.'carddetail WHERE carddetail_id in('.$_GET['consumables_id'].')';
		 $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'carddetail WHERE carddetail_id in('.$_GET['consumables_id'].')');
	...
	function goAppend(){
		...
		$id='card_id';	
			...
			exit("<script>alert('新增成功');location.href='card.php?action=upd&updid=".$id."';</script>");	
		}else{
			$info = '修改';
