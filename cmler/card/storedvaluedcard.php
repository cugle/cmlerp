...
function  addcard(){
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  marketingcard_id DESC  LIMIT 0 ,1");
			...
			//插入进货记录
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."storedvaluedcard` (`storedvaluedcard_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["storedvaluedcard_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')");
echo "INSERT INTO `".WEB_ADMIN_TABPOX."storedvaluedcard` (`item_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["item_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')";
			$id = $this -> dbObj -> Insert_ID();
			echo $id;
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					...
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."storedvaluedcarddetail` (storedvaluedcard_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')");
					echo "INSERT INTO `".WEB_ADMIN_TABPOX."storedvaluedcarddetail` (storedvaluedcard_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')";
					
					}
			
				}
	  exit("<script>window.location.href='storedvaluedcard.php?action=upd&updid=".$id."';</script>");
}	
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			$t -> set_var('ml');
						...
						if($condition<>''&&$ftable==''){
			$sql="select *,C.customer_id AS  customer_id,S.value as value from ".WEB_ADMIN_TABPOX."storedvaluedcard  S LEFT JOIN ".WEB_ADMIN_TABPOX."customer C on C.customer_id =S.customer_id LEFT JOIN ".WEB_ADMIN_TABPOX."membercard M on M.customer_id =C.customer_id  WHERE  S.status in (0,1,2,3) AND S.agencyid =".$_SESSION["currentorgan"].' and '.$condition.' GROUP BY storedvaluedcard_id' ;
			}else if($ftable<>''){
			...
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  `storedvaluedcard_id` DESC  LIMIT ".$offset." , ".$psize);
	    	...
	    	$t -> set_var('recordcount',$count);	
	     	...
	     	while ($inrrs = $inrs -> FetchRow()) {
				...
				//echo 'select membercard_no from '.WEB_ADMIN_TABPOX.'membercard  where customer_id ='.$inrrs["customer_id"];
				$t -> set_var('membercard_no',$this -> dbObj -> getone('select membercard_no from '.WEB_ADMIN_TABPOX.'membercard  where customer_id ='.$inrrs["customer_id"]));
			  	$t -> set_var('delete',$this -> getDelStr('',$inrrs['storedvaluedcard_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['storedvaluedcard_id']));		
	...
	function goDispAppend(){
		...
		$table=WEB_ADMIN_TABPOX.'storedvaluedcard';
		...
		$t -> set_var('userid',$this->getUid());		
			...
			$date=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'storedvaluedcard WHERE storedvaluedcard_id = '.$updid.' and agencyid ='.$_SESSION["currentorgan"]);
...
function statuslist($status_name,$arraselectid=0){
		...
		function selectlist($table,$id,$name,$selectid=0,$marketingcardtypeid=7){
	...
	function goDelete(){
	  if($_GET['consumables_id']==''){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'storedvaluedcard WHERE storedvaluedcard_id in('.$delid.')');
		}else{
		echo 'DELETE FROM '.WEB_ADMIN_TABPOX.'storedvaluedcarddetail WHERE storedvaluedcarddetail_id in('.$_GET['consumables_id'].')';
		 $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'storedvaluedcarddetail WHERE storedvaluedcarddetail_id in('.$_GET['consumables_id'].')');
	...
	function goAppend(){
		...
		$id='storedvaluedcard_id';	
			...
			$this -> dbObj ->Execute("update  ".WEB_ADMIN_TABPOX.$cardtable." set value=".$_POST['value']."  where agencyid =".$_SESSION["currentorgan"]."  and ".$cardtable."_id=".$id);
			...
			$this-> CardObj->updatecard($card_id,$card_no,$marketingcard_id,$customerid,$employeeid,$agencyid,$status,$activedate,$buydate,$cardtable,$overdate);
			 $this -> dbObj ->Execute("update  ".WEB_ADMIN_TABPOX.$cardtable." set value=".$_POST['value']."  where agencyid =".$_SESSION["currentorgan"]."  and ".$cardtable."_id=".$card_id);	 	
		}
//$this -> quit($info.'成功！');
