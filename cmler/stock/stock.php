...
class Pagecustomer extends admin {
	...
	function print1(){
		...
		if($ftable==''){$condition=$category.'="'.$keywords.'"';}else{$condition=$category.'="'.$keywords.'"';}
            ...
            $sqlsum='select sum(number) as totalnumber,sum(acount) as totalacount from '.WEB_ADMIN_TABPOX.'stock where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;
			}else if($ftable<>''){
			...
			$sqlsum='select sum(number) as totalnumber ,sum(acount) as totalacount from '.WEB_ADMIN_TABPOX.'stock s INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on s.produce_id =f.produce_id  where f.".$category." like '%".$keywords."%' and  s.agencyid =".$_SESSION["currentorgan"] ;
			}else{
			$sql='select *  from '.WEB_ADMIN_TABPOX.'stock  where  agencyid ='.$_SESSION["currentorgan"];
			$sqlsum='select sum(number) as totalnumber,sum(acount) as totalacount from '.WEB_ADMIN_TABPOX.'stock  where  agencyid ='.$_SESSION["currentorgan"];
			}
			
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY   number DESC   LIMIT ".$offset." , ".$psize);
			$result = &$this -> dbObj -> Execute($sql);	
			$totalacount=$this -> dbObj -> getrow($sqlsum);	
			$t -> set_var('totalacount',$totalacount['totalacount']);
			$t -> set_var('totalnumber',$totalacount['totalnumber']);
			...
			//$inrs = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'produce  where agencyid ='.$_SESSION["currentorgan"]);
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				$t -> set_var($produce);
				$unit_name=$this -> dbObj -> GetOne('select unit_name from '.WEB_ADMIN_TABPOX.'unit where unit_id ='.$produce["standardunit"]);
				
				
				$t -> set_var('unit_name',$unit_name);
		...
		$t -> set_var('prnt_time',date('Y-m-d'));
	...
	function disp(){
		...
		if($ftable==''){$condition=$category.'="'.$keywords.'"';}else{$condition=$category.'="'.$keywords.'"';}
			...
			if($condition<>''&&$ftable==''){		
			$sql='select * from '.WEB_ADMIN_TABPOX.'stock where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;
			$sql2='select sum(number) as totalnumber, sum(acount) as totalacount  from '.WEB_ADMIN_TABPOX.'stock where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition;
			}else if($ftable<>''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'stock s INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on s.produce_id =f.produce_id  where f.".$category." like '%".$keywords."%' and  s.agencyid =".$_SESSION["currentorgan"];
			$sql2='select  sum(number) as totalnumber, sum(acount) as totalacount  from '.WEB_ADMIN_TABPOX.'stock s INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on s.produce_id =f.produce_id  where f.".$category." like '%".$keywords."%' and  s.agencyid =".$_SESSION["currentorgan"];
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'stock  where  agencyid ='.$_SESSION["currentorgan"];
			$sql2='select  sum(number) as totalnumber, sum(acount) as totalacount  from '.WEB_ADMIN_TABPOX.'stock  where  agencyid ='.$_SESSION["currentorgan"];
			}
			...
			$total = &$this -> dbObj -> getrow($sql2);	
			$t -> set_var('totalnumber',$total['totalnumber']);
			$t -> set_var('totalacount',$total['totalacount']);			
			$t -> set_var('pagelist',$this -> page("?category=".$category."&keywords=".urlencode($keywords)."&ftable=".$ftable,$count,$psize,$pageid));		
	...
	function goDelete(){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'stock WHERE stock_id in('.$delid.') and agencyid='.$_SESSION["currentorgan"]);
		$this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'attendance WHERE stock_id in('.$delid.') and agencyid='.$_SESSION["currentorgan"]);
		$this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'salary  WHERE stock_id in('.$delid.') and agencyid='.$_SESSION["currentorgan"]);
		$info='删除';
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '增加';	
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."stock` (`stock_no`, `stock_name`, `address`, `genderid`, `birthday`,  `idnumber`, `tel`, `handphone`,  `zipcode`, `email`,  `memo`,  `agencyid`, `picpath`,ismarry,province,city,stocklevelid,attendancecode)VALUES ( '".$_POST["stock_no"]."','".$_POST["stock_name"]."', '".$_POST["address"]."','".$_POST["genderid"]."', '".$_POST["birthday"]."', '".$_POST["idnumber"]."','".$_POST["tel"]."', '".$_POST["handphone"]."','".$_POST["zipcode"]."', '".$_POST["email"]."','".$_POST["memo"]."','".$_SESSION["currentorgan"]."','".$_POST["picpath"]."','".$this->intnonull($_POST["ismarry"])."', '".$_POST["province"]."','".$_POST["city"]."','".$_POST["stocklevelid"]."','".$_POST["attendancecode"]."')");
			 $id = $this -> dbObj -> Insert_ID();
			 $this -> dbObj -> Execute("insert into `".WEB_ADMIN_TABPOX."attendance`(`stock_id`,agencyid) values ('$id',".$_SESSION["currentorgan"].")"); 
 			 $this -> dbObj -> Execute("insert into `".WEB_ADMIN_TABPOX."salary` (`stock_id`,`agencyid` )VALUES ('$id',".$_SESSION["currentorgan"].")");
			 echo "insert into `".WEB_ADMIN_TABPOX."salary` (`stock_id`,`agencyid` )VALUES ('$id',".$_SESSION["currentorgan"].")";
			 
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."stock` (`stock_no`, `stock_name`, `address`, `genderid`, `birthday`,  `idnumber`, `tel`, `handphone`,  `zipcode`, `email`,  `memo`,  `agencyid`, `picpath`,ismarry,province,city)VALUES ( '".$_POST["stock_no"]."','".$_POST["stock_name"]."', '".$_POST["address"]."','".$_POST["genderid"]."', '".$_POST["birthday"]."', '".$_POST["idnumber"]."','".$_POST["tel"]."', '".$_POST["handphone"]."','".$_POST["zipcode"]."', '".$_POST["email"]."','".$_POST["memo"]."','".$_SESSION["currentorgan"]."','".$_POST["picpath"]."','".$this->intnonull($_POST["ismarry"])."', '".$_POST["province"]."','".$_POST["city"]."')";
			$id = $this -> dbObj -> Insert_ID();
		}else{
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."stock` SET `stock_name` = '".$_POST["stock_name"]."',`stock_no` = '".$_POST["stock_no"]."',`idnumber` = '".$_POST["idnumber"]."',`tel` = '".$_POST["tel"]."',`handphone` ='".$_POST["handphone"]."',`email` = '".$_POST["email"]."',`address` = '".$_POST["address"]."',`zipcode` = '".$_POST["zipcode"]."',`birthday` = '".$_POST["birthday"]."',`genderid` = '".$this->intnonull($_POST["genderid"])."', picpath='".$_POST["picpath"]."',ismarry=".$this->intnonull($_POST['ismarry']).", province='".$_POST["province"]."', city='".$_POST["city"]."',stocklevelid='".$_POST["stocklevelid"]."',attendancecode='".$_POST["attendancecode"]."' WHERE stock_id =".$id);
...
return $nostr;
	...
	function quit($info){
		exit("<script>alert('$info');location.href='stock.php';</script>");
	}
