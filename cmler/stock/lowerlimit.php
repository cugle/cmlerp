...
class Pagecustomer extends admin {
	function disp(){
		...
		if($ftable==''){$condition="s.".$category.'="'.$keywords.'"';}else{$condition="s.".$category.'="'.$keywords.'"';}
		}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'stock s INNER JOIN '.WEB_ADMIN_TABPOX."produce f on s.produce_id =f.produce_id  where f.lowerlimit >s.number and  s.agencyid =".$_SESSION["currentorgan"] ." and ".$condition ;

			}else if($ftable<>''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'stock s INNER JOIN '.WEB_ADMIN_TABPOX."produce f on s.produce_id =f.produce_id  where f.lowerlimit >s.number and f.".$category." like '%".$keywords."%' and  s.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'stock s INNER JOIN '.WEB_ADMIN_TABPOX."produce f on s.produce_id =f.produce_id  where f.lowerlimit >s.number and  s.agencyid =".$_SESSION["currentorgan"] ;
			 
			}
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  stock_id ASC  LIMIT ".$offset." , ".$psize);
