...
//select s.*,td.real_number from  s_stock s left join s_takestockdetail td on s.produce_id =td.produce_id where s.warehouse_id =1 and s.agencyid=1 
		...
		$t -> set_var('submittoauditdisabled',''); 
		$t -> set_var('path',WEB_ADMIN_HTTPPATH.'/common/');
		$t -> parse('out','f');
		$t -> p('out');	
		}
		...
		//插入明细
			...
			$inrs -> Close();					
			 ...
			 $this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."takestockdetail` SET  `sys_number`='". $sys_number."', `real_number`='".$real_number."' WHERE produce_id=".$produce_id[$i]." and `warehouse_id`='".$_POST["warehouse_id"]."' and  `agencyid`='".$_SESSION["currentorgan"]."'");
			  //echo "UPDATE `".WEB_ADMIN_TABPOX."takestockdetail` SET  `sys_number`='". $sys_number."', `real_number`='".$real_number."' WHERE produce_id=".$produce_id[$i]." and `warehouse_id`='".$_POST["warehouse_id"]."' and  `agencyid`='".$_SESSION["currentorgan"]."'";
