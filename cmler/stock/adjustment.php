...
function audit(){	 
		...
		$res= $this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."adjustment` SET `status`=4  WHERE adjustment_id =".$_GET['id']);	
  
		//更新库存
		$this->stockObj=new stock();
		$warehouse_id=$_POST['warehouse_id'];
 		$adjustmentdata=$this -> dbObj -> GetRow("SELECT * FROM `".WEB_ADMIN_TABPOX."adjustment`  WHERE adjustment_id =".$_GET['id']);
		$inrs=$this -> dbObj -> Execute("SELECT * FROM `".WEB_ADMIN_TABPOX."adjustmentdetail`  WHERE adjustment_id =".$_GET['id']);	 
		...
		$res1=$this->stockObj->addnumber($inrrs["produce_id"],$inrrs['number'],$meanprice*$inrrs['number'],$warehouse_id,$agencyid);//改变库存
		
		$res3=$this->prodaybooksObj->main($inrrs['produce_id'],$adjustmentdata["warehouse_id"],$inrrs['number'],$meanprice*$inrrs['number'],8,$_GET['id'],$adjustmentdata['man'],"调整单; ".$adjustmentdata['memo'],$inrrs["agencyid"]);//商品流水账	
		...
		$this -> dbObj -> Execute("ROLLBACK");
		 exit("<script>alert('发生错误，提交失败，数据已经回滚。');window.location.href='adjustment.php';</script>");
		} 
...
function  addorder(){
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  order_id DESC  LIMIT 0 ,1");
			...
			//插入进货记录
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."adjustment` (`adjustment_no`,`order_id`,`warehouse_id`,`suppliers_id`,`employee_id`,`acount`,`creattime`,`memo`,`adjustment_time`,`man`,`agencyid`) VALUES ('" .$_GET["adjustment_no"]."','".$_GET['orderid']."', '".$inrrs["warehouse_id"]."', '" .$inrrs["suppliers_id"]."','".$inrrs["employee_id"]."', '" .$inrrs["acount"]."','".date('Y-m-d',time())."', '".$inrrs["memo"]."', '".date('Y-m-d',time())."','".$man."','".$_SESSION["currentorgan"]."')");
			//echo "INSERT INTO `".WEB_ADMIN_TABPOX."adjustment` (`adjustment_no`,`order_id`,`warehouse_id`,`suppliers_id`,`employee_id`,`acount`,`creattime`,`memo`,`agencyid`) VALUES ('" .$_GET["adjustment_no"]."','".$_GET['orderid']."', '".$inrrs["warehouse_id"]."', '" .$inrrs["suppliers_id"]."','".$inrrs["employee_id"]."', '" .$inrrs["acount"]."','".date('Y-m-d',time())."', '".$inrrs["memo"]."','".$_SESSION["currentorgan"]."')";
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."adjustmentdetail` (adjustment_id,produce_id,number,adjustment_price,price,discount,totalacount,memo,warehouse_id,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".$inrrs1["number"]."','".$inrrs1["orderprice"]*$purchdiscount."','".$inrrs1["orderprice"]."','".$purchdiscount."','".$inrrs1["totalacount"]."','".$inrrs1["memo"]."','".$inrrs1[warehouse_id]."','".$_SESSION["currentorgan"]."')");
				//	echo "INSERT INTO `".WEB_ADMIN_TABPOX."adjustmentdetail` (adjustment_id,produce_id,number,adjustment_price,totalacount,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".$inrrs1["number"]."','".$inrrs1["orderprice"]."','".$inrrs1["totalacount"]."','".$_SESSION["currentorgan"]."')";
					
					}
			
				}
	   exit("<script>window.location.href='adjustment.php?action=upd&updid=".$id."';</script>");
}
	function disp(){
		//定义模板
		$t = new Template('../template/stock');
		$t -> set_file('f','adjustment.html');
		$t->unknowns = "remove";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		$t -> set_block('f','mainlist','ml');		
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
	    	...
	    	$t -> set_var('recordcount',$count);	
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				$t -> set_var('printbill','<A href="?action=printbill&updid='.$inrrs['adjustment_id'].'">打印</A>');					
				$t -> parse('ml','mainlist',true);
			}
			$inrs -> Close();	
	...
	function goDispAppend(){
		...
		$t = new Template('../template/stock');
		$t -> set_file('f','adjustment_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
...
if($_POST['cproduce_id']!=''&&$_POST['cnumber']!=''){			
		...
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."adjustment` SET `acount` =".$acount." where adjustment_id=".$id) ;	
			...
			$id = $_POST[MODIFY.'id'];
			$man=$this -> dbObj -> getone('select username from '.WEB_ADMIN_TABPOX.'user where userid = '.$this->getUid());
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."adjustment` SET  `adjustment_no` = '".$_POST["adjustment_no"]."',`date` = '".$_POST["date"]."',`forwhat` = '".$_POST["forwhat"]."',`memo` = '".$_POST["memo"]."',`warehouse_id` = '".$_POST["warehouse_id"]."',`employee_id` = '".$_POST["employee_id"]."',`man`='".$man."'  WHERE adjustment_id =".$id);
			
echo "UPDATE `".WEB_ADMIN_TABPOX."adjustment` SET  `adjustment_no` = '".$_POST["adjustment_no"]."',`date` = '".$_POST["date"]."',`forwhat` = '".$_POST["forwhat"]."',`memo` = '".$_POST["memo"]."',`warehouse_id` = '".$_POST["warehouse_id"]."', `employee_id` = '".$_POST["employee_id"]."',`man`='".$man."'  WHERE adjustment_id =".$id;
		//echo $_POST["con_act"];
 if($_POST["con_act"]=='upd'){
		...
		if($_POST['issave']=='1'){
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."adjustment` SET `status`=0  WHERE adjustment_id =".$id);//0未提交，1已提交2已收部分3全部收完
	 
		

		exit("<script>alert('保存成功');window.location.href='adjustment.php';</script>");	
