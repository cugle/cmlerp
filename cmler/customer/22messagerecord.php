...
}else
{
	...
	function goAppend(){
		...
		if($this -> isAppend){
	        ...
	        echo $_POST['booktime']." ".$_POST['h'].":".$_POST['i'].":00";
			...
			$result=file_get_contents("http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$content1."&destnumbers=".$handphone[$i]."&sendTime=".date("Y-m-d H:i:s",$sendtime)."&bookTime=".date("Y-m-d H:i:s",strtotime($booktime)));
			echo "http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$content1."&destnumbers=".$handphone[$i]."&sendTime=".date("Y-m-d H:i:s",$sendtime)."&bookTime=".date("Y-m-d H:i:s",strtotime($booktime));
			if($sqlstr==''){
...
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."messagerecord` (`customer_id`, `customer_name`, `title`, `content`, `senddate`,  `handphone`, `employee_id`, `man`,  `state`, `agencyid`)VALUES ( '".$_POST["customer_id"]."','".$_POST["customer_name"]."', '".$_POST["title"]."','".$_POST["senddate"]."', '".$_POST["handphone"]."', '".$this->intnonull($_POST["employee_id"])."','".$_POST["man"]."', '".$this->intnonull($_POST["state"])."','".$_SESSION["currentorgan"]."')";
            ...
            $content=$_POST["content"];
			$booktime=$_POST['booktime']." ".$_POST['h'].":".$_POST['i'].":00";
			//$customer_name=$this -> dbObj -> getone('select c.customer_name from '.WEB_ADMIN_TABPOX.'customer c INNER JOIN '.WEB_ADMIN_TABPOX.'messagerecord mr on c.customer_id =mr.customer_id   where mr.messagerecord_id ='.$id);
			//echo $customer_name;
			//echo $content;
			//$content=strtr($content,"customername",$customer_name);
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."messagerecord` SET `customer_name` = '".$_POST["customer_name"]."',`customer_id` = '".$_POST["customer_id"]."',`title` = '".$_POST["title"]."',`content` = '".$content."',`senddate` ='".$_POST["senddate"]."',`handphone` = '".$_POST["handphone"]."',`employee_id` = '".$this->intnonull($_POST["employee_id"])."',`man` = '".$_POST["man"]."',`state` = '".$this->intnonull($_POST["state"])."', `messagetypeid`=".$_POST["messagetypeid"]." WHERE messagerecord_id =".$id);
echo "UPDATE `".WEB_ADMIN_TABPOX."messagerecord` SET `customer_name` = '".$_POST["customer_name"]."',`customer_id` = '".$_POST["customer_id"]."',`title` = '".$_POST["title"]."',`content` = '".$content."',`senddate` ='".$_POST["senddate"]."',`handphone` = '".$_POST["handphone"]."',`employee_id` = '".$this->intnonull($_POST["employee_id"])."',`man` = '".$_POST["man"]."',`state` = '".$this->intnonull($_POST["state"])."', `messagetypeid`=".$_POST["messagetypeid"]." WHERE messagerecord_id =".$id;

			//echo 'select c.customer_name from '.WEB_ADMIN_TABPOX.'customer c INNER JOIN '.WEB_ADMIN_TABPOX.'messagerecord mr on c.customer_id =mr.customer_id   where mr.messagerecord_id ='.$id;
			$result=file_get_contents("http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$content."&destnumbers=".$_POST["handphone"]."&sendTime=".date("Y-m-d H:i:s",time())."&bookTime=".date("Y-m-d H:i:s",strtotime($booktime));
			
			echo "http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$content."&destnumbers=".$_POST["handphone"]."&sendTime=".date("Y-m-d H:i:s",time());
		//	echo '<br>';
			//echo mb_convert_encoding("http://59.42.247.51/http.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$_POST["content"]."&destnumbers=".$_POST["handphone"]."&sendTime=".date("Y-m-d H:i:s",time()), "UTF-8", "GBK");
			$a=explode('&',$result);
            $b= explode('=',$a[0]);
			$state=$b[1];
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."messagerecord` SET `state` = '".$state."'  WHERE messagerecord_id =".$id);
