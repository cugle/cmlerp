...
class Pagecustomer extends admin {
	function disp(){
		...
		$t -> set_file('f','visiterecordtemplate.html');
		$t->unknowns = "remove";
		...
		if($ftable==''){$condition=$category.'="'.$keywords.'"';}else{$condition=$category.'="'.$keywords.'"';}
			...
			//$inrs = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'produce  where agencyid ='.$_SESSION["currentorgan"]);
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				$t -> set_var($inrrs);
			   	$t -> set_var('delete',$this -> getDelStr('',$inrrs['visiterecordtemplate_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['visiterecordtemplate_id']));				
	...
	function goDispAppend(){
		...
		$t = new Template('../template/customer');
		$t -> set_file('f','visiterecordtemplate_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		...
		$t -> set_var('visiterecordtemplate_name',"");	
		$t -> set_var('content',"");	
		$t -> set_var('basis',"");	
		$t -> set_var('error',"");	
		...
		$t -> set_var('memo',"");
			...
			$data=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'visiterecordtemplate WHERE visiterecordtemplate_id = '.$updid);
			$t -> set_var($data);
			if ($data['picpath']==''){
...
}else
{
	...
	function goDelete(){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'visiterecordtemplate WHERE visiterecordtemplate_id in('.$delid.')');
		$info='删除';
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '增加';	
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."visiterecordtemplate` (`visiterecordtemplate_name`, `content`, `agencyid`)VALUES ( '".$_POST["visiterecordtemplate_name"]."','".$_POST["content"]."','".$_SESSION["currentorgan"]."')");
...
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."customer` (`customer_no`, `customer_name`, `address`, `genderid`, `birthday`,  `idnumber`, `tel`, `handphone`,  `zipcode`, `email`,  `memo`,  `agencyid`, `picpath`,ismarry,province,city)VALUES ( '".$_POST["customer_no"]."','".$_POST["customer_name"]."', '".$_POST["address"]."','".$_POST["genderid"]."', '".$_POST["birthday"]."', '".$_POST["idnumber"]."','".$_POST["tel"]."', '".$_POST["handphone"]."','".$this->intnonull($_POST["zipcode"])."', '".$_POST["email"]."','".$_POST["memo"]."','".$_SESSION["currentorgan"]."','".$_POST["picpath"]."','".$this->intnonull($_POST["ismarry"])."', '".$_POST["province"]."','".$_POST["city"]."')";
			...
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."visiterecordtemplate` SET `visiterecordtemplate_name` = '".$_POST["visiterecordtemplate_name"]."',`content` = '".$_POST["content"]."' WHERE visiterecordtemplate_id =".$id);
...
//$this -> quit($info.'成功！');
	...
	function quit($info){
		exit("<script>alert('$info');location.href='visiterecordtemplate.php';</script>");
	}
