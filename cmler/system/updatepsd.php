...
class PageUser extends admin {
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			if($_POST['newpass']){
				$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."user SET username='".$_POST['username']."',userpass='".md5($_POST['newpass'])."' WHERE userid = $id");
				//echo 'UPDATE '.WEB_ADMIN_TABPOX."user SET username='".$_POST['username']."',userpass='".md5($_POST['password'])."' WHERE userid = $id";
			}else{
