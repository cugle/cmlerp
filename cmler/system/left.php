	...
	function goModify(){
		...
		if($_POST['ruleurl'] && isset($_POST['roleid'])){
			foreach ($_POST['roleid'] as $v1){
				$v = $_POST['base_'.$v1];
				$this -> dbObj -> Execute('INSERT INTO '.WEB_ADMIN_TABPOX.'rolerule(roleid,ruleid,issuperuser,canbrowse,canappend,canmodify,candelete,importer)'.
							"VALUES($v1,$updId,".
							(in_array('S',$v)?1:0).','.
							(in_array('B',$v)?1:0).','.
							(in_array('A',$v)?1:0).','.
							(in_array('M',$v)?1:0).','.
							(in_array('D',$v)?1:0).','.$this->getUid().')');
			}
