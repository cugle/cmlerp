...
class Pagecustomer extends admin {
    ...
    {   
        ...
        { 
            ...
            $this -> checkUser();//验证身份，这一步很重要。
            $this -> recoil();			
		}else if(isset($_GET['action']) && $_GET['action']=='manualcommissionexport'){
            $this -> checkUser();//验证身份，这一步很重要。
            $this -> manualcommissionexport();			
		}else{
            parent::Main();
        }
    }
function manualcommissionexport(){
		//定义模板
		$t = new Template('../template/analysis');
		$t -> set_file('f','manualcommissionexport.html');
		$t->unknowns = "remove";
...
function recoil(){
	...
	function disp(){
		//定义模板
		$t = new Template('../template/analysis');
		$t -> set_file('f','manualcommission.html');
		$t->unknowns = "remove";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		...
		if($ftable==''){$condition=$category.' like "'.$keywords.'"';}else{$condition=$category.' like "'.$keywords.'"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'sell A RIGHT JOIN '.WEB_ADMIN_TABPOX.'manualcommission  B ON A.sell_id=B.sell_id  where  A.agencyid ='.$_SESSION["currentorgan"].' and '.$condition.$hiddenredstr1 ;

			}else if($ftable<>''){
			if($ftable=="employee"){$ftable1="beauty";}else{$ftable1=$ftable;}
			$sql="select A.status as fstatus ,F.".$ftable."_id,A.*,B.* from ( ".WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'manualcommission B ON A.sell_id=B.sell_id)  INNER JOIN '.WEB_ADMIN_TABPOX.$ftable.' F on B.'.$ftable1.'_id =F.'.$ftable.'_id  where F.'.$category." like '".$keywords."' and A.agencyid =".$_SESSION["currentorgan"].$hiddenredstr1;
			

			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'manualcommission B ON A.sell_id=B.sell_id  where A.agencyid ='.$_SESSION["currentorgan"].$hiddenredstr1;
...
/*		$condition='';
		...
		if($ftable==''){$condition=$category.' like "'.$keywords.'"';}else{$condition=$category.' like "'.$keywords.'"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'manualcommission  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql="select * from ".WEB_ADMIN_TABPOX."manualcommission s INNER JOIN ".WEB_ADMIN_TABPOX."$ftable f on s.sell_id =f.sell_id  where f.".$category." like '%".$keywords."%' and f.agencyid =".$_SESSION["currentorgan"] ;
			
			 
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'manualcommission  where  agencyid ='.$_SESSION["currentorgan"];
			 
			}
			
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  manualcommission_id DESC,manualcommission_id DESC LIMIT ".$offset." , ".$psize);*/
	    	...
	    	$t -> set_var('recordcount',$count);	
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				$t -> set_var($inrrs);
				$selldata=$this -> dbObj -> getrow('select * from '.WEB_ADMIN_TABPOX.'sell  where sell_id ='.$inrrs["sell_id"]);
				$customername=$this -> dbObj -> getone('select customer_name from '.WEB_ADMIN_TABPOX.'customer  where customer_id ='.$selldata["customer_id"]);
				$customername=$customername<>''?$customername:'散客';
				$t -> set_var('customer_name',$customername);
				$t -> set_var('customer_id',$selldata["customer_id"]);
				$t -> set_var('sell_no',$selldata["sell_no"]);
			$type_name=array("销售收入","销售收入","储值卡充值收入","<font color=red>被红字冲销</font>");
			$t -> set_var('type_name',$type_name[$inrrs['type']]);
 
				$t -> set_var('view','<a href="#" onclick=viewbill('.$inrrs['sell_id'].');>查看单据</a>'); 
				$t -> set_var('print','<a href="?action=print&updid='.$inrrs['sell_id'].'" target="_blank">打印</a>');
				$t -> set_var('services_name',$this -> dbObj -> getone('select services_name from '.WEB_ADMIN_TABPOX.'services  where services_id ='.$inrrs["services_id"]));
				$beauty=$this -> dbObj -> getrow('select *  from '.WEB_ADMIN_TABPOX.'employee  where employee_id ='.$inrrs["beauty_id"]);
				$t -> set_var('beauty_name',$beauty['employee_name']);
				$t -> set_var('beauty_no',$beauty['employee_no']);
