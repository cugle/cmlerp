...
function recoil(){
	...
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "'.$keywords.'"';}else{$condition=$category.' like "'.$keywords.'"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'selldetail B ON A.sell_id=B.sell_id  where  A.agencyid ='.$_SESSION["currentorgan"].' and '.$condition.$hiddenredstr1 ;
			$sql1='select sum(B.amount) as totalaccount, sum(B.number) as totalnumber from '.WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'selldetail B ON A.sell_id=B.sell_id  where  A.agencyid ='.$_SESSION["currentorgan"].' and '.$condition.$hiddenredstr1 ;
			}else if($ftable<>''){
			if($ftable=='produce'){
				$sql="select A.status as fstatus ,F.".$ftable."_id,A.*,B.* from ( ".WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'selldetail B ON A.sell_id=B.sell_id)  INNER JOIN '.WEB_ADMIN_TABPOX.$ftable.' F on B.item_id =F.'.$ftable.'_id  where F.'.$category." like '%".$keywords."%' and A.agencyid =".$_SESSION["currentorgan"].$hiddenredstr1;
			$sql1="select sum(B.amount) as totalaccount, sum(B.number) as totalnumber from ( ".WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'selldetail B ON A.sell_id=B.sell_id)  INNER JOIN '.WEB_ADMIN_TABPOX.$ftable.' F on B.item_id =F.'.$ftable.'_id  where F.'.$category." like '%".$keywords."%' and A.agencyid =".$_SESSION["currentorgan"].$hiddenredstr1;
		 
			}else{
			$sql="select A.status as fstatus ,F.".$ftable."_id,A.*,B.* from ( ".WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'selldetail B ON A.sell_id=B.sell_id)  INNER JOIN '.WEB_ADMIN_TABPOX.$ftable.' F on A.'.$ftable.'_id =F.'.$ftable.'_id  where F.'.$category." like '".$keywords."' and A.agencyid =".$_SESSION["currentorgan"].$hiddenredstr1;
			$sql="select sum(B.amount) as totalaccount, sum(B.number) as totalnumber from ( ".WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'selldetail B ON A.sell_id=B.sell_id)  INNER JOIN '.WEB_ADMIN_TABPOX.$ftable.' F on A.'.$ftable.'_id =F.'.$ftable.'_id  where F.'.$category." like '".$keywords."' and A.agencyid =".$_SESSION["currentorgan"].$hiddenredstr1;
			}
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'selldetail B ON A.sell_id=B.sell_id  where A.agencyid ='.$_SESSION["currentorgan"].$hiddenredstr1;
			$sql1='select  sum(B.amount) as totalaccount, sum(B.number) as totalnumber  from '.WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'selldetail B ON A.sell_id=B.sell_id  where A.agencyid ='.$_SESSION["currentorgan"].$hiddenredstr1;
			}
...
/*		$condition='';
		...
		if($ftable==''){$condition=$category.' like "'.$keywords.'"';}else{$condition=$category.' like "'.$keywords.'"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'selldetail  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql="select f.status as fstatus ,f.customer_id,s.* from ".WEB_ADMIN_TABPOX."selldetail s INNER JOIN ".WEB_ADMIN_TABPOX."$ftable f on s.sell_id =f.sell_id  where f.".$category." like '".$keywords."' and f.agencyid =".$_SESSION["currentorgan"] ;
			
			 
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'selldetail  where  agencyid ='.$_SESSION["currentorgan"];
			 
			}*/
			
			 
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  B.selldetail_id DESC,A.sell_id DESC LIMIT ".$offset." , ".$psize);
			$result = &$this -> dbObj -> Execute($sql);	
			
			$inrrs=&$this -> dbObj ->GetArray($sql." ORDER BY  B.selldetail_id DESC,A.sell_id DESC LIMIT ".$offset." , ".$psize);
			$inrrscount=sizeof($inrrs);
			$total=&$this -> dbObj ->GetRow($sql1);
			$t -> set_var('totalaccount',$total['totalaccount']);
			$t -> set_var('totalnumber',$total['totalnumber']);
			...
			for($k=$inrrscount-1;$k>=0;$k--){
				$t -> set_var($inrrs[$k]);	
				//$t -> set_var($inrrs);
				 $customerid=$this -> dbObj -> getone('select customer_id from '.WEB_ADMIN_TABPOX.'sell  where sell_id ='.$inrrs[$k]["sell_id"]) ;
			...
			//$t -> set_var('status_name',$status_name[$inrrs[$k]['status']]);
				...
				$t -> set_var('sell_no',$this -> dbObj -> getone('select sell_no from '.WEB_ADMIN_TABPOX.'sell  where sell_id ='.$inrrs[$k]["sell_id"]));
				...
				$t -> set_var('creattime',$this -> dbObj -> getone('select creattime from '.WEB_ADMIN_TABPOX.'sell  where sell_id ='.$inrrs[$k]["sell_id"]));
				$t -> set_var('marketingcard_name',$this -> dbObj -> getone('select marketingcard_name  from '.WEB_ADMIN_TABPOX.'marketingcard  where marketingcard_id ='.$inrrs[$k]["cardid"])); 
				$t -> set_var('marketingcardtype_name',$this -> dbObj -> getone('select marketingcardtype_name  from '.WEB_ADMIN_TABPOX.'marketingcardtype  where marketingcardtype_id ='.$inrrs[$k]["cardtype"]));
				$t -> set_var('produce_name',$this -> dbObj -> getone('select produce_name  from '.WEB_ADMIN_TABPOX.'produce  where produce_id ='.$inrrs[$k]["item_id"]));
				$itemtype_name=array('单项服务',"购买产品","消费卡项","购买卡项","消费券项");	
				 
				$t -> set_var('itemtype_name',$itemtype_name[$inrrs[$k]["item_type"]]);
				...
				$t -> set_var('giving',$inrrs[$k]['discount']==0?"赠送":"");
				$t -> set_var('status_name',$status_name[$inrrs[$k]['status']]);				
