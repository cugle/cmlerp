...
class Pagecustomer extends admin {
	function disp(){
		...
		if($ftable==''){$condition=$category.'="'.$keywords.'"';}else{$condition=$category.'="'.$keywords.'"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'membercard  where  agencyid ='.$_SESSION["currentorgan"].' and  `salesowe` >0 and '.$condition ;
			$totalsql='select sum(salesowe) as totalown from '.WEB_ADMIN_TABPOX.'membercard  where  agencyid ='.$_SESSION["currentorgan"].' and  `salesowe` >0 and '.$condition ;
			}else if($ftable<>''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'membercard  p INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on p.customer_id =f.customer_id  where f.customer_name like '%".$keywords."%'  and p.`salesowe` >0 and  p.agencyid =".$_SESSION["currentorgan"] ;
			$totalsql='select sum(salesowe) as totalown  from '.WEB_ADMIN_TABPOX.'membercard  p INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on p.customer_id =f.customer_id  where f.customer_name like '%".$keywords."%'  and p.`salesowe` >0 and  p.agencyid =".$_SESSION["currentorgan"] ;
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'membercard   where   `salesowe` >0  and agencyid ='.$_SESSION["currentorgan"];
			$totalsql= 'select  sum(salesowe) as totalown from '.WEB_ADMIN_TABPOX.'membercard   where   `salesowe` >0  and agencyid ='.$_SESSION["currentorgan"];
			}
			 
			$totalown= &$this -> dbObj -> GetOne($totalsql);
	...
	function goDispAppend(){
		...
		$t = new Template('../template/analysis');
		$t -> set_file('f','memberown_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
...
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."membercard` (`membercard_no`, `cardlevel_id`, `customer_id`, `score`,`startdate`,`overdate`,` man`, ` agencyid`)VALUES ( '".$_POST["membercard_no"]."','".$_POST["cardlevel_id"]."', '".$_POST["customer_id"]."','".$this->intnonull($_POST["score"])."', '".$_POST["startdate"]."', '".$_POST["overdate"]."',  '".$_POST["man"]."', '".$_SESSION["currentorgan"]."')";
			...
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."membercard` SET `salesowe` = '".$_POST["salesowe"]."'  WHERE membercard_id =".$id);
...
return $nostr;
	...
	function quit($info){
		exit("<script>alert('$info');location.href='memberown.php';</script>");
	}
