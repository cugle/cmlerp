...
class Pagecustomer extends admin {
    ...
    {   
        ...
        { 
            ...
            $this -> checkUser();//验证身份，这一步很重要。
            $this -> recoil();			
		}else if(isset($_GET['action']) && $_GET['action']=='sellcardexport'){
            $this -> checkUser();//验证身份，这一步很重要。
            $this -> sellcardexport();			
		}else{
            parent::Main();
	...
	function sellcardexport(){
		//定义模板
		$t = new Template('../template/analysis');
		$t -> set_file('f','sellcardexport.html');
		$t->unknowns = "remove";
...
function recoil(){
	...
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "'.$keywords.'"';}else{$condition=$category.' like "'.$keywords.'"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'sellcarddetail B ON A.sell_id=B.sell_id  where  A.agencyid ='.$_SESSION["currentorgan"].' and '.$condition.$hiddenredstr1 ;

			}else if($ftable<>''){

			$sql="select A.status as fstatus ,F.".$ftable."_id,A.*,B.* from ( ".WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'sellcarddetail B ON A.sell_id=B.sell_id)  INNER JOIN '.WEB_ADMIN_TABPOX.$ftable.' F on A.'.$ftable.'_id =F.'.$ftable.'_id  where F.'.$category." like '".$keywords."' and A.agencyid =".$_SESSION["currentorgan"].$hiddenredstr1;
			

			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'sellcarddetail B ON A.sell_id=B.sell_id  where A.agencyid ='.$_SESSION["currentorgan"].$hiddenredstr1;
	    	...
	    	$t -> set_var('recordcount',$count);	
			 while ($inrrs = &$inrs -> FetchRow()) {
				...
				$customernamedata=$this -> dbObj -> GetRow('select * from '.WEB_ADMIN_TABPOX.'customer  where customer_id ='.$customerid);
				$customername=$customernamedata['customer_name'];
				$customerno=$customernamedata['customer_no'];
				$customername=$customername<>''?$customername:'散客';
				$t -> set_var('customer_name',$customername);
				$t -> set_var('customer_id',$customerid);
				$t -> set_var('customer_no',$customerno);
			...
			//$t -> set_var('status_name',$status_name[$inrrs['status']]);
				...
				$beautyid=explode(";",$inrrs["beauty_id"]);
				$beauty_str='';
				for ($i=0;$i<count($beautyid);$i++){
				$beauty_name=$this -> dbObj -> getone('select employee_name  from '.WEB_ADMIN_TABPOX.'employee  where employee_id ='.$beautyid[$i]);	
				$beauty_str=$beauty_str.'<a href="?category=beauty_id&keywords='.$beautyid[$i].'"> '.$beauty_name.' </a>';
				}
				$t -> set_var('beauty_name',$beauty_str);
				...
				$t -> set_var('marketingcard_name',$this -> dbObj -> getone('select marketingcard_name  from '.WEB_ADMIN_TABPOX.'marketingcard  where marketingcard_id ='.$inrrs["item_id"])); 
				$t -> set_var('marketingcardtype_name',$this -> dbObj -> getone('select marketingcardtype_name  from '.WEB_ADMIN_TABPOX.'marketingcardtype  where marketingcardtype_id ='.$inrrs["cardtype"]));
...
function print1(){
		...
		$sql3="select * from  ".WEB_ADMIN_TABPOX."sellcarddetail  where  sell_id=".$sellid;
...
//设置供应商
	...
	function goDelete(){
		...
		//$this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'sellcarddetail WHERE sell_id in('.$delid.')');
		//echo 'DELETE FROM '.WEB_ADMIN_TABPOX.'sellcarddetail WHERE selldetail_id in('.$delid.')';
 		$this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'sellcarddetail WHERE selldetail_id in('.$delid.')');
		$info='删除';
