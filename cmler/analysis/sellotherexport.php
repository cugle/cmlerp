<?php
$filename="款项历史单".date('Y-m-d').".xls";//先定义一个excel文件
...
$sql="SELECT A.status as sellstatus,A.*,B.* , C.* FROM (s_sell A RIGHT JOIN s_sellotherdetail  B ON a.sell_id=B.sell_id ) LEFT JOIN s_employee C ON C.employee_id=A.employee_id where A.agencyid = ".$_POST["agencyid"].' and '.$timecondition;
...
$itemname=$itemnamelist[$row['item_id']];
$cardtype=$cardtypelist[$row['cardtype']];
echo $row['item_id']."\t";
echo $cardtype."\t";
echo $itemname."\t";
