...
class Pagecustomer extends admin {
	...
	function goDispAppend(){
		...
		$t -> set_var('membercardlist',$this ->cardlist('marketingcard','marketingcard_id','marketingcard_name','',5));
		...
		//设置消耗品列表
			...
			$inrs = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'marketingcarddetail  where marketingcard_id  ='.$updid);
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				//echo 'select * from '.WEB_ADMIN_TABPOX.'services   where services_id ='.$inrrs["services_id"].' and agencyid ='.$_SESSION["currentorgan"];
				$data2=$this -> dbObj ->  Execute('select * from '.WEB_ADMIN_TABPOX.'servicecategory    where category_id  in ('.$inrrs["servicecategory_id"].')');
				 //echo 'select * from '.WEB_ADMIN_TABPOX.'servicecategory    where category_id  IN('.$inrrs["servicecategory_id"].')';
				$category_name='';
		...
		// 修改消耗品
		   ...
		   $t -> set_var('caction','upd');	
			...
			$t -> set_var('ccategory_id',$inrs2['servicecategory_id']);	
			$inrs4 = &$this -> dbObj -> Execute('select category_name  from '.WEB_ADMIN_TABPOX.'servicecategory   where category_id in ('.$inrs2['servicecategory_id'].')');
			
			$servicecategory_name='';
				while ($inrrs4 = $inrs4 -> FetchRow()) {
				$servicecategory_name=$servicecategory_name==''?$inrrs4['category_name']:$servicecategory_name.','.$inrrs4['category_name'];
					 
				}			
			$t -> set_var('ccategory_name',$servicecategory_name);
			//$t -> parse('cs','consumables',true);
			//$inrs2 -> Close();	
			}
		$t -> set_var('memo',$date['memo']);	
		
		$t -> set_var('membercardlist',$this ->cardlist('marketingcard','marketingcard_id','marketingcard_name',$date['inheritedid'],5));	
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."marketingcard` (`marketingcard_name`, `marketingcard_no`,`marketingcardtype_id`, `coderule`,`totaltimes`, `price`, `timelimit`, `pricepertime`, `assignservice`,`memo`, `agencyid`,`sellcommission`,`beautycommission`,`leadercommission`,inheritedid,bgdate,enddate,limitdate) VALUES ('" .$_POST["marketingcard_name"]."','".$_POST["marketingcard_no"]."', ".$marketingcardtype_id.",'".$_POST["coderule"]."', '".$_POST["totaltimes"]."', '".$this->intnonull($_POST["price"])."',  '".$this->intnonull($_POST["timelimit"])."','".$this->intnonull($_POST["pricepertime"])."','".$this->intnonull($_POST["assignservice"])."', '".$_POST["memo"]."','".$_SESSION["currentorgan"]."', '".$_POST["sellcommission"]."', '".$_POST["beautycommission"]."', '".$_POST["leadercommission"]."','".$_POST["inheritedid"]."','".$_POST["bgdate"]."','".$_POST["enddate"]."','".$limitdate."')");
			}else{
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."marketingcard` (`marketingcard_name`, `marketingcard_no`,`marketingcardtype_id`, `coderule`,`totaltimes`, `price`, `timelimit`, `pricepertime`, `assignservice`,`memo`, `agencyid`,`sellcommission`,`beautycommission`,`leadercommission`,inheritedid,limitdate) VALUES ('" .$_POST["marketingcard_name"]."','".$_POST["marketingcard_no"]."', ".$marketingcardtype_id.",'".$_POST["coderule"]."', '".$_POST["totaltimes"]."', '".$this->intnonull($_POST["price"])."',  '".$this->intnonull($_POST["timelimit"])."','".$this->intnonull($_POST["pricepertime"])."','".$this->intnonull($_POST["assignservice"])."', '".$_POST["memo"]."','".$_SESSION["currentorgan"]."', '".$_POST["sellcommission"]."', '".$_POST["beautycommission"]."', '".$_POST["leadercommission"]."','".$_POST["inheritedid"]."', '".$limitdate."')");
			}
		...
		exit("<script>alert('新增成功');location.href='membercard.php?action=upd&updid=".$id."';</script>");	
		}else{
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$limitdate=$_POST['limitdate']=='on'?1:0;
			if($_POST['bgdate']<>''&&$_POST['enddate']<>''){
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."marketingcard` SET `marketingcard_name` = '".$_POST["marketingcard_name"]."',`marketingcard_no` = '".$_POST["marketingcard_no"]."',`marketingcardtype_id`=".$marketingcardtype_id.",`coderule` = '".$_POST["coderule"]."',`totaltimes` = '".$this->intnonull($_POST["totaltimes"])."',`timelimit` ='".$this->intnonull($_POST["timelimit"])."',`price` = '".$this->intnonull($_POST["price"])."',`pricepertime` = '".$this->intnonull($_POST["pricepertime"])."',`assignservice` = '".$this->intnonull($_POST["assignservice"])."',inheritedid='".$_POST["inheritedid"]."' ,`memo` = '".$_POST["memo"]."',`sellcommission` = '".$_POST["sellcommission"]."',`beautycommission`= '".$_POST["beautycommission"]."',leadercommission='".$_POST["leadercommission"]."' ,`bgdate` = '".$_POST["bgdate"]."',`enddate` = '".$_POST["enddate"]."',`limitdate` = '".$limitdate."'  WHERE marketingcard_id =".$id);
			 //echo "UPDATE `".WEB_ADMIN_TABPOX."marketingcard` SET `marketingcard_name` = '".$_POST["marketingcard_name"]."',`marketingcard_no` = '".$_POST["marketingcard_no"]."',`marketingcardtype_id`=".$marketingcardtype_id.",`coderule` = '".$_POST["coderule"]."',`totaltimes` = '".$this->intnonull($_POST["totaltimes"])."',`timelimit` ='".$this->intnonull($_POST["timelimit"])."',`price` = '".$this->intnonull($_POST["price"])."',`pricepertime` = '".$this->intnonull($_POST["pricepertime"])."',`assignservice` = '".$this->intnonull($_POST["assignservice"])."',inheritedid='".$_POST["inheritedid"]."' ,`memo` = '".$_POST["memo"]."',`sellcommission` = '".$_POST["sellcommission"]."',`beautycommission`= '".$_POST["beautycommission"]."',leadercommission='".$_POST["leadercommission"]."' WHERE marketingcard_id =".$id;
		//echo $_POST["con_act"];
		}else{
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."marketingcard` SET `marketingcard_name` = '".$_POST["marketingcard_name"]."',`marketingcard_no` = '".$_POST["marketingcard_no"]."',`marketingcardtype_id`=".$marketingcardtype_id.",`coderule` = '".$_POST["coderule"]."',`totaltimes` = '".$this->intnonull($_POST["totaltimes"])."',`timelimit` ='".$this->intnonull($_POST["timelimit"])."',`price` = '".$this->intnonull($_POST["price"])."',`pricepertime` = '".$this->intnonull($_POST["pricepertime"])."',`assignservice` = '".$this->intnonull($_POST["assignservice"])."',inheritedid='".$_POST["inheritedid"]."' ,`memo` = '".$_POST["memo"]."',`sellcommission` = '".$_POST["sellcommission"]."',`beautycommission`= '".$_POST["beautycommission"]."',leadercommission='".$_POST["leadercommission"]."' ,`bgdate` = NULL,`enddate` = NULL,`limitdate` = '".$limitdate."' WHERE marketingcard_id =".$id);
		}
 if($_POST["con_act"]=='upd'){
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."marketingcarddetail` SET `servicecategory_id` = '".$_POST["ccategory_id"]."', `services_id` = '".$_POST["cservices_id"]."',`pricepertime` = '".$_POST["cpricepertime"]."',`services_times` = '".$_POST["cservices_times"]."',`memo` = '".$_POST["cmemo"]."' WHERE marketingcarddetail_id  =".$_POST['conupdid']);		
		//echo "UPDATE `".WEB_ADMIN_TABPOX."marketingcarddetail` SET `servicecategory_id` = '".$_POST["ccategory_id"]."', `services_id` = '".$_POST["cservices_id"]."',`pricepertime` = '".$_POST["cpricepertime"]."',`services_times` = '".$_POST["cservices_times"]."',`memo` = '".$_POST["cmemo"]."' WHERE marketingcarddetail_id  =".$_POST['conupdid'];
		
		exit("<script>alert('修改成功');location.href='membercard.php?action=upd&updid=".$id."';</script>");
		}else if($_POST['ccategory_id']<>''&&$_POST['cpricepertime']<>''&&$_POST['cservices_times']<>''){
