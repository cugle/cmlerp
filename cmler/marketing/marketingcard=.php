...
class Pagecustomer extends admin {
	function disp(){
		...
		$t -> set_file('f','marketingcard.html');
		$t->unknowns = "remove";
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			$sql="select * from ".WEB_ADMIN_TABPOX."marketingcard s INNER JOIN ".WEB_ADMIN_TABPOX."$ftable f on s.categoryid =f.category_id  where f.category_name like '%".$keywords."%' and  s.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'marketingcard  where  agencyid ='.$_SESSION["currentorgan"];
	...
	function goDispAppend(){
		...
		$t = new Template('../template/marketing');
		$t -> set_file('f','marketingcard_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '增加';	
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."marketingcard` (`marketingcard_name`, `marketingcard_no`, `coderule`,`totaltimes`, `price`, `timelimit`, `pricepertime`, `assignservice`,`memo`, `agencyid`) VALUES ('" .$_POST["marketingcard_name"]."','".$_POST["marketingcard_no"]."', '".$_POST["coderule"]."', '".$_POST["totaltimes"]."', '".$this->intnonull($_POST["price"])."',  '".$this->intnonull($_POST["timelimit"])."','".$this->intnonull($_POST["pricepertime"])."','".$this->intnonull($_POST["assignservice"])."', '".$_POST["memo"]."','".$_SESSION["currentorgan"]."')");
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."marketingcard` (`marketingcard_name`, `marketingcard_no`, `miner1`,`miner2`, `price`, `categoryid`, `commission`, `ucommission`,`compcent`,`ucompcent`,`memo`, `agencyid`) VALUES ('" .$_POST["marketingcard_name"]."','".$_POST["marketingcard_no"]."', '".$this->intnonull($_POST["miner1"])."','".$this->intnonull($_POST["miner2"])."','".$this->intnonull($_POST["price"])."', '".$_POST["categoryid"]."', '".$this->intnonull($_POST["commission"])."',  '".$this->intnonull($_POST["ucommission"])."','".$this->intnonull($_POST["compcent"])."','".$this->intnonull($_POST["ucompcent"])."', '".$_POST["memo"]."','".$_SESSION["currentorgan"]."')";
			$id = $this -> dbObj -> Insert_ID();
		}else{
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."marketingcard` SET `marketingcard_name` = '".$_POST["marketingcard_name"]."',`marketingcard_no` = '".$_POST["marketingcard_no"]."',`coderule` = '".$_POST["coderule"]."',`totaltimes` = '".$this->intnonull($_POST["totaltimes"])."',`timelimit` ='".$this->intnonull($_POST["timelimit"])."',`price` = '".$this->intnonull($_POST["price"])."',`pricepertime` = '".$this->intnonull($_POST["pricepertime"])."',`assignservice` = '".$this->intnonull($_POST["assignservice"])."',`memo` = '".$_POST["memo"]."' WHERE marketingcard_id =".$id);
 ...
 if($_POST["con_act"]=='upd'){
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."marketingcarddetail` SET `services_id` = '".$_POST["cservices_id"]."',`pricepertime` = '".$_POST["cpricepertime"]."',`services_times` = '".$_POST["cservices_times"]."',`memo` = '".$_POST["cmemo"]."' WHERE consumables_id  =".$_POST['conupdid']);		
		echo "UPDATE `".WEB_ADMIN_TABPOX."marketingcarddetail` SET `services_id` = '".$_POST["cservices_id"]."',`pricepertime` = '".$_POST["cpricepertime"]."',`services_times` = '".$_POST["cservices_times"]."',`memo` = '".$_POST["cmemo"]."' WHERE consumables_id  =".$_POST['conupdid'];
		}else{

		$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."marketingcarddetail` (marketingcard_id,services_id,pricepertime,services_times ,memo,agencyid) VALUES ('".$id."','".$_POST["cservices_id"]."','".$_POST["cpricepertime"]."','".$_POST["cservices_times"]."','".$_POST["cmemo"]."','".$_SESSION["currentorgan"]."')");
