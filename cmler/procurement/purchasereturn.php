...
$this -> dbObj -> Execute("START TRANSACTION");//事务开始。
	...
	$updid=$_POST['updid'];
	$res= $this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `status`=1  WHERE purchreturn_id =".$updid);//4表示被反冲3反冲2正常1未提交	
 ...
 //echo "INSERT INTO `".WEB_ADMIN_TABPOX."transfervoucher` (`transfervoucher_no`,`date`,`agencyid`,`creattime`,`man`) VALUES ('" .$transfervoucher_no."','".date('Y-m-d',time())."','".$_SESSION["currentorgan"]."','".date('Y-m-d H:i:s',time())."', '".$man."')";
		...
		//插入转账明细
				...
				//echo "INSERT INTO `".WEB_ADMIN_TABPOX."transfervoucherdetail` ( `transfervoucher_id`, `accounttitle_id`, `objecttype_id`, `objectid`, `lend`, `loan`, `memo`, `agencyid`) VALUES ('".$id."','83','2','".$suppliers_id."','".$inrrs1['totalacount']."','0','".$inrrs1['memo']."','".$_SESSION['currentorgan']."')";
				}
...
function  editorder(){
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  order_id DESC  LIMIT 0 ,1");
			...
			//$id = $this -> dbObj -> Insert_ID();
			//echo $id;
		$sqldel='delete from '.WEB_ADMIN_TABPOX.'purchasedetail where agencyid='.$_SESSION["currentorgan"].' and purchreturn_id='.$updid;
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturn_price,totalacount,agencyid) VALUES ('".$updid."','".$inrrs1["produce_id"]."','".$inrrs1["number"]."','".$inrrs1["orderprice"]."','".$inrrs1["totalacount"]."','".$_SESSION["currentorgan"]."')");
					//echo "INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturn_price,totalacount,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".$inrrs1["number"]."','".$inrrs1["orderprice"]."','".$inrrs1["totalacount"]."','".$_SESSION["currentorgan"]."')";
...
function  addorder(){
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  order_id DESC  LIMIT 0 ,1");
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturn_price,totalacount,memo,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".$inrrs1["number"]."','".$inrrs1["orderprice"]."','".$inrrs1["totalacount"]."','".$inrrs1["memo"]."','".$_SESSION["currentorgan"]."')");
				//	echo "INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturn_price,totalacount,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".$inrrs1["number"]."','".$inrrs1["orderprice"]."','".$inrrs1["totalacount"]."','".$_SESSION["currentorgan"]."')";
	...
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			$sql="select * from ".WEB_ADMIN_TABPOX."purchreturn INNER JOIN ".WEB_ADMIN_TABPOX."$ftable f on s.categoryid =f.category_id  where f.category_name like '%".$keywords."%' and  s.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'purchreturn WHERE  agencyid ='.$_SESSION["currentorgan"].' and status=0 ';
...
//设置供应商
		...
		//设置消耗品列表
			...
			$t -> set_var('ml');
			$inrs = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'purchasedetail  where purchreturn_id  ='.$updid);
	     	while ($inrrs = &$inrs -> FetchRow()) {
				$t -> set_var($inrrs);
	...
	function goDelete(){
	  if($_GET['purchdetail_id']==''){
		...
		$this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'purchasedetail WHERE purchreturn_id in('.$delid.')');
...
if($_POST['cproduce_id']!=''&&$_POST['cnumber']!=''){			
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchaseprice,totalacount,memo,agencyid) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cprice"]."','".$_POST["ctotalacount"]."','".$_POST["cmemo"]."','".$_SESSION["currentorgan"]."')");	

			}
			
		$acount = &$this -> dbObj -> GetOne('select sum(totalacount) from '.WEB_ADMIN_TABPOX.'purchasedetail    where purchreturn_id  ='.$id);
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `acount` =".$acount." where purchreturn_id=".$id) ;	
		
			exit("<script>alert('$info');window.location.href='purchreturn.php?action=upd&updid=".$id."';</script>");
		}else{
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$man=$this -> dbObj -> getone('select username from '.WEB_ADMIN_TABPOX.'user where userid = '.$this->getUid());
 ...
 if($_POST["con_act"]=='upd'){
		...
		$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturn_price,totalacount,agencyid) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cprice"]."','".$_POST["ctotalacount"]."','".$_SESSION["currentorgan"]."')");	
		...
		echo 'select sum(totalacount) from '.WEB_ADMIN_TABPOX.'purchasedetail    where purchreturn_id  ='.$id;
		$acount = &$this -> dbObj -> GetOne('select sum(totalacount) from '.WEB_ADMIN_TABPOX.'purchasedetail    where purchreturn_id  ='.$id);
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `acount` =".$acount." where purchreturn_id=".$id) ;
		}
		if($_POST['issave']=='1'){
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `status`=0  WHERE purchreturn_id =".$id);//0未提交，1已提交2已收部分3全部收完
