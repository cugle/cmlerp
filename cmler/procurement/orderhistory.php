...
class Pagecustomer extends admin {
    ...
    {   
        ...
        { 
            ...
            $this -> printorder();
        }else if(isset($_GET['action']) && $_GET['action']=='finished'){
            $this -> checkUser();//验证身份，这一步很重要。
            $this -> finished();			
    ...
    function turnback(){
	$inrs2 = &$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."order` SET `status` = '0' WHERE order_id =".$_GET['id']);	
	$info='修改订单状态';	
    ...
    function finished(){
	$inrs2 = &$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."order` SET `status` = '3' WHERE order_id =".$_GET['id']);	
	$info='修改订单状态';	
		if(mysql_affected_rows())
		$this -> quit($info.'成功！');
	    else
	...
	function  printorder(){
		...
		$t -> set_file('f','orderhistory_detail_print.html');
		$t->unknowns = "move";
...
//设置供应商
	...
	function disp(){
		//定义模板
		$t = new Template('../template/procurement');
		$t -> set_file('f','orderhistory.html');
		$t->unknowns = "remove";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		$t -> set_block('f','mainlist','ml');		
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			$status_name=array("","<font color=red>未收货</font>","<font color=green>已收部分</font>","<font color=blue>全部完成</font>");  
	     	while ($inrrs = &$inrs -> FetchRow()) {
				$t -> set_var($inrrs);
				...
				$t -> set_var('view','<a href="orderhistory.php?action=upd&updid='.$inrrs['order_id'].'">查看</a>');
				$t -> set_var('print','<a href="orderhistory.php?action=print&updid='.$inrrs['order_id'].'" target="_blank">打印</a>');
				$t -> parse('ml','mainlist',true);
			}
			$inrs -> Close();	
	...
	function goDispAppend(){
		...
		$t = new Template('../template/procurement');
		$t -> set_file('f','orderhistory_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
		...
		$t -> set_var('userid',$this->getUid());		
			...
			$data=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'order WHERE order_id = '.$updid);
			$finisheddisabled=array('disabled="disabled"','disabled="disabled"','','disabled="disabled"');  
			$t -> set_var('finisheddisabled',$finisheddisabled[$data['status']]);
			$turnbackdisabled=array('','','disabled="disabled"','disabled="disabled"'); 
			$t -> set_var('turnbackdisabled',$turnbackdisabled[$data['status']]);
			$t -> set_var('storageidlist',$this ->selectlist('warehouse','warehouse_id','warehouse_name',$data['warehouse_id']));	
			$t -> set_var('warehouse_name',$this -> dbObj -> getone('select warehouse_name from '.WEB_ADMIN_TABPOX.'warehouse where warehouse_id = '.$data['warehouse_id']));
			$t -> set_var('updid',$updid);
			$t -> set_var('action','upd');			
...
if($_POST['cproduce_id']!=''&&$_POST['cnumber']!=''){			
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."orderdetail` (order_id,produce_id,number,orderprice,totalacount,agencyid) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cprice"]."','".$_POST["ctotalacount"]."','".$_SESSION["currentorgan"]."')");	
			...
			exit("<script>alert('$info');window.location.href='order.php?action=upd&updid=".$id."';</script>");
		}else{
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."order` SET `order_name` = '".$_POST["order_name"]."',`order_no` = '".$_POST["order_no"]."',`categoryid` = '".$_POST["categoryid"]."',`miner1` = '".$this->intnonull($_POST["miner1"])."',`miner2` ='".$this->intnonull($_POST["miner2"])."',`price` = '".$this->intnonull($_POST["price"])."',`commission` = '".$this->intnonull($_POST["commission"])."',`ucommission` = '".$this->intnonull($_POST["ucommission"])."',`compcent` = '".$_POST["compcent"]."',`ucompcent` = '".$_POST["ucompcent"]."',`memo` = '".$_POST["memo"]."' WHERE order_id =".$id);
			//echo "UPDATE `".WEB_ADMIN_TABPOX."order` SET `order_name` = '".$_POST["order_name"]."',`order_no` = '".$_POST["order_no"]."',`categoryid` = '".$_POST["categoryid"]."',`miner1` = '".$this->intnonull($_POST["miner1"])."',`miner2` ='".$this->intnonull($_POST["miner2"])."',`price` = '".$this->intnonull($_POST["price"])."',`commission` = '".$this->intnonull($_POST["commission"])."',`ucommission` = '".$this->intnonull($_POST["ucommission"])."',`compcent` = '".$_POST["compcent"]."',`ucompcent` = '".$_POST["ucompcent"]."'，`memo` = '".$_POST["memo"]."' WHERE order_id =".$id;
		echo $_POST['issave'];
		if($_POST['issave']=='1'){
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."order` SET `status`=1  WHERE order_id =".$id);//0未提交，1已提交2反冲
		echo "UPDATE `".WEB_ADMIN_TABPOX."order` SET `status`=1  WHERE order_id =".$id;
		}
		echo $_POST["con_act"];
