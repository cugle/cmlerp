...
function audit(){	
		...
		$res=$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."order` SET `status`=1  WHERE order_id =".$_GET['id']);
		...
		exit("<script>alert('发生错误，审核失败，数据已经回滚。');window.location.href='order.php';</script>");	
		}	
...
//设置供应商
		...
		// 修改消耗品
		   ...
		   $t -> set_var('caction','upd');	
			...
			$t -> set_var('corderprice',$inrs2['orderprice']);
...
$discount=$_POST['cdiscount']/10;
if($_POST['cproduce_id']!=''&&$_POST['cnumber']!=''){			
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."orderdetail` (order_id,produce_id,number,orderprice,price,discount,totalacount,memo,warehouse_id,agencyid) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cprice"]*$discount."','".$_POST["cprice"]."','".$discount."','".$discount*$_POST["cprice"]*$_POST["cnumber"]."','".$_POST["cmemo"]."','".$_POST["warehouse_id"]."','".$_SESSION["currentorgan"]."')");	
		...
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."orderdetail` SET `produce_id` = '".$_POST["cproduce_id"]."',`number` = '".$_POST["cnumber"]."',`memo` = '".$_POST["cmemo"]."',totalacount='".$discount*$_POST["cprice"]*$_POST["cnumber"]."',warehouse_id='".$_POST["warehouse_id"]."' WHERE orderdetail_id  =".$_POST['proupdid']);		
		...
		echo $_POST['issave'];
		if($_POST['issave']=='1'){
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."order` SET `status`=0  WHERE order_id =".$id);//0未提交，1已提交2已收部分3全部收完
			exit("<script>alert('保存成功');window.location.href='order.php';</script>");
		}	
		}
//$this -> quit($info.'成功！');
