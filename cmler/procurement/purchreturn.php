...
$this->curObj=new currentaccount();
	$updid=$_POST['updid'];
	$res= $this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `status`=4  WHERE purchreturn_id =".$updid);//4表示被反冲3反冲2正常1未提交	
	//更新库存
	$purchreturn= $this -> dbObj -> GetRow("select * from `".WEB_ADMIN_TABPOX."purchreturn`   WHERE purchreturn_id =".$updid);

 
		$this->stockObj=new stock();
		$this->stockObj->purchrerurntostock($updid,$purchreturn["warehouse_id"],$_SESSION["currentorgan"]);
	//生产凭证
 	$condition =' purchreturn_id='.$updid;
	$sql='select * from '.WEB_ADMIN_TABPOX.'purchreturn  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;	
		...
		$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  purchreturn_id DESC  LIMIT 0 ,1");
			...
			$purchreturn_no=$inrrs['purchreturn_no'];
		 //查找供应商对应的会计科目：应付账款
		  ...
		  if($yifuaccounttitle_id==''){
			...
			//插入主记录
			$res1=$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."transfervoucher` (`transfervoucher_no`,`date`,`agencyid`,`creattime`,`man`,abstract,fromtype,frombillid) VALUES ('" .$transfervoucher_no."','".date('Y-m-d',time())."','".$_SESSION["currentorgan"]."','".date('Y-m-d H:i:s',time())."', '".$man."','采购退货单; ".$inrrs['memo']."',5,".$updid.")");
 //echo "INSERT INTO `".WEB_ADMIN_TABPOX."transfervoucher` (`transfervoucher_no`,`date`,`agencyid`,`creattime`,`man`) VALUES ('" .$transfervoucher_no."','".date('Y-m-d',time())."','".$_SESSION["currentorgan"]."','".date('Y-m-d H:i:s',time())."', '".$man."')";
		...
		$sqlstr='select * from '.WEB_ADMIN_TABPOX.'purchreturndetail  where  agencyid ='.$_SESSION["currentorgan"].' and purchreturn_id='.$updid;	
		...
		//插入转账明细
			...
			$res2=$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."transfervoucherdetail` ( `transfervoucher_id`, `accounttitle_id`, `objecttype_id`, `objectid`, `lend`, `loan`, `memo`, `agencyid`) VALUES ('".$id."','52','4','".$inrrs1['produce_id']."','0','".$inrrs1['totalacount']."','".$inrrs1['memo']."','".$_SESSION['currentorgan']."')");	
			$res2=$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."transfervoucherdetail` ( `transfervoucher_id`, `accounttitle_id`, `objecttype_id`, `objectid`, `lend`, `loan`, `memo`, `agencyid`) VALUES ('".$id."','".$yifuaccounttitle_id."','2','".$suppliers_id."','".$inrrs1['totalacount']."','0','".$inrrs1['memo']."','".$_SESSION['currentorgan']."')");	
			
		 $res3=$this->prodaybooksObj->main($inrrs1['produce_id'],$inrrs1["warehouse_id"],-$inrrs1['number'],-$inrrs1['totalacount'],5,$updid,$man,"采购退货; ".$inrrs['memo'],$_SESSION["currentorgan"]);//商品流水账	
			}
 ...
 //插入对账记录
 		...
 		$curaccount=$this ->curObj->addrecord(5,$updid,2,$_SESSION['currentorgan']);
		...
		$this -> dbObj -> Execute("ROLLBACK");
		 exit("<script>alert('发生错误，提交失败，数据已经回滚。');window.location.href='purchreturn.php';</script>");
		}
...
function  editorder(){
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  order_id DESC  LIMIT 0 ,1");
			...
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `purchreturn_no`='" .$_GET["purchreturn_no"]."',`order_id`='".$_GET['orderid']."',`warehouse_id`='".$inrrs["warehouse_id"]."',`suppliers_id`='" .$inrrs["suppliers_id"]."',`employee_id`='".$inrrs["employee_id"]."',`acount`='" .$inrrs["acount"]."',`creattime`='".date('Y-m-d',time())."',`memo`= '".$inrrs["memo"]."',`agencyid`='".$_SESSION["currentorgan"]."' WHERE purchreturn_id=".$updid);
		 	//echo "UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `purchreturn_no`='" .$_GET["purchreturn_no"]."',`order_id`='".$_GET['orderid']."',`warehouse_id`='".$inrrs["warehouse_id"]."',`suppliers_id`='" .$inrrs["suppliers_id"]."',`employee_id`='".$inrrs["employee_id"]."',`acount`='" .$inrrs["acount"]."',`creattime`='".date('Y-m-d',time())."',`memo`= '".$inrrs["memo"]."',`agencyid`='".$_SESSION["currentorgan"]."' WHERE purchreturn_id=".$updid;
			//$id = $this -> dbObj -> Insert_ID();
			//echo $id;
		$sqldel='delete from '.WEB_ADMIN_TABPOX.'purchreturndetail where agencyid='.$_SESSION["currentorgan"].' and purchreturn_id='.$updid;
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturnprice,totalacount,warehouse_id,agencyid) VALUES ('".$updid."','".$inrrs1["produce_id"]."','".$inrrs1["number"]."','".$inrrs1["orderprice"]."','".$inrrs1["totalacount"]."','".$inrrs1["warehouse_id"]."','".$_SESSION["currentorgan"]."')");
					//echo "INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturnprice,totalacount,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".$inrrs1["number"]."','".$inrrs1["orderprice"]."','".$inrrs1["totalacount"]."','".$_SESSION["currentorgan"]."')";
					
					}
			
				}
	   exit("<script>window.location.href='purchreturn.php?action=upd&updid=".$updid."';</script>");
}	
function  addorder(){
	$condition =' order_id='.$_GET['orderid'];
	$sql='select * from '.WEB_ADMIN_TABPOX.'order  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;	
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  order_id DESC  LIMIT 0 ,1");
			...
			//插入进货记录
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."purchreturn` (`purchreturn_no`,`order_id`,`warehouse_id`,`suppliers_id`,`employee_id`,`acount`,`creattime`,`memo`,`purchreturn_time`,`man`,`agencyid`) VALUES ('" .$_GET["purchreturn_no"]."','".$_GET['orderid']."', '".$inrrs["warehouse_id"]."', '" .$inrrs["suppliers_id"]."','".$inrrs["employee_id"]."', '" .$inrrs["acount"]."','".date('Y-m-d',time())."', '".$inrrs["memo"]."', '".date('Y-m-d',time())."','".$man."','".$_SESSION["currentorgan"]."')");
			//echo "INSERT INTO `".WEB_ADMIN_TABPOX."purchreturn` (`purchreturn_no`,`order_id`,`warehouse_id`,`suppliers_id`,`employee_id`,`acount`,`creattime`,`memo`,`agencyid`) VALUES ('" .$_GET["purchreturn_no"]."','".$_GET['orderid']."', '".$inrrs["warehouse_id"]."', '" .$inrrs["suppliers_id"]."','".$inrrs["employee_id"]."', '" .$inrrs["acount"]."','".date('Y-m-d',time())."', '".$inrrs["memo"]."','".$_SESSION["currentorgan"]."')";
			$id = $this -> dbObj -> Insert_ID();
			echo $id;
		$sqlstr='select * from '.WEB_ADMIN_TABPOX.'orderdetail  where  agencyid ='.$_SESSION["currentorgan"].' and order_id='.$_GET['orderid'];	
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturnprice,totalacount,memo,warehouse_id,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".$inrrs1["number"]."','".$inrrs1["orderprice"]."','".$inrrs1["totalacount"]."','".$inrrs1["memo"]."','".$inrrs1["warehouse_id"]."','".$_SESSION["currentorgan"]."')");
				//	echo "INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturnprice,totalacount,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".$inrrs1["number"]."','".$inrrs1["orderprice"]."','".$inrrs1["totalacount"]."','".$_SESSION["currentorgan"]."')";
					
					}
			
				}
	   exit("<script>window.location.href='purchreturn.php?action=upd&updid=".$id."';</script>");
}
	function disp(){
		//定义模板
		$t = new Template('../template/procurement');
		$t -> set_file('f','purchreturn.html');
		$t->unknowns = "remove";
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			$sql='select * from '.WEB_ADMIN_TABPOX.'purchreturn WHERE  agencyid ='.$_SESSION["currentorgan"].' and status in (0,1) ';
			 
			}

			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  purchreturn_id DESC  LIMIT ".$offset." , ".$psize);
	...
	function goDispAppend(){
		...
		$t = new Template('../template/procurement');
		$t -> set_file('f','purchreturn_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
		...
		$t -> set_var('acount','0');
		$t -> set_var('cdiscount',$purchdiscount=$this -> getValue('purchdiscount'));
		$t -> set_var('cpurchreturnprice','');
		...
		$this->getSupper()?$t -> set_var('cansupper',''):$t -> set_var('cansupper','none');	
		$t -> set_var('userid',$this->getUid());		
			...
			if($data['status']<1){$t -> set_var('auditdisabled','disabled');}
		...
		$t -> set_var('ctotalacount',"");
		$this->getSupper()?$t -> set_var('cansupper',''):$t -> set_var('cansupper','none');	
		$acount = &$this -> dbObj -> GetOne('select count(*) as acount from '.WEB_ADMIN_TABPOX.'purchreturndetail  where purchreturn_id  ='.$updid);
...
//设置供应商
		...
		// 修改消耗品
		    ...
		    if($_GET['cpurchreturndetail_id']!=''){
			//$t -> set_var('cs');
		   $t -> set_var('caction','upd');	
            
			$inrs2 = &$this -> dbObj -> GetRow('select * from '.WEB_ADMIN_TABPOX.'purchreturndetail  where purchreturndetail_id ='.$_GET['cpurchreturndetail_id']);
			//$t -> set_var($inrs);
			
		    $t -> set_var('cupdid',$inrs2['produce_id']);	
			$t -> set_var('proupdid',$_GET['cpurchreturndetail_id']);	
			...
			$t -> set_var('cpurchreturnprice',$inrs2['purchreturnprice']);
			$t -> set_var('cdiscount',$inrs2['purchase_discount']);
			$t -> set_var('ccounttotal',$inrs2['totalacount']);
...
if($_POST["con_act"]=='add'&&$_POST['cproduce_id']!=''&&$_POST['cnumber']!=''){			
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturnprice,purchase_discount,price ,totalacount,memo,warehouse_id,agencyid) VALUES ('".$id."','".$_POST["cproduce_id"]."', '".$_POST["cnumber"]."','".$_POST["cpurchreturnprice"]."','".$_POST["cdiscount"]."', '".$_POST["cprice"]."','".$_POST["ctotalacount"]."','".$_POST["cmemo"]."','".$_POST["warehouse_id"]."','".$_SESSION["currentorgan"]."')");	
           // echo "INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturnprice ,totalacount,memo,warehouse_id,agencyid) VALUES ('".$id."','".$_POST["cproduce_id"]."', '".$_POST["cnumber"]."', '".$_POST["cprice"]."','".$_POST["ctotalacount"]."','".$_POST["cmemo"]."','".$_POST["warehouse_id"]."','".$_SESSION["currentorgan"]."')";
			}
		...
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `acount` =".$acount." where purchreturn_id=".$id) ;	
			...
			$id = $_POST[MODIFY.'id'];
			$man=$this -> dbObj -> getone('select username from '.WEB_ADMIN_TABPOX.'user where userid = '.$this->getUid());
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `purchreturn_no` = '".$_POST["purchreturn_no"]."',`purchreturn_time` = '".$_POST["purchreturn_time"]."',`memo` = '".$_POST["memo"]."',`warehouse_id` = '".$_POST["warehouse_id"]."',`suppliers_id` = '".$_POST["suppliers_id"]."',`employee_id` = '".$_POST["employee_id"]."',`man`='".$man."'  WHERE purchreturn_id =".$id);
			

		//echo $_POST["con_act"];
 if($_POST["con_act"]=='upd'){
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturndetail` SET `produce_id` = '".$_POST["cproduce_id"]."',`number` = '".$_POST["cnumber"]."',`purchase_discount` = '".$_POST["cdiscount"]."',`purchreturnprice` = '".$_POST["cpurchreturnprice"]."',`price` = '".$_POST["cprice"]."',`memo` = '".$_POST["cmemo"]."',totalacount='".$_POST["ctotalacount"]."',warehouse_id='".$_POST["warehouse_id"]."'  WHERE purchreturndetail_id  =".$_POST['proupdid']);		
		// echo "UPDATE `".WEB_ADMIN_TABPOX."purchreturndetail` SET `produce_id` = '".$_POST["cproduce_id"]."',`number` = '".$_POST["cnumber"]."',`purchase_discount` = '".$_POST["cdiscount"]."',`purchreturnprice` = '".$_POST["cpurchreturnprice"]."',`price` = '".$_POST["cprice"]."',`memo` = '".$_POST["cmemo"]."',totalacount='".$_POST["ctotalacount"]."',warehouse_id='".$_POST["warehouse_id"]."'  WHERE purchreturndetail_id  =".$_POST['proupdid'];
		$acount = &$this -> dbObj -> GetOne('select sum(totalacount) from '.WEB_ADMIN_TABPOX.'purchreturndetail    where purchreturn_id  ='.$id);
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `acount` =".$acount." where purchreturn_id=".$id) ;
		...
		$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturnprice,purchase_discount,price,totalacount,warehouse_id,agencyid) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cpurchreturnprice"]."','".$_POST["cdiscount"]."','".$_POST["cprice"]."','".$_POST["ctotalacount"]."','".$_POST["warehouse_id"]."','".$_SESSION["currentorgan"]."')");	
		...
		//echo "INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturnprice,totalacount,warehouse_id,agencyid) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cprice"]."','".$_POST["ctotalacount"]."','".$_POST["warehouse_id"]."','".$_SESSION["currentorgan"]."')";
		...
		/*//更新库存
		$this->stockObj=new stock();
		$this->stockObj->purchrerurntostock($id,$_POST["warehouse_id"],$_SESSION["currentorgan"]);*/
		//$this->stockObj->main($_POST["cproduce_id"],$_POST["cnumber"],$_POST["warehouse_id"],$_SESSION["currentorgan"]);
		exit("<script>alert('保存成功');window.location.href='purchreturn.php';</script>");	
