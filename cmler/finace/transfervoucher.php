...
function audit(){	
		...
		while ($inrrs = &$inrs -> FetchRow()) {
			...
			$loan=$inrrs['loan'];
			$memo=$inrrs['memo'];
			$agencyid=$inrrs['agencyid'];
			$transfervoucher_id=$inrrs['transfervoucher_id'];
			$res2=$this->trhObj->main($accounttitle_id,$objecttype_id,$objectid,$lend,$loan,$memo,$date,$agencyid,$transfervoucher_id);
		}

		if($res){
		
		$this -> dbObj -> Execute("COMMIT");
		 exit("<script>alert('提交成功');history.go(-2);</script>");	
		}else{
...
//分页		
			...
			$sql='select * from '.WEB_ADMIN_TABPOX.'transfervoucher  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;
				...
				if($ftable=='sell'){$ftable1='frombillid';}
			$sql='select * from '.WEB_ADMIN_TABPOX.'transfervoucher r INNER JOIN '.WEB_ADMIN_TABPOX.$ftable." f on f.".$ftable."_id  =r.".$ftable1."   where f.".$category." like '%".$keywords."%' and  r.fromtype=1 and  r.agencyid =".$_SESSION["currentorgan"] ;
	...
	function goDispAppend(){
		...
		$t = new Template('../template/finace');
		$t -> set_file('f','transfervoucherbatch_detail.html');
		$t->unknowns = "keep";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
		...
		//设置内列表
			...
			$t -> set_var('ml');
			$batchnumber=$_GET['batchnumber']==''?15:$_GET['batchnumber'];
			$t -> set_var('batchnumber',$batchnumber);	
			
			if($_GET['batchnumber']==''){
				$t -> set_var('trdisplay','none');
				}else{
				$t -> set_var('addbatchnumberdisabled',"disabled");	
				}
	     	for ($i=1;$i<=$batchnumber;$i++) {
				...
				$t -> set_var('i',$i);
				$t -> set_var('aclist','aclist'.$i);
				$t -> set_var('accounttitle_id','accounttitle_id'.$i);
				$t -> set_var('objecttype_id','objecttype_id'.$i);
				$t -> set_var('object_name','object_name'.$i);
				$t -> set_var('object_id','object_id'.$i);
				
				$t -> set_var('lend','lend'.$i);
				$t -> set_var('loan','loan'.$i);
				$t -> set_var('memo','memo'.$i);
				$t -> parse('ml','mainlist',true);
			}
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."transfervoucher` (`transfervoucher_no`,`date`,`agencyid`,`creattime`,`man`) VALUES ('" .$_POST["transfervoucher_no"]."','".$_POST["date"]."','".$_SESSION["currentorgan"]."','".$_POST["creattime"]."', '".$_POST["man"]."')");
 			//echo "INSERT INTO `".WEB_ADMIN_TABPOX."transfervoucher` (`transfervoucher_no`,`date`,`agencyid`,`creattime`,`man`) VALUES ('" .$_POST["transfervoucher_no"]."','".$_POST["date"]."','".$_SESSION["currentorgan"]."','".$_POST["creattime"]."', '".$_POST["man"]."')";
...
if($_POST['subtype']=='3'){	
		$rownumber=$_POST['rownumber'];
 		for($i=1;$i<=$rownumber;$i++){
			...
			$accounttitle_id='accounttitle_id'.$i;
			$objecttype_id='objecttype_id'.$i;
			...
			$loan=$loan==''?0:$loan;
			$memo='memo'.$i;
			if($_POST[$accounttitle_id]<>''){
			$res=$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."transfervoucherdetail` (`transfervoucher_id`, `accounttitle_id`, `objecttype_id`, `objectid`, `lend`, `loan`, `memo`, `agencyid`) VALUES ('".$id."','".$_POST[$accounttitle_id]."','".$_POST[$objecttype_id]."','".$_POST[$object_id]."','".$_POST[$lend]."','".$_POST[$loan]."','".$_POST[$memo]."','".$_SESSION["currentorgan"]."')");
			}
			 // echo "INSERT INTO `".WEB_ADMIN_TABPOX."transfervoucherdetail` (`transfervoucher_id`, `accounttitle_id`, `objecttype_id`, `objectid`, `lend`, `loan`, `memo`, `agencyid`) VALUES ('".$id."','".$_POST[$accounttitle_id]."','".$_POST[$objecttype_id]."','".$_POST[$object_id]."','".$_POST[$lend]."','".$_POST[$loan]."','".$_POST[$memo]."','".$_SESSION["currentorgan"]."')";
			//if($res){$j=$res&&$res;}
 		}
		}			
