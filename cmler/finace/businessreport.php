...
function recoil(){
	...
	function analysis(){
		...
		if($ftable==''){$condition=$category.' like "'.$keywords.'"';}else{$condition=$category.' like "'.$keywords.'"';}
			...
			$sql='select * from  `'.WEB_ADMIN_TABPOX.'sell` A   INNER JOIN ( select * from '.WEB_ADMIN_TABPOX.'sellconsumedetail  union SELECT * FROM '.WEB_ADMIN_TABPOX.'sellservicesdetail ) B  on A.sell_id=B.sell_id  where  A.agencyid ='.$_SESSION["currentorgan"].$hiddenredstr1; 
			...
			$no=1;
			$totcalxianjinvalue=0;
			$totcalyinkavalue=0;
	     	while ($inrrs = &$inrs -> FetchRow()) {
				$t -> set_var($inrrs);
				 $t -> set_var('no',$no);
				 $no=$no+1;
				 $shoukuan=$inrrs['xianjinvalue']+$inrrs['yinkavalue'];
				 $t -> set_var('shoukuan',$shoukuan);
				...
				$beauty_str=$beauty_str." ".$beauty_name;

				}
				$t -> set_var('beauty_name',$beauty_str);
				
				
				$totcalxianjinvalue=$totcalxianjinvalue+ $inrrs['xianjinvalue'];
				$totcalyinkavalue=$totcalyinkavalue+ $inrrs['yinkavalue'];
		...
		$t -> set_var('totcalyinkavalue',$totcalyinkavalue);	
		$t -> set_var('totcalxianjinvalue',$totcalxianjinvalue);	
