...
//===============================================begin今日项目======================================================================	
	...
	$inrstoday=$this -> dbObj -> Execute($todaysql); 
		...
		$customerconsume=$this -> dbObj -> GetOne("select sum(A.number) as customerconsume FROM  (".$sqlstrt3." union ".$sqlstrt4.") A WHERE A.sell_id=".$inrrstoday['sell_id']." and A.discount>0");
		...
		$customerproduceamount=$this -> dbObj -> GetOne("select sum(amount) from  ".WEB_ADMIN_TABPOX."selldetail where  sell_id=".$inrrstoday['sell_id']." and discount>0 and item_type=1");
		...
		//定金
		$inrsdingjintoday=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE A.sell_id =".$inrrstoday['sell_id']."  AND status in(1,4,5) and A.item_type=5 and A.item_id=2  and A.agencyid =".$_SESSION['currentorgan']); 
		...
		//预收款
		$inrsyufutoday=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE A.sell_id =".$inrrstoday['sell_id']."  AND status in(1,4,5) and A.item_type=5 and A.item_id=0 and A.agencyid =".$_SESSION['currentorgan']); 
		...
		$totalyufu=$totalyufu+$inrsyufutoday;
				//预收款
		$inrsyufuptoday=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE A.sell_id =".$inrrstoday['sell_id']."  AND status in(1,4,5) and A.item_type=5 and A.item_id=6 and A.agencyid =".$_SESSION['currentorgan']); 
...
//=======================================================		
		...
		//本月定金
		$totaladdupdingjin=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE   B.creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5) and A.item_type=5 and A.item_id=2  and A.agencyid =".$_SESSION['currentorgan']); 
		 $totaladdupdingjin1= $this -> dbObj -> GetOne("select sum(dingjinvalue) as dingjin FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5)  and  agencyid =".$_SESSION['currentorgan']);
		 //echo "select sum(dingjinvalue) as dingjin FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5)  and  agencyid =".$_SESSION['currentorgan'];
		// echo "select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE   B.creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5) and A.item_type=5 and A.item_id=2  and A.agencyid =".$_SESSION['currentorgan'];
		 $totaladdupdingjin=$totaladdupdingjin-$totaladdupdingjin1;
		$t -> set_var('totaladdupdingjin',$totaladdupdingjin);		
		//本月预收款
		$totaladdupyufu=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE  B.creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5) and A.item_type=5 and A.item_id=0 and A.agencyid =".$_SESSION['currentorgan']); 
		  $totaladdupyufu1= $this -> dbObj -> GetOne("select sum(yufuvalue) as yufu FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5) and agencyid =".$_SESSION['currentorgan']);
		 $totaladdupyufu=$totaladdupyufu-$totaladdupyufu1;
		//$t -> set_var('totaladdupyufu',$totaladdupyufu);
		//本月预收产品款
		$totaladdupyufup=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE  B.creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5) and A.item_type=5 and A.item_id=6 and A.agencyid =".$_SESSION['currentorgan']); 
		 $totaladdupyufup1= $this -> dbObj -> GetOne("select sum(yufuproducevalue) as yufup FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5) ");
		 $totaladdupyufup=$totaladdupyufup-$totaladdupyufup1;
		//$t -> set_var('totaladdupyufup',$totaladdupyufup);		 
		//产品累计
		$totaladdupbeautyproduceamount=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt1.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE  B.creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5) and A.item_type=1  and A.agencyid =".$_SESSION['currentorgan']); 
...
$membercard=$this -> dbObj -> Execute("select * from  ".WEB_ADMIN_TABPOX."marketingcard  where  marketingcardtype_id   in (5)  and  agencyid =".$_SESSION['currentorgan'].' order by  marketingcard_id desc');//会籍卡
	...
	$inrs6=$this->dbObj->Execute("select *,A.employee_id AS consultant_id from (".$sql6.") A INNER JOIN ".WEB_ADMIN_TABPOX."sell B ON A.sell_id= B.sell_id where  B.creattime between '".$bgdate."' and '".$enddate."' and A.agencyid =".$_SESSION['currentorgan']);
...
while ($inrrscard = $inrscard -> FetchRow()) {	 
	  ...
	  if($inrrscard['type']=='1'){
		 ...
		 $cardnumber1=$this -> dbObj -> GetRow("select count(*) as cardnumber FROM  ".WEB_ADMIN_TABPOX."sellcarddetail A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id left join  ".WEB_ADMIN_TABPOX."marketingcard C ON A.item_id=C.marketingcard_id  WHERE  B.status in(1,4,5) and A.item_type =3  and A.employee_id=".$employeedata['employee_id']." and B.creattime between '".$bgdate."' and '".$enddate."' and C.inheritedid in(".$inrrscard['cardidlist'].") and A.agencyid =".$_SESSION['currentorgan']);//这里假设了开卡只可以选一个顾问
...
/*	$sqlstr2="select * from  ".WEB_ADMIN_TABPOX."sellcarddetail ";
	...
	$sqlstr5="select * from  ".WEB_ADMIN_TABPOX."sellotherdetail ";*/
	$sql=$sqlstr1;
 	$inrs=$this -> dbObj -> Execute("select A.* FROM  (".$sql.") A INNER JOIN  ".WEB_ADMIN_TABPOX."sell B  ON A.sell_id=B.sell_id WHERE  B.creattime between '".$bgdate."' and '".$enddate."' AND A.item_type=1  AND B.status in(1,4,5)  and   A.agencyid =".$_SESSION['currentorgan']); 	
	// echo "select * FROM  (".$sql.") A INNER JOIN  ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE  B.creattime between '".$bgdate."' and '".$enddate."' AND A.item_type=1 AND B.status in(1,4,5)   and    A.agencyid =".$_SESSION['currentorgan'];
	while ($inrrs = $inrs -> FetchRow()) {
    //=====顾问=============================		 
