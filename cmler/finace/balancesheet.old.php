...
//资产							
				...
				$zcbegining=$zcbegining?$zcbegining:0;
				$t -> set_var('zcbegining',$zcbegining);						
				...
				$t -> set_var('zcend',$zcend['zcend']-2*$reducezcend['reducezcend']);	
				...
				$t -> set_var('fzend',$fzend['fzend']-2*$reducefzend['reducefzend']);	
...
//资产							
				...
				if($fzdata[$i]['parentid']<>0){
					...
					$space='';
					$t -> set_var('fzname','<strong>'.$fzdata[$i]['balancesheettype_name'].'</strong>');		
					}				
				...
				$t -> set_var('fzend','');
					...
					if($count==$i+1){
					$space='';
					$t -> set_var('fzname','<strong>'.$space.$fzdata[$count2-1]['balancesheettype_name'].'</strong>');	
				$fzbegining=$this -> dbObj ->GetOne('SELECT A.end FROM '.WEB_ADMIN_TABPOX.'balancesheet A INNER JOIN '.WEB_ADMIN_TABPOX.'balancesheettype B ON A.balancesheettype_id =B.balancesheettype_id  WHERE B.balancesheettype_id ='.$fzdata[$count2-1]['balancesheettype_id'].' and B.agencyid='.$_SESSION["currentorgan"].'    ORDER BY A.balancesheet_id DESC');
				$fzbegining=$fzbegining?$fzbegining:0;
				$t -> set_var('fzbegining',$fzbegining);				
				
				
				$fzend=$this -> dbObj ->GetRow('SELECT sum(A.loan-A.lend) as fzend FROM '.WEB_ADMIN_TABPOX.'transfervoucherdetail A INNER JOIN '.WEB_ADMIN_TABPOX.'transfervoucher B ON A.transfervoucher_id=B.transfervoucher_id WHERE A.accounttitle_id in ('.$fzdata[$count2-1]['accounttitleid'].') and B.agencyid='.$_SESSION["currentorgan"].' and '.$timecondition1);
				$reducefzend=$this -> dbObj ->GetRow('SELECT sum(A.lend-A.loan) as reducefzend FROM '.WEB_ADMIN_TABPOX.'transfervoucherdetail A INNER JOIN '.WEB_ADMIN_TABPOX.'transfervoucher B ON A.transfervoucher_id=B.transfervoucher_id WHERE A.accounttitle_id in ('.$fzaddorreduce[$count2-1]["addorreduce"].') and B.agencyid='.$_SESSION["currentorgan"].' and '.$timecondition1);
				$fzend['fzend']=$fzend['fzend']?$fzend['fzend']:0;
				 $reducefzend['reducefzend']=$reducefzend['reducefzend']?$reducefzend['reducefzend']:0;
				$t -> set_var('fzend',$fzend['fzend']-2*$reducefzend['reducefzend']);	
				$t -> set_var('fzno',$czlastno);
