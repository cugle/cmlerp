...
class Pagecustomer extends admin {
	...
	function disp(){
		...
		$t -> set_file('f','roomgroup.html');
		$t->unknowns = "remove";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		$t -> set_block('f','roomgroup','rg');		
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'%"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'roomgroup  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'roomgroup r INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on r.roomgroup_id  =f.roomgroup_id   where f.roomgroup_name like '%".$keywords."%' and  r.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'roomgroup  where  agencyid ='.$_SESSION["currentorgan"];
			 
			}
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  roomgroup_id DESC  LIMIT ".$offset." , ".$psize);
			...
			//$inrs = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'roomgroup  where agencyid ='.$_SESSION["currentorgan"]);
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				$t -> set_var($inrrs);
			   	$t -> set_var('delete',$this -> getDelStr('',$inrrs['roomgroup_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['roomgroup_id']));
				$t -> parse('rg','roomgroup',true);
			}
			$inrs -> Close();	
	...
	function goDispAppend(){
		...
		$t = new Template('../template/basic');
		$t -> set_file('f','roomgroup_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		...
		$t -> set_var('userid',$this->getUid());		
			...
			$t -> set_var($this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'roomgroup WHERE roomgroup_id = '.$updid));			
			$t -> set_var('error',"");
			$t -> set_var('showeditdiv',"");
			$t -> set_var('updid',$updid);
			$t -> set_var('action','upd');
			$t -> set_var('actionName','修改');
			$inrs = &$this -> dbObj -> Execute('SELECT * FROM '.WEB_ADMIN_TABPOX.'roomgroup WHERE roomgroup_id = '.$updid);	
			while ($inrrs = &$inrs -> FetchRow()) {
				$t -> set_var($inrrs);
			}
			$inrs -> Close();
	...
	function goDelete(){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'roomgroup WHERE roomgroup_id in('.$delid.')');
		//$this -> quit('删除成功！');
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '增加';	
			echo "INSERT INTO `".WEB_ADMIN_TABPOX."roomgroup` (`roomgroup_name` ,`memo` ,`agencyid` )VALUES ( '".$_POST["roomgroup_name"]."','".$_POST["memo"]."','".$_SESSION["currentorgan"]."')";
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."roomgroup` (`roomgroup_name` ,`memo` ,`agencyid` )VALUES ( '".$_POST["roomgroup_name"]."','".$_POST["memo"]."','".$_SESSION["currentorgan"]."')");
			...
			$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."roomgroup SET roomgroup_name='".$_POST["roomgroup_name"]."', memo='".$_POST["memo"]."' WHERE roomgroup_id =".$id);
...
//$this -> quit($info.'成功！');
	...
	function quit($info){
		exit("<script>alert('$info');location.href='roomgroup.php';</script>");
	}
