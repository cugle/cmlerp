...
class Pagecustomer extends admin {
	...
	function goDispAppend(){
		...
		$t -> set_var('man',$this -> dbObj -> getone('select username from '.WEB_ADMIN_TABPOX.'user where userid = '.$this->getUid()));
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			$content1= str_replace('{birthday}',date("Y-m-d",strtotime($booktime)),$content1);	
			 $result=file_get_contents("http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$content1."&destnumbers=".$handphone[$i]."&sendTime=".date("Y-m-d h:m:s",$sendtime)."&bookTime=".$booktime);
			//echo "http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$content1."&destnumbers=".$handphone[$i]."&sendTime=".date("Y-m-d h:m:s",$sendtime)."&bookTime=".$booktime;
...
//echo "UPDATE `".WEB_ADMIN_TABPOX."messagerecord` SET `customer_name` = '".$_POST["customer_name"]."',`customer_id` = '".$_POST["customer_id"]."',`title` = '".$_POST["title"]."',`content` = '".$content."',`senddate` ='".$_POST["senddate"]."',`handphone` = '".$_POST["handphone"]."',`employee_id` = '".$this->intnonull($_POST["employee_id"])."',`man` = '".$_POST["man"]."',`state` = '".$this->intnonull($_POST["state"])."', `messagetypeid`=".$_POST["messagetypeid"].", booktime='".$booktime."' WHERE messagerecord_id =".$id;
			...
			//发送短信
			 $result=file_get_contents("http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$content."&destnumbers=".$_POST["handphone"]."&sendTime=".date("Y-m-d h:m:s",time())."&bookTime=".date('Y-m-d h:m:s',strtotime($_GET['booktime'])));
			
			//echo "http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$content."&destnumbers=".$_POST["handphone"]."&sendTime=".date("Y-m-d h:m:s",time())."&bookTime=".$booktime;
           // echo $_POST['booktime'];
			//echo $booktime;
			$a=explode('&',$result);
            $b= explode('=',$a[0]);
