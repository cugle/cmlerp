...
class Pagecustomer extends admin {
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '赠送'.$_POST["zengsong"].'元';	
			//$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."membercard` SET `freecost` =freecost+ '".$_POST["zengsong"]."'  WHERE membercard_id =".$_POST['membercard_id']);		
			//echo "UPDATE `".WEB_ADMIN_TABPOX."membercard` SET `freecost` =freecost+ '".$_POST["zengsong"]."'  WHERE membercard_id =".$_POST['membercard_id'];
			 $this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."recharge` (`membercard_id`, `costtype`, `value`, `employee_id`, `agencyid`)VALUES ( '".$_POST["membercard_id"]."',3,'".$_POST["zengsong"]."', '".$_POST["employee_id"]."', '".$_SESSION["currentorgan"]."')");
            echo"INSERT INTO `".WEB_ADMIN_TABPOX."recharge` (`membercard_id`, `costtype`, `value`, `employee_id`, `agencyid`)VALUES ( '".$_POST["membercard_id"]."',3,'".$_POST["zengsong"]."', '".$_POST["employee_id"]."', '".$_SESSION["currentorgan"]."')";
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."membercard` (`membercard_no`, `cardlevel_id`, `customer_id`, `score`,`startdate`,`overdate`,` man`, ` agencyid`)VALUES ( '".$_POST["membercard_no"]."','".$_POST["cardlevel_id"]."', '".$_POST["customer_id"]."','".$this->intnonull($_POST["score"])."', '".$_POST["startdate"]."', '".$_POST["overdate"]."',  '".$_POST["man"]."', '".$_SESSION["currentorgan"]."')";
		 ...
		 $item_id=4;//定金$itemnamelist=array('预收款',"储值卡充值","定金","还款","赠送手工","其他","预付产品款","赠送产品款","赠送购卡款");
		 $number=1;
		 $value=$_POST["zengsong"];
		 $price=$_POST["zengsong"];
		 $discount=0;
		 $cardtype=0;//款项
		 $cardid=0;
