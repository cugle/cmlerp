...
class Pagecustomer extends admin {
	...
	function goDispAppend(){
		...
		$t -> set_var('deposit',$memberdata['deposit']);
		$t -> set_var('depositmemo','');
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '下定金'.$_POST["dingjin"].'元';	
			//$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."membercard` SET `deposit` =deposit+ '".$_POST["dingjin"]."',depositmemo='".$_POST["depositmemo"]."'  WHERE membercard_id =".$_POST['membercard_id']);		
...
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."membercard` (`membercard_no`, `cardlevel_id`, `customer_id`, `score`,`startdate`,`overdate`,` man`, ` agencyid`)VALUES ( '".$_POST["membercard_no"]."','".$_POST["cardlevel_id"]."', '".$_POST["customer_id"]."','".$this->intnonull($_POST["score"])."', '".$_POST["startdate"]."', '".$_POST["overdate"]."',  '".$_POST["man"]."', '".$_SESSION["currentorgan"]."')";
	...
	// echo 'SELECT * FROM '.WEB_ADMIN_TABPOX.'account WHERE `type`='.$acounttypeid.' and  agencyid='.$agencyid; 
		...
		//$memo=$repaymentmemo<>''?$repaymentmemo:'预付定金';
		 ...
		 $item_id=2;//定金$itemnamelist=array('预收款',"储值卡充值","定金","还款","赠送手工","其他","预付产品款","赠送产品款","赠送购卡款");
		 $number=1;
		 $value=$_POST["dingjin"];
		 $price=$_POST["dingjin"];
		 $discount=10;
...
//$this -> quit($info.'成功！');
		if(mysql_affected_rows())
		$this -> quit($info.'成功！定金账户余额为：'.($_POST["dingjin"]+$_POST["deposit"]));
	    else
		$this -> quit($info.'失败！');
