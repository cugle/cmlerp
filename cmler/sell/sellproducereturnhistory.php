	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  purchreturn_id DESC  LIMIT 0 ,1");
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."purchreturndetail` (purchreturn_id,produce_id,number,purchreturnprice,totalacount,memo,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".(-$inrrs1["number"])."','".$inrrs1["purchreturnprice"]."','".(-$inrrs1["totalacount"])."','".$inrrs1["memo"]."','".$inrrs1["agencyid"]."')");
	   ...
	   $this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."purchreturn` SET `status`=3  WHERE purchreturn_id =".$_GET['id']);//4表示被反冲3反冲2正常1未提交	
	  	//更新库存
