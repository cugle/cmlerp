...
//		  <{optname} {attrib}='{subusername}'>{subusername}</{optname}>
		...
		//是否有未读的新公告，有则显示公告提示框，无则隐藏。
		$news_id=$this -> po -> dbObj -> GetOne('select news_id from '.WEB_ADMIN_TABPOX.'news order by createtime desc ');
	
		$isread=$this -> po -> dbObj -> Getrow('select * from '.WEB_ADMIN_TABPOX.'usernews where news_id='.$news_id.' and  user_id='.$this -> po -> loginObj->getUid());
		
		if(!$isread&&$news_id){
			$t -> set_var('newsscript','<script language="javascript" src="{path1}js/news.js"></script>');
			$t -> set_var('doit()','doit();');
			}
		//资源路径
		$t -> set_var('path',WEB_ADMIN_HTTPPATH.'/template/mainStyle/style3/');
		$t -> set_var('path1',WEB_ADMIN_HTTPPATH.'/common/');
		$t -> parse('out','f');
		$t -> p('out');
	}
	function forAjax(){
