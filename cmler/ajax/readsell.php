...
if($column=='customer_id'){
	$inrs = $this -> dbObj -> Execute("select * from  ".WEB_ADMIN_TABPOX.$table."    where agencyid=".$agencyid." and ".$column."=".$value." order by sell_id desc");
	}else{
$inrs = $this -> dbObj -> Execute("select * from  ".WEB_ADMIN_TABPOX.$table."    where agencyid=".$agencyid." and (TO_DAYS(creattime)   =   TO_DAYS(NOW())  or  `status`  =0) order by sell_id desc");
