...
class Pagecustomer extends admin {
	function disp(){
		...
		$t -> set_file('f','getmessagerecord.html');
		$t->unknowns = "remove";
		...
		if($ftable==''){$condition=$category.'="'.$keywords.'"';}else{$condition=$category.'="'.$keywords.'"';}
			...
			$sql='select * from '.WEB_ADMIN_TABPOX.'messagerecord p INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on p.".$ftable."id =f.".$ftable."_id  where f.".$ftable."_name like '%".$keywords."%' and  p.agencyid =".$_SESSION["currentorgan"] ;
			echo $sql;
			}else{
			...
			//$inrs = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'produce  where agencyid ='.$_SESSION["currentorgan"]);
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				$t -> set_var('messagetypename',$this -> dbObj -> getone('select messagetype_name from '.WEB_ADMIN_TABPOX.'messagetype where messagetype_id ='.$inrrs["messagetypeid"]));
