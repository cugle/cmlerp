...
class Pageselldetail extends admin {
	function disp(){
		//定义模板
		$t = new Template('../template/getdata');
		$t -> set_file('f','getsellproduce.html');
		$t->unknowns = "keep";
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'%"';}else{$condition=$category.' like " %'.$keywords.'%"';}
			...
			$sql='select * from '.WEB_ADMIN_TABPOX.'selldetail p INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on p.categoryid =f.category_id  where f.category_name like '%".$keywords."%' and  p.agencyid =".$_SESSION["currentorgan"] ;
			...
			$t -> set_var('pagelist',$this -> page("?category=".$category."&keywords=".urlencode($keywords)."&ftable=".$ftable,$count,$psize,$pageid));		
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				$t -> set_var('code',$produce['code']);
				$t -> set_var('produce_name',$produce['produce_name']);
				$t -> set_var('produce_no',$produce['produce_no']);
				$t -> set_var('produce_id',$produce['produce_id']);
	...
	function quit($info){
		exit("<script>alert('$info');location.href='selldetail.php';</script>");
	}
