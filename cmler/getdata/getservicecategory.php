...
class Pageservicecategory extends admin {
	function disp(){
		//定义模板
		$t = new Template('../template/getdata');
		$t -> set_file('f','getservicecategory.html');
		$t->unknowns = "keep";
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'%"';}else{$condition=$category.' like " %'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'servicecategory  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'servicecategory p INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on p.categoryid =f.category_id  where f.category_name like '%".$keywords."%' and  p.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'servicecategory  where  agencyid ='.$_SESSION["currentorgan"];
	...
	function quit($info){
		exit("<script>alert('$info');location.href='servicecategory.php';</script>");
	}
