...
function recoil(){
	...
	function disp(){
		...
		$t -> set_file('f','accounthistory.html');
		$t->unknowns = "remove";
		...
		if($ftable==''){$condition=$category.' like "'.$keywords.'"';}else{$condition=$category.' like "'.$keywords.'"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'accounthistory  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql="select f.status as fstatus ,f.customer_id,s.* from ".WEB_ADMIN_TABPOX."accounthistory s INNER JOIN ".WEB_ADMIN_TABPOX."$ftable f on s.customer_id =f.customer_id  where f.".$category." like '%".$keywords."%' and f.agencyid =".$_SESSION["currentorgan"] ;
			
			 
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'accounthistory  where  agencyid ='.$_SESSION["currentorgan"];
			 
			}
			
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  accounthistory_id DESC,accounthistory_id DESC LIMIT ".$offset." , ".$psize);
			...
			$type_name=array("销售收入","销售收入","储值卡充值收入","预收款","进货款支出","退货返款");
			$t -> set_var('type_name',$type_name[$inrrs['type']]);
				$tyeptablelist=array("sell","sell","sell","sell","purchase","purchreturn");
 				$billdata=$this -> dbObj -> getrow('select * from '.WEB_ADMIN_TABPOX.$tyeptablelist[$inrrs["type"]].'  where '.$tyeptablelist[$inrrs["type"]].'_id ='.$inrrs["sellid"]);
 
				$billno=$billdata[$tyeptablelist[$inrrs["type"]].'_no'];
				
				
				
				$t -> set_var('bill_no',$billno);
 				if($inrrs['type']==4){
				$t -> set_var('view','<a href="#" onclick=viewpurchbill('.$inrrs['sellid'].');>查看单据</a>'); 				
				$t -> set_var('print','<a href="../procurement//purchasehistory.php?action=print&updid='.$inrrs['sellid'].'" target="_blank">打印</a>'); 
 
				}else if($inrrs['type']==5){
				$t -> set_var('view','<a href="#" onclick=viewpurchreturnbill('.$inrrs['sellid'].');>查看单据</a>'); 				
				$t -> set_var('print','<a href="../procurement/purchreturnhistory.php?action=print&updid='.$inrrs['sellid'].'" target="_blank">打印</a>'); 
 
				}else{
				$t -> set_var('view','<a href="#" onclick=viewbill('.$inrrs['sellid'].');>查看单据</a>'); 
				
				$t -> set_var('print','<a href="?action=print&updid='.$inrrs['sellid'].'" target="_blank">打印</a>');
				}
				$t -> set_var('account_name',$this -> dbObj -> getone('select account_name from '.WEB_ADMIN_TABPOX.'account  where account_id ='.$inrrs["account_id"]));
			...
			$t -> set_var('status_name',$status_name[$billdata['status']]);	
