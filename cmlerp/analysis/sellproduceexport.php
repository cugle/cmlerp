...
$sql="SELECT A.status as sellstatus,A.*,B.* , C.* FROM (s_sell A RIGHT JOIN s_selldetail  B ON a.sell_id=B.sell_id ) LEFT JOIN s_employee C ON C.employee_id=A.employee_id where A.agencyid = ".$_POST["agencyid"].' and '.$timecondition;
...
$serstr='SELECT * FROM s_produce WHERE produce_id='.$row['item_id'].' limit 1';
 
$serdata=mysql_query($serstr); 	
while($servicesdata =mysql_fetch_array($serdata)){
$services_name=$servicesdata['produce_name'];
$services_no=$servicesdata['produce_no'];
