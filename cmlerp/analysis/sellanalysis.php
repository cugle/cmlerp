...
function recoil(){
	...
	function disp(){
		...
		$t -> set_file('f','sellanalysis.html');
		$t->unknowns = "remove";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		$t -> set_var('bgdate',date('Y-m-d',strtotime("-1 month",time())));
	...
	function analysis(){
		//定义模板
		$t = new Template('../template/analysis');
		$t -> set_file('f','sellanalysis1.html');
		$t->unknowns = "remove";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		...
		if($ftable==''){$condition=$category.' like "'.$keywords.'"';}else{$condition=$category.' like "'.$keywords.'"';}
			...
			$sql="select f.status as fstatus ,f.customer_id,s.* from ".WEB_ADMIN_TABPOX."sell s INNER JOIN ".WEB_ADMIN_TABPOX."$ftable f on s.customer_id =f.customer_id  where f.".$category." like '%".$keywords."%' and f.agencyid =".$_SESSION["currentorgan"] ;
			
			 
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'sell  where  agencyid ='.$_SESSION["currentorgan"].' and '.$timecondition;
			 $sql1='select sum(realpay) as sum_realpay,sum(xianjinvalue) as sum_xianjinvalue,sum(yinkavalue) as sum_yinkavalue,sum(payable1) as sum_payable1   from '.WEB_ADMIN_TABPOX.'sell  where  agencyid ='.$_SESSION["currentorgan"].' and '.$timecondition.' and status =1'; 
			}
			 
			$totaldata=$this -> dbObj -> GetRow($sql1);
			$totalrealpay=$totaldata['sum_realpay'];
			$totalxianjinvalue=$totaldata['sum_xianjinvalue'];
			$totalyinkavalue=$totaldata['sum_yinkavalue'];
			$totalpayable1=$totaldata['sum_payable1'];
			$own=$totalpayable1-$totalrealpay;
			$bgdate=$_GET["bgdate"];
			$enddate=$_GET["enddate"];
			$info= "从".$_GET["bgdate"]."到".$_GET["enddate"]."的业绩为".$totalrealpay;
			$t -> set_var('info',$info);
			$t -> set_var('bgdate',$bgdate);
			$t -> set_var('enddate',$enddate);
			$t -> set_var('totalrealpay',$totalrealpay);
			$t -> set_var('totalxianjinvalue',$totalxianjinvalue);
			$t -> set_var('totalyinkavalue',$totalyinkavalue);
			$t -> set_var('totalrealpay',$totalrealpay);
			$t -> set_var('totalpayable1',$totalpayable1);
			$t -> set_var('own',$own);
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  sell_no DESC,sell_id DESC LIMIT ".$offset." , ".$psize);
			$result = &$this -> dbObj -> Execute($sql);		
			$count=$result->RecordCount();
			$t -> set_var('pagelist',$this -> page("?action=analysis&bgdate=".$bgdate."&enddate=".$enddate."&category=".$category."&keywords=".urlencode($keywords)."&ftable=".$ftable,$count,$psize,$pageid));			
