...
class Pagecustomer extends admin {
    ...
    {   
        ...
        { 
            ...
            $this -> checkUser();//验证身份，这一步很重要。
            $this -> recoil();			
		}else if(isset($_GET['action']) && $_GET['action']=='serviceconsumeexport'){
            $this -> checkUser();//验证身份，这一步很重要。
            $this -> serviceconsumeexport();			
		}else{
            parent::Main();
        }
    }
	function serviceconsumeexport(){
		//定义模板
		$t = new Template('../template/analysis');
		$t -> set_file('f','serviceconsumeexport.html');
		$t->unknowns = "remove";
...
function recoil(){
	...
	function disp(){
		//定义模板
		$t = new Template('../template/analysis');
		$t -> set_file('f','serviceconsume.html');
		$t->unknowns = "remove";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		...
		if($ftable==''){$condition=$category.' like "'.$keywords.'"';}else{$condition=$category.' like "'.$keywords.'"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'sellconsumedetail B ON A.sell_id=B.sell_id  where  A.agencyid ='.$_SESSION["currentorgan"].' and '.$condition.$hiddenredstr1 ;

			}else if($ftable<>''){

			$sql="select A.status as fstatus ,F.".$ftable."_id,A.*,B.* from ( ".WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'sellconsumedetail B ON A.sell_id=B.sell_id)  INNER JOIN '.WEB_ADMIN_TABPOX.$ftable.' F on A.'.$ftable.'_id =F.'.$ftable.'_id  where F.'.$category." like '".$keywords."' and A.agencyid =".$_SESSION["currentorgan"].$hiddenredstr1;
			

			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'sell A INNER JOIN '.WEB_ADMIN_TABPOX.'sellconsumedetail B ON A.sell_id=B.sell_id  where A.agencyid ='.$_SESSION["currentorgan"].$hiddenredstr1;
			...
			//$t -> set_var('status_name',$status_name[$inrrs['status']]);
				...
				$t -> set_var('creattime',$selldata['creattime']);
				$beauty_str='';
				...
				$t -> set_var('services_name',$this -> dbObj -> getone('select services_name  from '.WEB_ADMIN_TABPOX.'services  where services_id ='.$inrrs["item_id"]));
...
//设置供应商
	...
	function goDelete(){
		...
		//echo 'DELETE FROM '.WEB_ADMIN_TABPOX.'sellconsumedetail WHERE selldetail_id in('.$delid.')';
 		$this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'sellconsumedetail WHERE selldetail_id in('.$delid.')');
		$info='删除';
