...
class Pagemarketingcard extends admin {
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'%"';}else{$condition=$category.' like " %'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'marketingcard  where   marketingcardtype_id=5 and  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;
			...
			$sql='select * from '.WEB_ADMIN_TABPOX.'marketingcard  where marketingcardtype_id=5 and  agencyid ='.$_SESSION["currentorgan"];
