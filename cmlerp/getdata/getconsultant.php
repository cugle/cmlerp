...
class Pagecustomer extends admin {
	function disp(){
		...
		$condition1=$condition==''?$parameter[$i].' in ('.$keywords[$i].')':$condition." and ".$parameter[$i].'in ('.$keywords[$i].')';
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'employee A INNER JOIN '.WEB_ADMIN_TABPOX.'employeelevel B ON A.employeelevelid=B.employeelevel_id   where  A.status=1 and A.agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'employee  p INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on p.categoryid =f.category_id  where   p.status=1 and f.category_name like '%".$keywords."%' and  p.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'employee A INNER JOIN '.WEB_ADMIN_TABPOX.'employeelevel B ON A.employeelevelid=B.employeelevel_id   where   A.status=1 and A.agencyid ='.$_SESSION["currentorgan"]; 
			 
			}
			
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  employee_id DESC  LIMIT ".$offset." , ".$psize);
