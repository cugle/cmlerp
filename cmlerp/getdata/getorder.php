...
class Pageorder extends admin {
	function disp(){
		//定义模板
		$t = new Template('../template/getdata');
		$t -> set_file('f','getorder.html');
		$t->unknowns = "keep";
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'%"';}else{$condition=$category.' like " %'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'order  where  agencyid ='.$_SESSION["currentorgan"]." and  status between '1' and '2' and ".$condition ;

			}else if($ftable<>''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'order p INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on p.".$ftable."_id =f.".$ftable."_id  where f.".$category." like '%".$keywords."%' and  p.agencyid =".$_SESSION["currentorgan"]." and  p.status between '1' and '2'";
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'order  where  agencyid ='.$_SESSION["currentorgan"]." and  status between '1' and '2' ";
	...
	function quit($info){
		exit("<script>alert('$info');location.href='order.php';</script>");
	}
