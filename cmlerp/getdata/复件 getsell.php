...
class Pagesell extends admin {
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'%"';}else{$condition=$category.' like " %'.$keywords.'%"';}
			...
			$sql='select * from '.WEB_ADMIN_TABPOX.'sell p INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on p.categoryid =f.category_id  where f.category_name like '%".$keywords."%' and  p.agencyid =".$_SESSION["currentorgan"] ;
	...
	function quit($info){
		exit("<script>alert('$info');location.href='sell.php';</script>");
	}
