...
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."messagerecord` (`customer_id`, `customer_name`, `title`, `content`, `senddate`,  `handphone`, `employee_id`, `man`,  `state`, `agencyid`)VALUES ( '".$_POST["customer_id"]."','".$_POST["customer_name"]."', '".$_POST["title"]."','".$_POST["senddate"]."', '".$_POST["handphone"]."', '".$this->intnonull($_POST["employee_id"])."','".$_POST["man"]."', '".$this->intnonull($_POST["state"])."','".$_SESSION["currentorgan"]."')";
			...
			$result=file_get_contents("http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$_POST["content"]."&destnumbers=".$_POST["handphone"]."&sendTime=".date("Y-m-d H:i:s",time()));
			echo "http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$_POST["content"]."&destnumbers=".$_POST["handphone"]."&sendTime=".date("Y-m-d H:i:s",time());
			echo $result;
			$a=explode('&',$result);
            $b= explode('=',$a[0]);
			$state=$b[1];
...
echo "UPDATE `".WEB_ADMIN_TABPOX."messagerecord` SET `customer_name` = '".$_POST["customer_name"]."',`customer_id` = '".$_POST["customer_id"]."',`title` = '".$_POST["title"]."',`content` = '".$content."',`senddate` ='".$_POST["senddate"]."',`handphone` = '".$_POST["handphone"]."',`employee_id` = '".$this->intnonull($_POST["employee_id"])."',`man` = '".$_POST["man"]."',`state` = '".$this->intnonull($_POST["state"])."' WHERE messagerecord_id =".$id;
			...
			$result=file_get_contents("http://59.42.247.51/http1.php?act=send&orgid=".$orgid."&username=".$username."&passwd=".$passwd."&msg=".$content."&destnumbers=".$_POST["handphone"]."&sendTime=".date("Y-m-d H:i:s",time()));
