	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			//$sFile = file_get_contents("http://59.42.247.51/http1.php", false, $cxContext);   
			 //$content1=mb_convert_encoding($content1,'gbk','utf-8');        //这个就没有乱码
			//$msg=urlencode($content1);
...
$result= $sFile;   			
			...
			$sqlstr="( '".$customer_id[$i]."','".$customer_name[$i]."', '".$_POST["title"]."', '".$content1."','".$_POST["senddate"]."', '".$handphone[$i]."', '".$this->intnonull($_POST["employee_id"])."','".$_POST["man"]."', '".$this->intnonull($_POST["state"])."','".$_POST["messagetypeid"]."','".$booktime."','".$_SESSION["currentorgan"]."','".$_POST["cetype"]."')";
		
		  	$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."messagerecord` (`customer_id`, `customer_name`, `title`, `content`, `senddate`,  `handphone`, `employee_id`, `man`,  `state`, `messagetypeid`, `booktime`,`agencyid`,cetype)VALUES ".$sqlstr);
