...
/////////////////////////////////////////////////////////////
class db{
	function &getLink($debug=false,$newconn=false,$type=null,$host=null,$user=null,$pass=null,$dbname=null){
		$type = is_null($type) ? WEB_ADMIN_DBTYPE : $type;
		$db = &ADONewConnection( $type );
		$db -> debug = $debug;
		$db -> Connect(
			is_null($host)   ? WEB_ADMIN_DBHOST : $host,
			is_null($user)   ? WEB_ADMIN_DBUSER : $user,
			is_null($pass)   ? WEB_ADMIN_DBPASS : $pass,
			is_null($dbname) ? WEB_ADMIN_DBNAME : $dbname,
			$newconn // new connect
		);
		$db -> SetFetchMode(ADODB_FETCH_ASSOC);
		if ($type == 'mysql') $db->Execute('SET NAMES UTF8');
		if ($type == 'oracle'){
