...
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."itemcard` (`item_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["item_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')";
	...
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql="select *,C.customer_id AS  customer_id,S.status AS  status  from ".WEB_ADMIN_TABPOX."itemcard  S LEFT JOIN ".WEB_ADMIN_TABPOX."customer C on C.customer_id =S.customer_id LEFT JOIN ".WEB_ADMIN_TABPOX."membercard M on M.customer_id =C.customer_id  WHERE  S.status in (0,1,2,3) AND S.agencyid =".$_SESSION["currentorgan"].' and '.$condition.' GROUP BY itemcard_id';
			}else if($ftable<>''){
	...
	function goDispAppend(){
		...
		$t -> set_var('userid',$this->getUid());		
			...
			//echo 'SELECT * FROM '.WEB_ADMIN_TABPOX.'itemcard WHERE itemcard_id = '.$updid.' and agencyid ='.$_SESSION["currentorgan"];
