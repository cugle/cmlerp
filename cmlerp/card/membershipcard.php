...
function  addcard(){
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  marketingcard_id DESC  LIMIT 0 ,1");
			...
			//插入进货记录
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."membershipcard` (`membershipcard_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["membershipcard_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')");
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."membershipcard` (`item_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["item_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')";
			$id = $this -> dbObj -> Insert_ID();
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					...
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."membershipcarddetail` (membershipcard_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')");
					//echo "INSERT INTO `".WEB_ADMIN_TABPOX."membershipcarddetail` (membershipcard_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')";
					
					}
			
				}
	  exit("<script>window.location.href='membershipcard.php?action=upd&updid=".$id."';</script>");
}	
	function disp(){
	
		//定义模板
		$membershipcardtype_id= "1";
		$t = new Template('../template/card');
		$t -> set_file('f','membershipcard.html');
		$t->unknowns = "remove";
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql="select *,C.customer_id AS  customer_id,S.status AS  status from ".WEB_ADMIN_TABPOX."membershipcard  S LEFT JOIN ".WEB_ADMIN_TABPOX."customer C on C.customer_id =S.customer_id LEFT JOIN ".WEB_ADMIN_TABPOX."membercard M on M.customer_id =C.customer_id  WHERE  S.status in (0,1,2,3) AND S.agencyid =".$_SESSION["currentorgan"].' and '.$condition.' GROUP BY membershipcard_id';
			}else if($ftable<>''){
			...
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  `membershipcard_id` DESC  LIMIT ".$offset." , ".$psize);
			//echo $sql." ORDER BY  `membershipcard_id` DESC  LIMIT ".$offset." , ".$psize;
			...
			$status_name=array("未激活","使用中","挂失","停用","<font color=red>报废</font>");
	     	while ($inrrs = $inrs -> FetchRow()) {
				...
				$t -> set_var('statusname',$status_name[$inrrs['status']]);
			  	$t -> set_var('delete',$this -> getDelStr('',$inrrs['membershipcard_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['membershipcard_id']));		
	...
	function goDispAppend(){
		...
		$t = new Template('../template/card');
		$t -> set_file('f','membershipcard_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
		...
		$table=WEB_ADMIN_TABPOX.'membershipcard';
		$column='membershipcard_no';
		$number=5;
		$id='membershipcard_id';	
		$t -> set_var('membershipcard_no',$this->makeno($Prefix,$agency_no,$table,$column,$number,$id));				
		$t -> set_var('overdate',"");		
		$t -> set_var('membershipcard_name',"");	
		...
		$t -> set_var('userid',$this->getUid());		
			...
			$date=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'membershipcard WHERE membershipcard_id = '.$updid.' and agencyid ='.$_SESSION["currentorgan"]);
		...
		$t -> set_var('conupdid',"");	
		if($date['status']=='4'){
		$t -> set_var('deletedisabled',"");	
		}else{
			$t -> set_var('deletedisabled',"disabled");
		}
		...
		for ($i=0;$i<count($remaintimes);$i++)	{
			...
			$servicesdata=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'servicecategory  WHERE categoryid  in ('.$item_id[$i].') and agencyid ='.$_SESSION["currentorgan"]);		
			...
			$services_times=$this->dbObj->GetOne('SELECT services_times FROM '.WEB_ADMIN_TABPOX.'marketingcarddetail WHERE  servicecategory_id  = "'.$item_id[$i].'" and marketingcard_id='.$date["marketingcard_id"].' and agencyid ='.$_SESSION["currentorgan"]);
			//echo 'SELECT services_times FROM '.WEB_ADMIN_TABPOX.'marketingcarddetail WHERE  servicecategory_id  = "'.$item_id[$i].'" and marketingcard_id='.$date["marketingcard_id"].' and agencyid ='.$_SESSION["currentorgan"];
			
			$t -> set_var('services_no',$item_id[$i]);
			$t -> set_var('services_name',$type[$i]);
...
function statuslist($status_name,$arraselectid=0){
	...
	function goDelete(){
	  if($_GET['consumables_id']==''){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'membershipcard WHERE membershipcard_id in('.$delid.')');
		}else{
		//echo 'DELETE FROM '.WEB_ADMIN_TABPOX.'membershipcarddetail WHERE membershipcarddetail_id in('.$_GET['consumables_id'].')';
		 $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'membershipcarddetail WHERE membershipcarddetail_id in('.$_GET['consumables_id'].')');
		
		}
		$info='删除';
		if(mysql_affected_rows())
		
		if($_GET['qgtype']==''){exit("<script>alert('".$info."成功');window.location.href='membershipcard.php';</script>");}else{
		$this -> quit($info.'成功！');}
	    else
		$this -> quit($info.'失败！');
	}
	function goAppend(){
		...
		if($this -> isAppend){
			...
			exit("<script>alert('新增成功');location.href='membershipcard.php?action=upd&updid=".$id."';</script>");	
		}else{
			$info = '修改';
			...
			$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX.$cardtable.'  SET  remaintimeslist ="'.$_POST["remaintimeslist"].'" ,remaintimes="'.$_POST["remaintimes"].'" WHERE '.$cardtable.'_id in('.$id.')');
