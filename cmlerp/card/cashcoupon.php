...
function  addcard(){
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  marketingcard_id DESC  LIMIT 0 ,1");
			...
			//插入进货记录
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."cashcoupon` (`cashcoupon_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["cashcoupon_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')");
echo "INSERT INTO `".WEB_ADMIN_TABPOX."cashcoupon` (`item_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["item_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')";
			$id = $this -> dbObj -> Insert_ID();
			echo $id;
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					...
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."cashcoupondetail` (cashcoupon_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')");
					echo "INSERT INTO `".WEB_ADMIN_TABPOX."cashcoupondetail` (cashcoupon_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')";
					
					}
			
				}
	  exit("<script>window.location.href='cashcoupon.php?action=upd&updid=".$id."';</script>");
}	
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			$t -> set_var('ml');
						...
						if($condition<>''&&$ftable==''){
			$sql="select *,C.customer_id AS  customer_id,S.status AS  status  from ".WEB_ADMIN_TABPOX."cashcoupon  S LEFT JOIN ".WEB_ADMIN_TABPOX."customer C on C.customer_id =S.customer_id LEFT JOIN ".WEB_ADMIN_TABPOX."membercard M on M.customer_id =C.customer_id  WHERE  S.status in (0,1,2,3) AND S.agencyid =".$_SESSION["currentorgan"].' and '.$condition.' GROUP BY cashcoupon_id' ;
			}else if($ftable<>''){
			...
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  `cashcoupon_id` DESC  LIMIT ".$offset." , ".$psize);
			...
			$status_name=array("未激活","使用中","挂失","停用","<font color=red>已经使用</font>");
			$t -> set_var('status_name',$status_name[$inrrs['status']]);
						
			  	$t -> set_var('delete',$this -> getDelStr('',$inrrs['cashcoupon_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['cashcoupon_id']));		
	...
	function goDispAppend(){
		...
		$table=WEB_ADMIN_TABPOX.'cashcoupon';
		$column='cashcoupon_no';
		$number=5;
		$id='cashcoupon_id';	
		$t -> set_var('cashcoupon_no',$this->makeno($Prefix,$agency_no,$table,$column,$number,$id));				
		$t -> set_var('value',"");	
		$t -> set_var('overdate',"");	
		$t -> set_var('cashcoupon_name',"");	
		...
		$t -> set_var('userid',$this->getUid());		
			...
			$date=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'cashcoupon WHERE cashcoupon_id = '.$updid.' and agencyid ='.$_SESSION["currentorgan"]);
...
function statuslist($status_name,$arraselectid=0){
		...
		function selectlist($table,$id,$name,$selectid=0,$marketingcardtypeid=6){
	...
	function goDelete(){
	  if($_GET['consumables_id']==''){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'cashcoupon WHERE cashcoupon_id in('.$delid.')');
		}else{
		//echo 'DELETE FROM '.WEB_ADMIN_TABPOX.'cashcoupondetail WHERE cashcoupondetail_id in('.$_GET['consumables_id'].')';
		 $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'cashcoupondetail WHERE cashcoupondetail_id in('.$_GET['consumables_id'].')');
