...
function  addcard(){
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  marketingcard_id DESC  LIMIT 0 ,1");
			...
			//插入进货记录
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."itemcard` (`itemcard_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["itemcard_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')");
//echo "INSERT INTO `".WEB_ADMIN_TABPOX."itemcard` (`item_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["item_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')";
			$id = $this -> dbObj -> Insert_ID();
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					...
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."itemcarddetail` (itemcard_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')");
					//echo "INSERT INTO `".WEB_ADMIN_TABPOX."itemcarddetail` (itemcard_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')";
					
					}
			
				}
	  exit("<script>window.location.href='itemcard.php?action=upd&updid=".$id."';</script>");
}	
	function disp(){
		//定义模板
		$itemcardtype_id= "1";
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'itemcard  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql="select * from ".WEB_ADMIN_TABPOX."itemcard s INNER JOIN ".WEB_ADMIN_TABPOX."$ftable f on s.categoryid =f.category_id  where f.category_name like '%".$keywords."%' and s.itemcardtype_id ='.$itemcardtype_id.'  and  s.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'itemcard  where   agencyid ='.$_SESSION["currentorgan"];
			 
			}
			
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  `itemcard_id` DESC  LIMIT ".$offset." , ".$psize);
	    	...
	    	$t -> set_var('recordcount',$count);	
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				//$t -> set_var('category_name',$this -> dbObj -> getone('select category_name from '.WEB_ADMIN_TABPOX.'servicecategory  where category_id ='.$inrrs["categoryid"]));
			  	$t -> set_var('delete',$this -> getDelStr('',$inrrs['itemcard_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['itemcard_id']));		
	...
	function goDispAppend(){
		...
		$table=WEB_ADMIN_TABPOX.'itemcard';
		$column='itemcard_no';
		$number=5;
		$id='itemcard_id';	
		$t -> set_var('itemcard_no',$this->makeno($Prefix,$agency_no,$table,$column,$number,$id));				
		//$t -> set_var('itemcard_no',"");	
		$t -> set_var('itemcard_name',"");	
		...
		$t -> set_var('userid',$this->getUid());		
			...
			$date=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'itemcard WHERE itemcard_id = '.$updid.' and agencyid ='.$_SESSION["currentorgan"]);
...
function statuslist($status_name,$arraselectid=0){
	...
	function goDelete(){
	  if($_GET['consumables_id']==''){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'itemcard WHERE itemcard_id in('.$delid.')');
		}else{
		echo 'DELETE FROM '.WEB_ADMIN_TABPOX.'itemcarddetail WHERE itemcarddetail_id in('.$_GET['consumables_id'].')';
		 $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'itemcarddetail WHERE itemcarddetail_id in('.$_GET['consumables_id'].')');
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			$card_id=$id ;
			$cardtable='itemcard';
			$this-> CardObj->updatecard($card_id,$card_no,$marketingcard_id,$customerid,$employeeid,$agencyid,$status,$activedate,$buydate,$cardtable);
