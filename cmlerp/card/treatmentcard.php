...
function  addcard(){
	...
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  marketingcard_id DESC  LIMIT 0 ,1");
			...
			//插入进货记录
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."treatmentcard` (`treatmentcard_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["treatmentcard_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')");
echo "INSERT INTO `".WEB_ADMIN_TABPOX."treatmentcard` (`item_no`,`marketingcard_id`,`totaltimes`,`price`,`timelimit`,`buydate`,`activedate`,`employee_id`,`agencyid`) VALUES ('" .$_GET["item_no"]."','".$_GET['marketingcard_id']."', '".$inrrs["totaltimes"]."', '" .$inrrs["price"]."','".$inrrs["timelimit"]."', '".date('Y-m-d',time())."', '".date('Y-m-d',time())."','".$employee_id."','".$_SESSION["currentorgan"]."')";
			$id = $this -> dbObj -> Insert_ID();
		...
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					...
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."treatmentcarddetail` (treatmentcard_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')");
					//echo "INSERT INTO `".WEB_ADMIN_TABPOX."treatmentcarddetail` (treatmentcard_id,services_id ,remaintimes,totaltimes,agencyid) VALUES ('".$id."','".$inrrs1["services_id"]."','".$inrrs1["totaltimes"]."','".$inrrs1["totaltimes"]."','".$_SESSION["currentorgan"]."')";
					
					}
			
				}
	  exit("<script>window.location.href='treatmentcard.php?action=upd&updid=".$id."';</script>");
}	
	function disp(){
		//定义模板
		$treatmentcardtype_id= "1";
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql="select *,C.customer_id AS  customer_id,S.status AS  status   from ".WEB_ADMIN_TABPOX."treatmentcard  S LEFT JOIN ".WEB_ADMIN_TABPOX."customer C on C.customer_id =S.customer_id LEFT JOIN ".WEB_ADMIN_TABPOX."membercard M on M.customer_id =C.customer_id  WHERE  S.status in (0,1,2,3) AND S.agencyid =".$_SESSION["currentorgan"].' and '.$condition.' GROUP BY treatmentcard_id';
			}else if($ftable<>''){
			...
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  `treatmentcard_id` DESC  LIMIT ".$offset." , ".$psize);
	    	...
	    	$t -> set_var('recordcount',$count);	
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				//$t -> set_var('category_name',$this -> dbObj -> getone('select category_name from '.WEB_ADMIN_TABPOX.'servicecategory  where category_id ='.$inrrs["categoryid"]));
			  	$t -> set_var('delete',$this -> getDelStr('',$inrrs['treatmentcard_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['treatmentcard_id']));		
	...
	function goDispAppend(){
		...
		$table=WEB_ADMIN_TABPOX.'treatmentcard';
		$column='treatmentcard_no';
		$number=5;
		$id='treatmentcard_id';	
		$t -> set_var('treatmentcard_no',$this->makeno($Prefix,$agency_no,$table,$column,$number,$id));				
		 $t -> set_var('overdate',"");	
		$t -> set_var('treatmentcard_name',"");	
		...
		$t -> set_var('userid',$this->getUid());		
			...
			$date=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'treatmentcard WHERE treatmentcard_id = '.$updid.' and agencyid ='.$_SESSION["currentorgan"]);
...
function statuslist($status_name,$arraselectid=0){
	...
	function goDelete(){
	  if($_GET['consumables_id']==''){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'treatmentcard WHERE treatmentcard_id in('.$delid.')');
		}else{
		echo 'DELETE FROM '.WEB_ADMIN_TABPOX.'treatmentcarddetail WHERE treatmentcarddetail_id in('.$_GET['consumables_id'].')';
		 $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'treatmentcarddetail WHERE treatmentcarddetail_id in('.$_GET['consumables_id'].')');
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX.$cardtable.'  SET  remaintimeslist ="'.$_POST["remaintimeslist"].'" WHERE '.$cardtable.'_id in('.$id.')');
		}
//$this -> quit($info.'成功！');
