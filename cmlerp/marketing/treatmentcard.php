...
class Pagecustomer extends admin {
	function disp(){
		//定义模板
		$marketingcardtype_id= "2";
		$t = new Template('../template/marketing');
		$t -> set_file('f','treatmentcard.html');
		$t->unknowns = "remove";
	...
	function goDispAppend(){
		...
		$t = new Template('../template/marketing');
		$t -> set_file('f','treatmentcard_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
 ...
 //echo "INSERT INTO `".WEB_ADMIN_TABPOX."marketingcard` (`marketingcard_name`,`marketingcardtype_id`, `marketingcard_no`, `coderule`,`totaltimes`, `price`, `timelimit`, `pricepertime`, `assignservice`,`memo`, `agencyid`) VALUES ('" .$_POST["marketingcard_name"]."','".$marketingcardtype_id."','".$_POST["marketingcard_no"]."', '".$_POST["coderule"]."', '".$_POST["totaltimes"]."', '".$this->intnonull($_POST["price"])."',  '".$this->intnonull($_POST["timelimit"])."','".$this->intnonull($_POST["pricepertime"])."','".$this->intnonull($_POST["assignservice"])."', '".$_POST["memo"]."','".$_SESSION["currentorgan"]."')";
	 ...
	 $this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."marketingcard` (`marketingcard_name`,`marketingcardtype_id`, `marketingcard_no`, `coderule`,`totaltimes`, `price`, `timelimit`, `pricepertime`, `assignservice`,`memo`, `agencyid`,`sellcommission`,`leadercommission`,`beautycommission`,bgdate,enddate,limitdate,pricetype) VALUES ('" .$_POST["marketingcard_name"]."','".$marketingcardtype_id."','".$_POST["marketingcard_no"]."', '".$_POST["coderule"]."', '".$_POST["totaltimes"]."', '".$this->intnonull($_POST["price"])."',  '".$this->intnonull($_POST["timelimit"])."','".$this->intnonull($_POST["pricepertime"])."','".$this->intnonull($_POST["assignservice"])."', '".$_POST["memo"]."','".$_SESSION["currentorgan"]."', '".$_POST["sellcommission"]."', '".$_POST["leadercommission"]."', '".$_POST["beautycommission"]."',null,null,'".$limitdate."','".$_POST["pricetype"]."')");
 }
			...
			exit("<script>alert('新增成功');location.href='treatmentcard.php?action=upd&updid=".$id."';</script>");	
		}else{
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$limitdate=$_POST['limitdate']=='on'?1:0;
		...
		//echo $_POST["con_act"];
			...
			//echo "UPDATE `".WEB_ADMIN_TABPOX."marketingcard` SET `marketingcard_name` = '".$_POST["marketingcard_name"]."',`marketingcardtype_id` = '".$marketingcardtype_id."',`marketingcard_no` = '".$_POST["marketingcard_no"]."',`coderule` = '".$_POST["coderule"]."',`totaltimes` = '".$this->intnonull($_POST["totaltimes"])."',`timelimit` ='".$this->intnonull($_POST["timelimit"])."',`price` = '".$this->intnonull($_POST["price"])."',`pricepertime` = '".$this->intnonull($_POST["pricepertime"])."',`assignservice` = '".$this->intnonull($_POST["assignservice"])."',`memo` = '".$_POST["memo"]."',`sellcommission` = '".$_POST["sellcommission"]."',`leadercommission` = '".$_POST["leadercommission"]."' ,`beautycommission` = '".$_POST["beautycommission"]."' ,`bgdate` = null,`enddate` = null,`limitdate` = '".$limitdate."'  WHERE marketingcard_id =".$id;
		}
 if($_POST["con_act"]=='upd'){
	 ...
	 //echo "UPDATE `".WEB_ADMIN_TABPOX."marketingcarddetail` SET `services_id` = '".$_POST["cservices_id"]."',`pricepertime` = '".$_POST["cpricepertime"]."',`services_times` = '".$_POST["cservices_times"]."',`memo` = '".$_POST["cmemo"]."' WHERE marketingcarddetail_id  =".$_POST['conupdid'];
		exit("<script>alert('修改成功');location.href='treatmentcard.php?action=upd&updid=".$id."';</script>");
