...
class Pagecustomer extends admin {
	...
	$res=$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."promotion` SET `status` = 5 where promotion_id=".$_GET['updid']);	
	 
		if($res&&$status==4){
		$this -> dbObj -> Execute("COMMIT");
			exit("<script>alert('审核成功');window.location.href='promotionhistory.php';</script>");
		}else{
	...
	$res=$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."promotion` SET `status` = 6 where promotion_id=".$_GET['updid']);	
		if($res&&$status==4){
		$this -> dbObj -> Execute("COMMIT");	
			exit("<script>alert('撤回成功');window.location.href='promotionhistory.php';</script>");
		}else{
		$this -> dbObj -> Execute("ROLLBACK");
			$this -> quit('撤回失败！');	
		} 
	...
	$this -> dbObj -> Execute("START TRANSACTION");//事务开始。
	$status=$this -> dbObj -> GetOne("select status from `".WEB_ADMIN_TABPOX."promotion` where promotion_id=".$_GET['updid']);
	$res=$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."promotion` SET `status` = 2 where promotion_id=".$_GET['updid']);	
		if($res&&$status==5){
		$this -> dbObj -> Execute("COMMIT");
		exit("<script>alert('操作成功');window.location.href='promotionhistory.php';</script>");	
		}else{
		$this -> dbObj -> Execute("ROLLBACK");
			$this -> quit('操作失败！');	
		} 
	...
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'promotion  where promotiontype_id ='.$promotiontype_id.' and status>1 and status<6 and agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql="select * from ".WEB_ADMIN_TABPOX."promotion s INNER JOIN ".WEB_ADMIN_TABPOX."$ftable f on s.categoryid =f.category_id  where f.category_name like '%".$keywords."%' and s.promotiontype_id ='.$promotiontype_id.' and s.status>1 and s.status<6  and  s.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'promotion  where  agencyid ='.$_SESSION["currentorgan"].' and status>1 and status<6 ';
	...
	function goDispAppend(){
		...
		$t = new Template('../template/marketing');
		$t -> set_file('f','promotionhistory_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		...
		$auditdisabledlist=array("disabled","disabled","disabled","disabled","","disabled","disabled","disabled");
		$t -> set_var('auditdisabled',$auditdisabledlist[$date['status']]);	//审核按钮可用状态
		$recoildisabledlist=array("disabled","disabled","disabled","disabled","","disabled","disabled","disabled");
		$t -> set_var('recoildisabled',$recoildisabledlist[$date['status']]);	//撤回/反冲 按钮可用状态
		$stopdisabledlist=array("disabled","disabled","disabled","disabled","disabled","","disabled","disabled");
		$t -> set_var('stopdisabled',$stopdisabledlist[$date['status']]);	//作废 按钮可用状态	 
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."promotion` (`promotion_no`,`promotion_name`, `bgdate`, `enddate`,`assigntype`, `memo`, man,`agencyid`) VALUES ('" .$_POST["promotion_no"]."','".$_POST["promotion_name"]."','".$_POST["bgdate"]."', '".$_POST["enddate"]."', '".$_POST["assigntype"]."', '".$_POST["memo"]."', '".$man."','".$_SESSION["currentorgan"]."')");
  ...
  //echo "INSERT INTO `".WEB_ADMIN_TABPOX."promotion` (`promotion_no`,`promotion_name`, `bgdate`, `enddate`,`assigntype`, `memo`, `agencyid`) VALUES ('" .$_POST["promotion_no"]."','".$_POST["promotion_name"]."','".$_POST["bgdate"]."', '".$_POST["enddate"]."', '".$_POST["assigntype"]."', '".$_POST["memo"]."', '".$_SESSION["currentorgan"]."')";
			...
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."promotion` SET `promotion_name` = '".$_POST["promotion_name"]."', `promotion_no` = '".$_POST["promotion_no"]."',`bgdate` = '".$_POST["bgdate"]."',`enddate` = '".$_POST["enddate"]."',`assigntype` ='".$_POST["assigntype"]."', `memo` = '".$_POST["memo"]."' WHERE promotion_id =".$id);
