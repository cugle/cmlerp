...
class Pagecustomer extends admin {
	function disp(){
		//定义模板
		$marketingcardtype_id= "6";
		$t = new Template('../template/marketing');
		$t -> set_file('f','cashcoupon.html');
		$t->unknowns = "remove";
	...
	function goDispAppend(){
		...
		$t = new Template('../template/marketing');
		$t -> set_file('f','cashcoupon_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."marketingcard` (`marketingcard_name`,`marketingcardtype_id`, `marketingcard_no`, `coderule`,`totaltimes`, `price`, `timelimit`, `pricepertime`, `assignservice`,`value` ,`memo`, `agencyid`,bgdate,enddate,limitdate) VALUES ('" .$_POST["marketingcard_name"]."','".$marketingcardtype_id."','".$_POST["marketingcard_no"]."', '".$_POST["coderule"]."', '".$_POST["totaltimes"]."', '".$this->intnonull($_POST["price"])."',  '".$this->intnonull($_POST["timelimit"])."','".$this->intnonull($_POST["pricepertime"])."','".$this->intnonull($_POST["assignservice"])."','".$_POST["value"]."', '".$_POST["memo"]."','".$_SESSION["currentorgan"]."','".$_POST["bgdate"]."','".$_POST["enddate"]."','".$limitdate."')");
// echo "INSERT INTO `".WEB_ADMIN_TABPOX."marketingcard` (`marketingcard_name`,`marketingcardtype_id`, `marketingcard_no`, `coderule`,`totaltimes`, `price`, `timelimit`, `pricepertime`, `assignservice`,`memo`, `agencyid`) VALUES ('" .$_POST["marketingcard_name"]."','".$marketingcardtype_id."','".$_POST["marketingcard_no"]."', '".$_POST["coderule"]."', '".$_POST["totaltimes"]."', '".$this->intnonull($_POST["price"])."',  '".$this->intnonull($_POST["timelimit"])."','".$this->intnonull($_POST["pricepertime"])."','".$this->intnonull($_POST["assignservice"])."', '".$_POST["memo"]."','".$_SESSION["currentorgan"]."')";
			...
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."marketingcard` (`marketingcard_name`,`marketingcardtype_id`, `marketingcard_no`, `coderule`,`totaltimes`, `price`, `timelimit`, `pricepertime`, `assignservice`,`value` ,`memo`, `agencyid`,limitdate) VALUES ('" .$_POST["marketingcard_name"]."','".$marketingcardtype_id."','".$_POST["marketingcard_no"]."', '".$_POST["coderule"]."', '".$_POST["totaltimes"]."', '".$this->intnonull($_POST["price"])."',  '".$this->intnonull($_POST["timelimit"])."','".$this->intnonull($_POST["pricepertime"])."','".$this->intnonull($_POST["assignservice"])."','".$_POST["value"]."', '".$_POST["memo"]."','".$_SESSION["currentorgan"]."', '".$limitdate."')");
			}
			...
			exit("<script>alert('新增成功');location.href='cashcoupon.php?action=upd&updid=".$id."';</script>");	
		}else{
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$limitdate=$_POST['limitdate']=='on'?1:0;
			if($_POST['bgdate']<>''&&$_POST['enddate']<>''){
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."marketingcard` SET `marketingcard_name` = '".$_POST["marketingcard_name"]."',`marketingcardtype_id` = '".$marketingcardtype_id."',`marketingcard_no` = '".$_POST["marketingcard_no"]."',`coderule` = '".$_POST["coderule"]."',`totaltimes` = '".$this->intnonull($_POST["totaltimes"])."',`timelimit` ='".$this->intnonull($_POST["timelimit"])."',`price` = '".$this->intnonull($_POST["price"])."',`pricepertime` = '".$this->intnonull($_POST["pricepertime"])."',`assignservice` = '".$this->intnonull($_POST["assignservice"])."',`value` = '".$_POST["value"]."',`memo` = '".$_POST["memo"]."',`bgdate` = '".$_POST["bgdate"]."',`enddate` = '".$_POST["enddate"]."',`limitdate` = '".$limitdate."'  WHERE marketingcard_id =".$id);

		}else{
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."marketingcard` SET `marketingcard_name` = '".$_POST["marketingcard_name"]."',`marketingcardtype_id` = '".$marketingcardtype_id."',`marketingcard_no` = '".$_POST["marketingcard_no"]."',`coderule` = '".$_POST["coderule"]."',`totaltimes` = '".$this->intnonull($_POST["totaltimes"])."',`timelimit` ='".$this->intnonull($_POST["timelimit"])."',`price` = '".$this->intnonull($_POST["price"])."',`pricepertime` = '".$this->intnonull($_POST["pricepertime"])."',`assignservice` = '".$this->intnonull($_POST["assignservice"])."',`value` = '".$_POST["value"]."',`memo` = '".$_POST["memo"]."' ,`bgdate` = NULL,`enddate` = NULL,`limitdate` = '".$limitdate."'  WHERE marketingcard_id =".$id);	
 ...
 if($_POST["con_act"]=='upd'){
		...
		exit("<script>alert('修改成功');location.href='cashcoupon.php?action=upd&updid=".$id."';</script>");
		}else{
