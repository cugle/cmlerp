...
class Pagecustomer extends admin {
	function disp(){
		//定义模板
		$marketingcardtype_id= "4";
		$t = new Template('../template/marketing');
		$t -> set_file('f','feelingcard.html');
		$t->unknowns = "remove";
	...
	function goDispAppend(){
		...
		$t = new Template('../template/marketing');
		$t -> set_file('f','feelingcard_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."marketingcard` (`marketingcard_name`,`marketingcardtype_id`, `marketingcard_no`, `coderule`,`totaltimes`, `price`, `timelimit`, `pricepertime`, `assignservice`,`value`,`memo`, `agencyid`,`sellcommission`,`leadercommission`,`beautycommission`,bgdate,enddate,limitdate) VALUES ('" .$_POST["marketingcard_name"]."','".$marketingcardtype_id."','".$_POST["marketingcard_no"]."', '".$_POST["coderule"]."', '".$_POST["totaltimes"]."', '".$this->intnonull($_POST["price"])."',  '".$this->intnonull($_POST["timelimit"])."','".$this->intnonull($_POST["pricepertime"])."','".$this->intnonull($_POST["assignservice"])."', '".$_POST["value"]."', '".$_POST["memo"]."','".$_SESSION["currentorgan"]."', '".$_POST["sellcommission"]."', '".$_POST["leadercommission"]."', '".$_POST["beautycommission"]."',null,null,'".$limitdate."')");
			}
			...
			exit("<script>alert('新增成功');location.href='feelingcard.php?action=upd&updid=".$id."';</script>");	
		}else{
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$limitdate=$_POST['limitdate']=='on'?1:0;
 ...
 if($_POST["ser_act"]=='upd'){
		...
		exit("<script>alert('修改成功');location.href='feelingcard.php?action=upd&updid=".$id."';</script>");
