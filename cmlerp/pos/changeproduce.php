...
class Pageproduce extends admin {
	...
	function disp(){
		...
		$t -> set_file('f','changeproduce.html');
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'%"';}else{$condition=$category.' like " %'.$keywords.'%"';}
			...
			$t -> set_var('customer_id',$customer_name);
			$t -> set_var('oldproducelist',$this -> oldproduce($customer_id));
...
function oldproduce($customer_id=0){
if($customer_id==0){
	$oldproduce='';
}else{
			 $sql='select * from '.WEB_ADMIN_TABPOX.'selldetail A inner join  '.WEB_ADMIN_TABPOX.'sell B ON A.sell_id =B.sell_id  where B.customer_id='.$customer_id ;
 
			 $inrs = &$this -> dbObj -> Execute($sql);
			 $oldproduce='';
			
			 while ($inrrs = &$inrs -> FetchRow()) {	 
				$produce_name=$this -> dbObj -> getone('select produce_name from '.WEB_ADMIN_TABPOX.'produce  where produce_id ='.$inrrs["item_id"]);
				$oldproduce=$oldproduce.'<option value="'.$inrrs["selldetail_id"].'@@@'.$inrrs["cardid"].'@@@'.$inrrs["item_id"].'">'.$produce_name.$inrrs["amount"].'</option> ';
				}
} 
			return $oldproduce;							 
}
...
$selldetail_id=$oldproduce[0];
		 ...
		 $id = $this->SellObj->addsellitem($sellid,$item_type=1,$item_id,$number,$value,$price,10,$beauty_id,$cardtable='selldetail',1,0,0,$employee_id);
		//最后那个1和0分别指 默认仓库，自动指定仓库
		// $this -> dbObj -> Execute('INSERT INTO  '.WEB_ADMIN_TABPOX.'sell WHERE membershipcard_id in('.$delid.')');
		 $this -> dbObj -> Execute('update  '.WEB_ADMIN_TABPOX.'selldetail set returnstatus=1, returnid='.$selldetail_id.' WHERE selldetail_id in('.$id.')');
		 echo 'update  '.WEB_ADMIN_TABPOX.'selldetail set returnstatus=1 , returnid='.$selldetail_id.' WHERE selldetail_id in('.$id.')';
