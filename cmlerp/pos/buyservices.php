...
class Pageservices extends admin {
	...
	function disp(){
		...
		$t -> set_file('f','buyservices1.html');
		$t->unknowns = "keep";
	...
	function step2(){
		//定义模板
		$t = new Template('../template/pos');
		$t -> set_file('f','buyservices2.html');
		$t->unknowns = "keep";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		$t -> set_block('f','mainlist','ml');
		$servicesdata=$_GET['servicesdata'];
		$servicesdata = explode('@@@',$servicesdata);
		$services_id=$servicesdata[0];
		$services_no=$servicesdata[1];
		$services_name=$servicesdata[2];
		$code=$servicesdata[3];
		$price=$servicesdata[4];
	...
	function step3(){
		...
		// $this -> dbObj -> Execute('INSERT INTO  '.WEB_ADMIN_TABPOX.'sell WHERE membershipcard_id in('.$delid.')');
		 ...
		 //定义模板
		$t = new Template('../template/pos');
		$t -> set_file('f','buyservices3.html');
		$t->unknowns = "keep";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		$t -> set_block('f','mainlist','ml');
