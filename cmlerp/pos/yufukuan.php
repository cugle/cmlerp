...
class Pagecustomer extends admin {
	...
	function goDispAppend(){
		...
		$t -> set_var('yufukuan',$memberdata['yufukuan']);
		$t -> set_var('yufu_memo','');
		  
		
		$t -> set_var('yufuvalue','0.00');
	...
	function goAppend(){
		...
		if($this -> isAppend){
		 ...
		 $item_id=0;
		 $number=1;
		 $value=$_POST["yufuvalue"];
		 $price=$_POST["yufuvalue"];
		 $discount=10;
		 $cardtype=0;
		 $cardid=0;
		 $customercardid=$_POST['customer_id'];
		 $employee_id=$_POST['employee_id'];
		 $itemmemo=$_POST['yufu_memo'];
...
//$this -> quit($info.'成功！');
		if(mysql_affected_rows()){
		$yufuvalue=$this->dbObj->GetOne('select yufukuan from '.WEB_ADMIN_TABPOX.'customer  WHERE   agencyid='.$agencyid.' and customer_id ='.$_POST["customer_id"]);	
...
return $nostr;
	...
	function quit($info){
		exit("<script>alert('$info');window.parent.close();</script>");
	}
