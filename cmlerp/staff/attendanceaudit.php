...
class Pagecustomer extends admin {
    ...
    function viewhistorty(){
		...
		$_GET['action']==viewhistorty?$t -> set_var('auditdisabled','disabled="disabled"'):$t -> set_var('auditdisabled','');	
	...
	if ($_GET['auditstatus']==0){
	 $this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."attendance SET `auditstatus` = 3 WHERE agencyid =".$_SESSION["currentorgan"]);
	 $info='撤回';
	 }else {
	 $this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."attendance SET `auditstatus` = 2 where agencyid =".$_SESSION["currentorgan"]);  
	 $info='审核';	
