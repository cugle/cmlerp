...
class Pageselectcustomer extends admin {
	...
	function disp(){
		...
		if($_GET[qgtype]<>""&$_GET[Sale_ID]<>""){
		  $t -> set_var('pagelist',$this -> page("customer.php?qgtype=".$_GET[qgtype]."&Sale_ID=".$_GET[Sale_ID],$count,$psize,$pageid));
		}else
		{
        $t -> set_var('pagelist',$this -> page($_SERVER['HTTP_REFERER'],$count,$psize,$pageid));
		}
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '增加';
			$this -> dbObj -> Execute('INSERT INTO '.WEB_ADMIN_TABPOX."customer(customername,customerpass,loginnum,importer)values('".$_POST['customername']."','".md5($_POST['customerpass'])."',0,".$this->getUid().")");
			...
			if($_POST['password']){
				...
				$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."customer SET customername='".$_POST['customername']."',gender=".$_POST["gender"].",birthday='".$_POST["birthday"]."',national='".$_POST["national"]."', idnumber='".$_POST["idnumber"]."', tel='".$_POST["tel"]."', handphone='".$_POST["handphone"]."', address='".$_POST["address"]."', zipcode='".$_POST["zipcode"]."', email='".$_POST["email"]."',areaid=".$_POST["ClassID"].",userid=".$_POST["Sale_ID"]." WHERE customerid = $id");
			}
