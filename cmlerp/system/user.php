...
class Pagecustomer extends admin {
	...
	function disp(){
		...
		if($ftable==''){$condition=$category.'="'.$keywords.'"';}else{$condition=$category.'="'.$keywords.'"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'user  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'user a INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on a.categoryid =f.category_id  where f.category_name like '%".$keywords."%' and  a.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'user where  agencyid ='.$_SESSION["currentorgan"];
			 
			}
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  userid DESC  LIMIT ".$offset." , ".$psize);
			...
			//$inrs = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'user  where agencyid ='.$_SESSION["currentorgan"]);
			 //echo 'select * from '.WEB_ADMIN_TABPOX.'roomgroup  where agencyid ='.$_SESSION["currentorgan"];
	     	while ($inrrs = &$inrs -> FetchRow()) {
				$t -> set_var($inrrs);

				$data = $this -> dbObj -> Execute("select g.* from ".WEB_ADMIN_TABPOX."usergroup ug inner join ".WEB_ADMIN_TABPOX."group g on ug.groupid=g.groupid where ug.userid = ".$inrrs['userid']);
			while ($inrrs1 = &$data -> FetchRow()) {
				foreach ($inrrs1 as $k=>$v) $t -> set_var($k,$v);
				$t -> parse('g','group',true);
			}
			$data -> Close();				
			   	$t -> set_var('delete',$this -> getDelStr('',$inrrs['userid']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['userid']));						   
	...
	function goDispAppend(){
		...
		if($this -> isAppend){
			...
			$data=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'user WHERE userid = '.$updid);
			$t -> set_var($data);
			
			$t -> set_var($this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'employee WHERE employee_id = '.$data['employee_id']));
	...
	function goAppend(){
		...
		if($this -> isAppend){
			...
			$this -> dbObj -> Execute('INSERT INTO '.WEB_ADMIN_TABPOX."user(username,userpass,loginnum,importer,employee_id,agencyid)values('".$_POST['username']."','".md5($_POST['password'])."',0,".$this->getUid().",'".$_POST['employee_id']."',".$_SESSION["currentorgan"].")");
			...
			if($_POST['password']){
				$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."user SET username='".$_POST['username']."',userpass='".md5($_POST['password'])."',employee_id='".$_POST['employee_id']."' WHERE userid = $id");
			}else{
				$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."user SET username='".$_POST['username']."',employee_id='".$_POST['employee_id']."' WHERE userid = $id");
			}
