...
class Pagevisiteplan extends admin {
	...
	function disp(){
		...
		$condition = $condition ? " WHERE userid=".$this ->getUid()." and ".$condition : "";	
	...
	function PPClass_sale($userid=0){
    		$sale =$this -> dbObj -> Execute("SELECT * FROM ".WEB_ADMIN_TABPOX."grouprole gr, ".WEB_ADMIN_TABPOX."usergroup ug, ".WEB_ADMIN_TABPOX."user u WHERE gr.groupid = ug.groupid AND u.userid = ug.userid AND gr.roleid in(2,3,4,6)");
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '增加';
			$this -> dbObj -> Execute('INSERT INTO '.WEB_ADMIN_TABPOX."visiteplan(plantitle,customerid,userid,plantime,plan_content,planmemo)values('".$_POST['plantitle']."',".$_POST['customerid'].",".$_POST['userid'].",'".$_POST["plantime"]."','".$_POST["plan_content"]."','".$_POST["planmemo"]."')");
			...
			if($_POST['password']){
				...
				$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."visiteplan SET plantitle='".$_POST['plantitle']."',customerid=".$_POST["customerid"].",userid=".$_POST["userid"].",plantime='".$_POST["plantime"]."', creattime='".$_POST["creattime"]."', plan_content='".$_POST["plan_content"]."',planmemo='".$_POST["planmemo"]."' WHERE planid =".$id);
