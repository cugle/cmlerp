...
class Pagedepreciation extends admin {
	function disp(){
		//定义模板
		$t = new Template('../template/fixedassets');
		$t -> set_file('f','depreciation.html');
		$t->unknowns = "remove";
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'%"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'depreciation  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'depreciation p INNER JOIN '.WEB_ADMIN_TABPOX.$ftable." f on p.".$ftable."_id =f.".$ftable."_id  where f.".$category." like '%".$keywords."%' and  p.agencyid =".$_SESSION["currentorgan"] ;
			 
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'depreciation  where  agencyid ='.$_SESSION["currentorgan"];
			 
			}
			 
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  depreciation_id DESC  LIMIT ".$offset." , ".$psize);
			...
			//$inrs = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'produce  where agencyid ='.$_SESSION["currentorgan"]);
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				$t -> set_var('catalog_name',$fixedassetscatalog['fixedassetscatalog_name']);	
			   	$t -> set_var('delete',$this -> getDelStr('',$inrrs['depreciation_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['depreciation_id']));				
	...
	function goDispAppend(){
		...
		$t = new Template('../template/fixedassets');
		$t -> set_file('f','depreciation_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		...
		if($_GET['fixedassets_id']<>''){
			...
			$data=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'depreciation WHERE depreciation_id = '.$updid);
			$t -> set_var($data);
	...
	function goDelete(){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'depreciation WHERE depreciation_id in('.$delid.')');
		$info='删除';
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '增加';	
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."depreciation` ( `fixedassets_id`, `depreciationvalue`, `date`, `memo`, `agencyid`)VALUES ('".$_POST["fixedassets_id"]."','".$_POST["depreciationvalue"]."','".$_POST["date"]."','".$_POST["memo"]."','".$_SESSION["currentorgan"]."')");
 
echo "INSERT INTO `".WEB_ADMIN_TABPOX."depreciation` (`fixedassets_id`, `depreciationvalue`, `date`, `memo`, `agencyid`)VALUES ('".$_POST["fixedassets_id"]."','".$_POST["depreciationvalue"]."','".$_POST["date"]."','".$_POST["memo"]."','".$_SESSION["currentorgan"]."')";
			$id = $this -> dbObj -> Insert_ID();
		}else{
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."depreciation` SET `fixedassets_id`='".$_POST["fixedassets_id"]."', `depreciationvalue`='".$_POST["depreciationvalue"]."', `date`='".$_POST["date"]."', `memo`='".$_POST["memo"]."'  WHERE depreciation_id =".$id);
...
return $nostr;
	...
	function quit($info){
		exit("<script>alert('$info');location.href='depreciation.php';</script>");
	}
