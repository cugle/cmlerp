...
function disp(){	
	...
	function printbill(){
		...
		$t = new Template('../template/finace');
		$t -> set_file('f','incomestatement_bill.html');
		$t->unknowns = "remove";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		...
		$timecondition=' date  between "'.$bgdate.'" and "'.$enddate.'"';	
		
		$bgyear=$this -> dbObj -> getone('select enddate  from '.WEB_ADMIN_TABPOX.'annualbatch  where agencyid ='.$_SESSION["currentorgan"].' order by annualbatch_id desc');	//年结
		
		
		if (!$bgyear){//如果没有年结过 
			//$bgyear=$this -> dbObj -> getone('select creattime  from '.WEB_ADMIN_TABPOX.'sell  where agencyid ='.$_SESSION["currentorgan"].' order by sell_id asc');	 
			
			 $bgyear=date("Y",time());//
			 $bgyear=$bgyear."-01-01";//设置为本年一月一日；
			 //$bgdate=date("Y-m-d",strtotime("$m+1 days",strtotime($bgdate)));//设置开始时间为上次月结的下一天
		}else{
		$bgyear=date("Y-m-d",strtotime("$m+1 days",strtotime($bgyear)));
		}

		$sqlinc='select * from '.WEB_ADMIN_TABPOX.'incomestatementtype  where type in (0,1)  ORDER BY orderid asc, incomestatementtype_id asc';//
		...
		$sqlyeartransfervoucherhistory1=$sqlyeartransfervoucherhistory1<>''?$sqlyeartransfervoucherhistory1.' union ':'';
			$sqlyeartransfervoucherhistory1=$sqlyeartransfervoucherhistory1.' (select A.accounttitle_id,A.transfervoucher_id,B.date,sum(A.lend) as lend ,sum(A.loan) as loan from '.WEB_ADMIN_TABPOX.'transfervoucherdetail A INNER JOIN  '.WEB_ADMIN_TABPOX.'transfervoucher  B ON A.transfervoucher_id=B.transfervoucher_id where A.accounttitle_id='.$inrrsaccounttile["accounttitle_id"].' and B.agencyid in ('.$agencyid.') and  B.date  between "'.$bgyear.'" and "'.$enddate.'" having sum(A.lend) is not null )';//本年数
			
		$sqlmonthtransfervoucherhistory1=$sqlmonthtransfervoucherhistory1<>''?$sqlmonthtransfervoucherhistory1.' union ':'';	
			$sqlmonthtransfervoucherhistory1=$sqlmonthtransfervoucherhistory1.' (select A.accounttitle_id,A.transfervoucher_id,B.date,sum(A.lend) as lend ,sum(A.loan) as loan from '.WEB_ADMIN_TABPOX.'transfervoucherdetail A INNER JOIN  '.WEB_ADMIN_TABPOX.'transfervoucher  B ON A.transfervoucher_id=B.transfervoucher_id where A.accounttitle_id='.$inrrsaccounttile["accounttitle_id"].' and B.agencyid in ('.$agencyid.') and  B.date  between "'.$bgdate.'" and "'.$enddate.'" having sum(A.lend) is not null )';	//本月数		
			//$sqlbgyeartransfervoucherhistory1=$sqlbgyeartransfervoucherhistory1<>''?$sqlbgyeartransfervoucherhistory1.' union ':'';	//期末数
			//$sqlbgyeartransfervoucherhistory1=$sqlbgyeartransfervoucherhistory1.' (select * from '.WEB_ADMIN_TABPOX.'transfervoucherhistory1  where accounttitle_id='.$inrrsaccounttile["accounttitle_id"].' and agencyid in ('.$agencyid.') and  date  between "'.$bgyear.'" and "'.$enddate.'" order by `transfervoucherhistory_id` asc limit 1 )';	//年初数		
		}
		...
		$this -> dbObj -> Execute('CREATE TEMPORARY TABLE IF NOT EXISTS `'.WEB_ADMIN_TABPOX.'incomestatementmonth'.$this->getUid().'` '.$sqlmonthtransfervoucherhistory1);
		$sqlmonthtransfervoucherhistory1='select * from '.WEB_ADMIN_TABPOX.'incomestatementmonth'.$this->getUid();
		
		$this -> dbObj -> Execute('CREATE TEMPORARY TABLE IF NOT EXISTS `'.WEB_ADMIN_TABPOX.'incomestatementyear'.$this->getUid().'` '.$sqlyeartransfervoucherhistory1);
		$sqlyeartransfervoucherhistory1='select * from '.WEB_ADMIN_TABPOX.'incomestatementyear'.$this->getUid();		
		
		//$this -> dbObj -> Execute('CREATE TEMPORARY TABLE IF NOT EXISTS `'.WEB_ADMIN_TABPOX.'incomestatement'.$this->getUid().'` '.$sqltransfervoucherhistory1);
		//$sqltransfervoucherhistory1='select * from '.WEB_ADMIN_TABPOX.'incomestatement'.$this->getUid();			
		
		$incdata=&$this -> dbObj ->GetArray($sqlinc);
		$countinc=sizeof($incdata);
		...
		$sqlmonthinc=' select  A.accounttitle_id,sum(lend) as lend,sum(loan) as loan from ('.$sqlmonthtransfervoucherhistory1.')  A  WHERE A.accounttitle_id in ('.$incdata[$i]["accounttitleid"].') ';//本月部分
		$sqlmonthincreduce=' select  A.*,sum(A.balance-A.lend+A.loan) as endbalance  from ('.$sqlmonthtransfervoucherhistory1.')  A  WHERE A.accounttitle_id in ('.$incdata[$i]["addorreduce"].') ';//本月部分  取反 利用 原理借-贷 -2（借-贷）=贷-借
		
		
		$sqlyearinc=' select A.accounttitle_id,sum(lend) as lend,sum(loan) as loan from ('.$sqlyeartransfervoucherhistory1.')  A  WHERE A.accounttitle_id in ('.$incdata[$i]["accounttitleid"].') ';//本年数据
		$sqlyearincreduce=' select A.accounttitle_id,sum(lend) as lend,sum(loan) as loan from ('.$sqlyeartransfervoucherhistory1.')  A  WHERE A.accounttitle_id in ('.$incdata[$i]["addorreduce"].') ';//本年数据需减部分
		
		//$sqlbgyearinc=' select A.*, sum(A.balance-A.lend+A.loan) as endbalance from ('.$sqlbgyeartransfervoucherhistory1.')  A  WHERE A.accounttitle_id in ('.$incdata[$i]["accounttitleid"].') ';//年初
	//	$sqlbgyearincreduce=' select A.*, sum(A.balance-A.lend+A.loan) as endbalance from ('.$sqlbgyeartransfervoucherhistory1.')  A  WHERE A.accounttitle_id in ('.$incdata[$i]["addorreduce"].') ';//年初 
		
		//资产
		if($incdata[$i]['parentid']<>0){
					 
		$parentid1=$this -> dbObj ->GetOne('SELECT  parentid FROM '.WEB_ADMIN_TABPOX.'incomestatementtype where incomestatementtype_id='.$incdata[$i]["parentid"]);
		...
		$t -> set_var('zcname',$space.$incdata[$i]['incomestatementtype_name']);
		}else{
		$space='';
		$t -> set_var('zcname','<strong>'.$space.$incdata[$i]['incomestatementtype_name'].'</strong>');
		}		
		
		
		if($incdata[$i]['incomestatementtype_no']<>""){
		
		$t -> set_var('zcno',$zcno);
		$zcno=$zcno+1;
		 
		$monthinc=$this -> dbObj -> GetRow($sqlmonthinc);//本月
		$monthincreduce=$this -> dbObj -> GetRow($sqlmonthincreduce);//本月
		//$inc=$this -> dbObj -> GetRow($sqlinc); //
		$yearinc=$this -> dbObj -> GetRow($sqlyearinc); //本年
		$yearincreduce=$this -> dbObj -> GetRow($sqlyearincreduce); //本年
		//$bgyearincreduce=$this -> dbObj -> GetRow($sqlbgyearincreduce); //年初
		$thismonth=$monthinc['lend']-$monthinc['loan']-2*($monthincreduce['lend']-$monthincreduce['loan']);
		$thisyear=$yearinc['lend']-$yearinc['loan']-2*($yearincreduce['lend']-$yearincreduce['loan']);
		if($zcno==10){
 
			print_r($incyear);
			print_r($incyearreduce);
		...
		$thismonth=$thismonth;
		$thisyear=$thisyear;
		}else if($incdata[$i]['type']==0){

		$thismonth=-$thismonth;
		$thisyear=-$thisyear;
		}

		$t -> set_var('thismonth',$thismonth);
		$t -> set_var('addup',$thisyear);
		...
		$t -> set_var('year',substr($_GET["enddate"],0,4));
		$t -> set_var('month',substr($_GET["bgdate"],5,2));
