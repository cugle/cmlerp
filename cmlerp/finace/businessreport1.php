...
class Pagecustomer extends admin {
	...
	//	$t -> set_block('f','main','m');
		...
		$_POST["bgdate"]=$_GET["bgdate"]==''?$_POST["bgdate"]:$_GET["bgdate"];
		$_POST["enddate"]=$_GET["enddate"]==''?$_POST["enddate"]:$_GET["enddate"];
		...
		$t -> set_var('lastdate',date('Y-m-d',strtotime("$m-1 days",strtotime($enddate))));
		$t -> set_var('nextdate',date('Y-m-d',strtotime("$m+1 days",strtotime($enddate))));
		//$t -> set_var('m');
		//统计插入monthly表
		//美容师业绩统计
		//顾问业绩统计
...
会籍卡值对*/
 ...
 //========散客款项类====================
	$sql="select  * FROM ".WEB_ADMIN_TABPOX."sell  where  status in(1,4,5,6)  and   agencyid =".$_SESSION["currentorgan"].' and  creattime between "'.$bgdate.'" and "'.$enddate.'"' ; 
	$sellid='';
 	$sqltoday="select  * FROM ".WEB_ADMIN_TABPOX."sell  where  status in(1,4,5,6)  and   agencyid =".$_SESSION["currentorgan"].' and  creattime between "'.$bgtodate.'" and "'.$endtodate.'"' ; 
	 $sqltempsell='CREATE TEMPORARY TABLE  IF NOT EXISTS `'.WEB_ADMIN_TABPOX.'sell'.$this->getUid().'` '.$sql;
	 $sqltemptodaysell='CREATE TEMPORARY TABLE  IF NOT EXISTS `'.WEB_ADMIN_TABPOX.'selltoday'.$this->getUid().'` '.$sqltoday;
	$this -> dbObj -> Execute($sqltempsell);
	$this -> dbObj -> Execute($sqltemptodaysell);
	$inrs=$this -> dbObj -> Execute($sql);
	$totalnewnumber = $inrs->RecordCount() ;
	while ($inrrs = $inrs -> FetchRow()) {
		$sellid=$sellid==''?$inrrs['sell_id']:$sellid.",".$inrrs['sell_id'];	 
	}//将整个月的sellid串接成字符串 此处值得商榷！！！一个月的单太多了
...
$cmldiscount=$this->cmlpriceObj->fordaily($bgdate,$enddate);//打折处理储值卡 现金券，赠送消费金额2010.1.20=======================================================

//========本月散客款项类====================
$sql1="select  A.*, (SELECT sum(amount)  FROM ".WEB_ADMIN_TABPOX."sellotherdetail WHERE selldetail_id = B.selldetail_id) as yufu FROM  ".WEB_ADMIN_TABPOX."sell A LEFT JOIN  ".WEB_ADMIN_TABPOX."sellotherdetail B ON A.sell_id = B.sell_id  where A.status in(1,4,5,6) and (A.membercard_no='' or A.membercard_no is NULL) and  A.agencyid =".$_SESSION['currentorgan'].' and    A.creattime between "'.$bgdate.'" and "'.$enddate.'" GROUP BY A.sell_id'; 	
 
	$inrs1=$this -> dbObj -> Execute($sql1);
	$newnumber = $inrs1->RecordCount() ;//散客人数
	$totalnewyufu=0;
	$xianjin=0;
	$yinka=0;
	$billnumber=0;
	...
	while ($inrrs1 = $inrs1 -> FetchRow()) {
		...
		$own=$own+$inrrs1['payable1']-$inrrs1['realpay'];//欠款
		
		}		
	}

		$todaydeal=$this -> deal($bgtodate,$endtodate);//今日预定散客
		$totaldeal=$this -> deal($bgdate,$enddate);//今日预定散客
...
//========本月会员款项类====================
	$sql2="select  A.*, (SELECT sum(amount)  FROM ".WEB_ADMIN_TABPOX."sellotherdetail WHERE selldetail_id = B.selldetail_id) as yufu FROM  ".WEB_ADMIN_TABPOX."sell A LEFT JOIN  ".WEB_ADMIN_TABPOX."sellotherdetail B ON A.sell_id = B.sell_id  where A.status in(1,4,5,6) and A.membercard_no<>'' and  A.agencyid =".$_SESSION['currentorgan'].' and  A.creattime between "'.$bgdate.'" and "'.$enddate.'" GROUP BY A.sell_id'; 	
...
/*	echo "散客数".$newnumber;
	...
	$totalmemyufu=0;
	$totalnewyufu=$this -> newyufunumber($bgtodate,$enddate);//本月预定散客
	$totalmemyufu=$this -> memyufunumber($sellid);//本月预定散客	
	$t -> set_var('totalnewyufu',$totalnewyufu);
	$t -> set_var('totalmemyufu',$totalmemyufu);
	$t -> set_var('totojine',sprintf ("%01.2f",$xianjin+$yinka));
 
//==========今日散客============================
	$sql1_1="select  A.*, (SELECT sum(amount)  FROM ".WEB_ADMIN_TABPOX."sellotherdetail WHERE selldetail_id = B.selldetail_id) as yufu FROM  ".WEB_ADMIN_TABPOX."sell A LEFT JOIN  ".WEB_ADMIN_TABPOX."sellotherdetail B ON A.sell_id = B.sell_id  where A.status in(1,4,5,6) and A.sell_id in(".$sellidtoday.") and (A.membercard_no='' or A.membercard_no is NULL) and A.agencyid =".$_SESSION['currentorgan'].' and  A.creattime between "'.$bgtodate.'" and "'.$endtodate.'" GROUP BY A.sell_id'; 	
...
//==========今日会员============================
	$sql2_1="select  A.*, (SELECT sum(amount)  FROM ".WEB_ADMIN_TABPOX."sellotherdetail WHERE selldetail_id = B.selldetail_id) as yufu FROM  ".WEB_ADMIN_TABPOX."sell A LEFT JOIN  ".WEB_ADMIN_TABPOX."sellotherdetail B ON A.sell_id = B.sell_id  where A.status in(1,4,5,6) and A.sell_id in(".$sellidtoday.") and A.membercard_no<>'' and  A.agencyid =".$_SESSION['currentorgan'].' and  A.creattime between "'.$bgtodate.'" and "'.$endtodate.'" GROUP BY A.sell_id'; 	
	...
	$newyufu=0;
	$memyufu=0;
	$newyufu=$this -> newyufunumber($bgtodate,$endtodate);//今日预定散客
...
/*		$sellidtoday1=explode(",",$sellidtoday);
		for ($i=0;$i<count($sellidtoday1);$i++)	{
 			...
 			$inrs6=$this -> dbObj -> Execute("select * FROM  (".$sqlt6.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE A.sell_id =".$sellidtoday1[$i]."  AND status in(1,4,5,6)  and   A.agencyid =".$_SESSION['currentorgan'].' order by B.sell_no'); 
	...
	//$todaysql="select  * FROM  (select  * FROM ".WEB_ADMIN_TABPOX."sellconsumedetail  union select  * FROM ".WEB_ADMIN_TABPOX."sellservicesdetail ) A  RIGHT JOIN   ".WEB_ADMIN_TABPOX."sell  B  ON     A.sell_id = B.sell_id   LEFT JOIN ".WEB_ADMIN_TABPOX."selldetail C ON B.sell_id = C.sell_id   LEFT JOIN ".WEB_ADMIN_TABPOX."sellcarddetail D ON    B.sell_id = D.sell_id    WHERE  B.agencyid =".$_SESSION['currentorgan']." and B.creattime between '".$bgtodate."' and' ".$endtodate."'  " ;
	 $todaysql= "select * FROM   ".WEB_ADMIN_TABPOX."sell   WHERE   agencyid =".$_SESSION['currentorgan']." and status in(1,4,5,6)  and   creattime between '".$bgtodate."' and' ".$endtodate."'  order by  sell_no" ;
	 $sqltoday= "select A.* FROM   (".$sqlt6.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE    B.agencyid =".$_SESSION['currentorgan']." and B.creattime between '".$bgtodate."' and' ".$endtodate."'  and B.status in(1,4,5,6) order by  B.sell_no" ;
	 
	 $sqltemptodayselldetail='CREATE TEMPORARY TABLE  IF NOT EXISTS `'.WEB_ADMIN_TABPOX.'selldetail'.$this->getUid().'` '.$sqltoday;
	$this -> dbObj -> Execute($sqltemptodayselldetail);
	
	$sqlmonth= "select A.* FROM   (".$sqlt6.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE    B.agencyid =".$_SESSION['currentorgan']." and B.creattime between '".$bgdate."' and' ".$enddate."'  and B.status in(1,4,5,6) order by  B.sell_no" ;
	 
	$sqltempmonthselldetail='CREATE TEMPORARY TABLE  IF NOT EXISTS `'.WEB_ADMIN_TABPOX.'selldetailmonth'.$this->getUid().'` '.$sqlmonth;
	$this -> dbObj -> Execute($sqltempmonthselldetail);
//===============================================begin今日项目======================================================================	
	...
	$inrstoday=$this -> dbObj -> Execute($todaysql); 
		...
		$customerconsume=$this -> dbObj -> GetOne("select sum(A.number) as customerconsume FROM  `".WEB_ADMIN_TABPOX."selldetail".$this->getUid()."` A INNER JOIN `".WEB_ADMIN_TABPOX."selltoday".$this->getUid()."`  B ON A.sell_id=B.sell_id   WHERE  B.creattime between '".$bgtodate."' and '".$endtodate."' and  A.item_type IN(0,2,4)  AND  A.discount>0");
		
		$customerconsumeitems=$customerconsume;
		...
		$customerproduceamount=$this -> dbObj -> GetOne("select sum(amount) from  ".WEB_ADMIN_TABPOX."selldetail  A INNER JOIN ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE  B.creattime between '".$bgtodate."' and '".$endtodate."' and discount>0 and item_type=1");
		...
		//其他付方式
		$paytypeenlist=array('zengsongvalue','dingjinvalue','chuzhikavalue','xianjinquanvalue','yufuvalue','yufuproducevalue','zengsongproducevalue','zengsongcardvalue');
		$paytypelist=array('扣赠送账户','扣定金','扣储值卡','扣现金券','扣预收款','扣预收产品款','扣赠送产品款','扣赠送购卡款');
		$otherpay='';
		...
		//美容师列表
		$t -> set_var('blt');
 		//$inrsbeautytoday=$this -> dbObj -> Execute("select * FROM  (".$sqlstrt3." union ".$sqlstrt4.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE A.sell_id =".$inrrstoday['sell_id']."  AND status in(1,4,5,6)  and A.discount>0 and  A.agencyid =".$_SESSION['currentorgan']); 

		$inrsbeautytoday=$this -> dbObj -> Execute("select * FROM `".WEB_ADMIN_TABPOX."selldetail".$this->getUid()."`    WHERE   sell_id =".$inrrstoday['sell_id']);
		//echo "select * FROM `".WEB_ADMIN_TABPOX."selldetail".$this->getUid()."` A INNER JOIN `".WEB_ADMIN_TABPOX."selltoday".$this->getUid()."`  B ON A.sell_id=B.sell_id   WHERE  B.creattime between '".$bgtodate."' and '".$endtodate."' and  A.item_type IN(0,2,4)  AND  A.discount>0";
 		$beautylist='';
 		$beautyliststr='';	
		$beautymanual='';
		//美容师手工 现金券 储值卡 赠送消费金 打折处理。2010.1.2
		...
		//定金
		$inrsdingjintoday=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  `".WEB_ADMIN_TABPOX."selldetail".$this->getUid()."` A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE A.sell_id =".$inrrstoday['sell_id']."  AND status in(1,4,5,6) and A.item_type=5 and A.item_id=2  and A.agencyid =".$_SESSION['currentorgan']); 
		$dingjin1= $this -> dbObj -> GetOne("select dingjinvalue FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  sell_id =".$inrrstoday['sell_id']);
		$inrsdingjintoday=$inrsdingjintoday-$dingjin1;
		 $totaldingjin=$totaldingjin+$inrsdingjintoday;
		$t -> set_var('dingjin',$inrsdingjintoday);		
		//预收款
		$inrsyufutoday=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  `".WEB_ADMIN_TABPOX."selldetail".$this->getUid()."` A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE A.sell_id =".$inrrstoday['sell_id']."  AND status in(1,4,5,6) and A.item_type=5 and A.item_id=0 and A.agencyid =".$_SESSION['currentorgan']); 
		$yufu1= $this -> dbObj -> GetOne("select yufuvalue FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  sell_id =".$inrrstoday['sell_id']);
		$inrsyufutoday=$inrsyufutoday-$yufu1;
		$t -> set_var('yufu',$inrsyufutoday);
		$totalyufu=$totalyufu+$inrsyufutoday;
				//预收款
		$inrsyufuptoday=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM   `".WEB_ADMIN_TABPOX."selldetail".$this->getUid()."` A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE A.sell_id =".$inrrstoday['sell_id']."  AND status in(1,4,5,6) and A.item_type=5 and A.item_id=6 and A.agencyid =".$_SESSION['currentorgan']); 
		$yufup1= $this -> dbObj -> GetOne("select yufuproducevalue FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  sell_id =".$inrrstoday['sell_id']);
		$inrsyufuptoday=$inrsyufuptoday-$yufup1;
		$t -> set_var('yufup',$inrsyufuptoday);
		$totalyufup=$totalyufup+$inrsyufuptoday;
		//使用卡券类
 		$inrscoupontoday=$this -> dbObj -> Execute("select * FROM   `".WEB_ADMIN_TABPOX."selldetail".$this->getUid()."` A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE A.sell_id =".$inrrstoday['sell_id']."  AND status in(1,4,5,6)  and  A.item_type in(0,2,4)  and A.agencyid =".$_SESSION['currentorgan']); 
		...
		//======购买卡项===========================
 		$inrstoday2=$this -> dbObj -> Execute("select * FROM  `".WEB_ADMIN_TABPOX."selldetail".$this->getUid()."` A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE A.sell_id =".$inrrstoday['sell_id']."  AND status in(1,4,5,6) and A.item_type in(1,3,5)  and A.agencyid =".$_SESSION['currentorgan'] ); 
		...
		while ($inrrstoday2= $inrstoday2 -> FetchRow()) {
			...
			$buycardid=$buycardid==''?$inrrstoday2["item_id"]:$buycardid.",".$inrrstoday2["item_id"];
				...
				}else if ($inrrstoday2['item_type']==5&&$inrrstoday2['item_id']<>0&&$inrrstoday2['item_id']<>2){//款项
...
//=======================================================		
		...
		$t -> parse('t','todayitems',true);	
		 ...
		 $t -> set_var('totalown',$this->totalown($bgdate,$enddate));
		$t -> set_var('owntoday',sprintf ("%01.2f",$owntoday));
		$t -> set_var('totalbeautymanual',sprintf ("%01.2f",$this->totalbeautymanual($bgdate,$enddate,$cmldiscount)));
		...
		//本月定金 
		$totaladdupdingjin=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE   B.creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5,6) and A.item_type=5 and A.item_id=2  and A.agencyid =".$_SESSION['currentorgan']); 
		 $totaladdupdingjin1= $this -> dbObj -> GetOne("select sum(dingjinvalue) as dingjin FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5,6)  and  agencyid =".$_SESSION['currentorgan']);
		 //echo "select sum(dingjinvalue) as dingjin FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5,6)  and  agencyid =".$_SESSION['currentorgan'];
		// echo "select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE   B.creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5,6) and A.item_type=5 and A.item_id=2  and A.agencyid =".$_SESSION['currentorgan'];
		 $totaladdupdingjin=$totaladdupdingjin-$totaladdupdingjin1;
		$t -> set_var('totaladdupdingjin',$totaladdupdingjin);		
		//本月预收款
		$totaladdupyufu=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE  B.creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5,6) and A.item_type=5 and A.item_id=0 and A.agencyid =".$_SESSION['currentorgan']); 
		  $totaladdupyufu1= $this -> dbObj -> GetOne("select sum(yufuvalue) as yufu FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5,6) and agencyid =".$_SESSION['currentorgan']);
		 $totaladdupyufu=$totaladdupyufu-$totaladdupyufu1;
		//$t -> set_var('totaladdupyufu',$totaladdupyufu);
		//本月预收产品款
		$totaladdupyufup=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt5.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE  B.creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5,6) and A.item_type=5 and A.item_id=6 and A.agencyid =".$_SESSION['currentorgan']); 
		 $totaladdupyufup1= $this -> dbObj -> GetOne("select sum(yufuproducevalue) as yufup FROM  ".WEB_ADMIN_TABPOX."sell   WHERE  creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5,6) ");
		 $totaladdupyufup=$totaladdupyufup-$totaladdupyufup1;
		//$t -> set_var('totaladdupyufup',$totaladdupyufup);		 
		//产品累计
		$totaladdupbeautyproduceamount=$this -> dbObj -> GetOne("select sum(A.amount) as amount FROM  (".$sqlstrt1.") A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE  B.creattime between '".$bgdate."' and '".$enddate."'  AND status in(1,4,5,6) and A.item_type=1  and A.agencyid =".$_SESSION['currentorgan']); 
		 
	$t -> set_var('totaladdupbeautyproduceamount',$totaladdupbeautyproduceamount);
...
//========服务类====================	
	...
	$sql3="select  * FROM ".WEB_ADMIN_TABPOX."sell A LEFT JOIN  ".WEB_ADMIN_TABPOX."sellconsumedetail B ON A.sell_id = B.sell_id  where A.status in(1,4,5,6)  and  A.agencyid =".$_SESSION['currentorgan'].' and B.discount>0 and  A.creattime between "'.$bgdate.'" and "'.$enddate.'"' ; 		
	...
	$sql4="select  * FROM ".WEB_ADMIN_TABPOX."sell A LEFT JOIN  ".WEB_ADMIN_TABPOX."sellservicesdetail B ON A.sell_id = B.sell_id  where A.status in(1,4,5,6)  and  A.agencyid =".$_SESSION["currentorgan"].' and B.discount>0 and  A.creattime between "'.$bgdate.'" and "'.$enddate.'"'; 	
...
/*//===================================
	//购买卡项 
$sql3="select  * FROM ".WEB_ADMIN_TABPOX."sell A LEFT JOIN  ".WEB_ADMIN_TABPOX."sellcarddetail B ON A.sell_id = B.sell_id  where A.status in(1,4,5,6)  and  A.agencyid =".$_SESSION["currentorgan"].' and    B.item_type=3 AND  B.discount>0 and  A.creattime between "'.$bgdate.'" and "'.$enddate.'"' ; 		
...
//===================================
	//购买产品 
	$sql5="select  * FROM ".WEB_ADMIN_TABPOX."sell A LEFT JOIN  ".WEB_ADMIN_TABPOX."selldetail B ON A.sell_id = B.sell_id  where A.status in(1,4,5,6)  and  A.agencyid =".$_SESSION["currentorgan"].' and  B.item_type=1 AND  B.discount>0 and  A.creattime between "'.$bgdate.'" and "'.$enddate.'"' ; 		
...
$employee=$this -> dbObj -> Execute("select * from  ".WEB_ADMIN_TABPOX."employee  where   status=1 and  employeelevelid  in (2,3)  and  agencyid =".$_SESSION['currentorgan']);//顾问
$beauty=$this -> dbObj -> Execute("select * from  ".WEB_ADMIN_TABPOX."employee  where  status=1 and  employeelevelid in (1)  and  agencyid =".$_SESSION['currentorgan']);//美容师
$membercard=$this -> dbObj -> Execute("select * from  ".WEB_ADMIN_TABPOX."marketingcard  where  marketingcardtype_id   in (5)  and  agencyid =".$_SESSION['currentorgan'].' order by  marketingcard_id desc');//会籍卡
	...
	$inrs6=$this->dbObj->Execute("select *,A.employee_id AS consultant_id from `".WEB_ADMIN_TABPOX.'selldetailmonth'.$this->getUid()."` A INNER JOIN ".WEB_ADMIN_TABPOX."sell B ON A.sell_id= B.sell_id where A.agencyid =".$_SESSION['currentorgan']);
	
	while ($inrrs6 = $inrs6 -> FetchRow()) {
//=====顾问=============================		 
		if($inrrs6["consultant_id"]<>'' && $inrrs6["consultant_id"]<>0){
			
		$employee_id=explode(";",$inrrs6["consultant_id"]);
...
//=========美容师===============
		...
		//$beautymanual["'".$beauty_id[$i]."'"]=$beautymanual["'".$beauty_id[$i]."'"]+$inrrs6['value']*$inrrs6['number']/count($beauty_id);
			...
			$beautymanual["'".$beauty_id[$i]."'"]=$beautymanual["'".$beauty_id[$i]."'"]+$inrrs6['value']*$inrrs6['number']/count($beauty_id);
			$consumenumber["'".$beauty_id[$i]."'"]=$consumenumber["'".$beauty_id[$i]."'"]+$inrrs6['number']/count($beauty_id);//美容师手工费业绩（非提成）
		...
		for ($i=0;$i<count($beauty_id);$i++)	{
			...
			$beautyczk["'".$beauty_id[$i]."'"]=$beautyczk["'".$beauty_id[$i]."'"]+$inrrs6['value']*$inrrs6['number']/count($beauty_id);
				...
				if($marketingcard['inheritedid'] ==$inrrscard['cardidlist']){
					...
					$beautymembernumber["'".$beauty_id[$i]."'"]["'".$inrrscard['cardidlist']."'"]=sprintf ("%01.2f",$beautymembernumber["'".$beauty_id[$i]."'"]["'".$inrrscard['cardidlist']."'"]+$inrrs6['number']/count($beauty_id)); 
...
/*	 if($inrrs6["item_type"]==1){//美容师产品销售
		 $beauty_id=explode(";",$inrrs6["beauty_id"]);
		 for ($i=0;$i<count($beauty_id);$i++)	{
		 //$beautyproduce["'".$beauty_id[$i]."'"]=$beautyproduce["'".$beauty_id[$i]."'"]+$inrrs6['value']*$inrrs6['number']/count($beauty_id);
		 }
		 }*/
	}
...
$consultantitemtoday=$this ->consultantitemtoday($bgtodate,$endtodate);
$consultantproducetoday=$this ->consultantproducetoday($bgtodate,$endtodate);
$consultantaddupyeji=$this ->consultantaddupyeji($bgdate,$enddate);
$consultantproduce=$this -> consultantproduce($bgdate,$enddate);
...
while ($employeedata = $employee -> FetchRow()) {//顾问业绩
		...
		$t -> set_var('consultantproduce',sprintf ("%01.2f",$consultantproduce["'".$employeedata['employee_id']."'"]));
		$totaladdupconsultantproduce=$totaladdupconsultantproduce+$consultantproduce["'".$employeedata['employee_id']."'"];
...
/*		while ($membercarddata = $membercard -> FetchRow()) {//会籍卡
		...
		$consultantmemberstr='';
$cardyeji=$this->cardyeji($bgdate,$enddate);	
...
while ($inrrscard = $inrscard -> FetchRow()) {	 
	  ...
	  if($inrrscard['type']=='1'){
		 ...
		 $cardnumber1=$this -> dbObj -> GetRow("select count(*) as cardnumber FROM `".WEB_ADMIN_TABPOX.'selldetailmonth'.$this->getUid()."` A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id left join  ".WEB_ADMIN_TABPOX."marketingcard C ON A.item_id=C.marketingcard_id  WHERE A. B.status in(1,4,5,6) and A.item_type =3  and A.employee_id=".$employeedata['employee_id']." and B.creattime between '".$bgdate."' and '".$enddate."' and C.inheritedid in(".$inrrscard['cardidlist'].") and A.agencyid =".$_SESSION['currentorgan']);//这里假设了开卡只可以选一个顾问
           $cardnumber1['cardnumber']=$cardnumber1['cardnumber']==0?'':$cardnumber1['cardnumber'];
		  
		 }else if($inrrscard['type']=='0'){
		
	  
			 $cardjine1=$this -> dbObj -> GetRow("select sum(A.amount) as cardjine FROM  `".WEB_ADMIN_TABPOX.'selldetailmonth'.$this->getUid()."` A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id left join  ".WEB_ADMIN_TABPOX."marketingcard C ON A.item_id=C.marketingcard_id  WHERE  B.status in(1,4,5,6) and A.item_type =3 and  A.employee_id=".$employeedata['employee_id']." and B.creattime between '".$bgdate."' and '".$enddate."' and C.inheritedid in(".$inrrscard['cardidlist'].") and A.agencyid =".$_SESSION['currentorgan']);
			   $cardjine1['cardjine']=  $cardjine1['cardjine']==0?'': $cardjine1['cardjine'];			  
		
		 }	
		 $t -> set_var('consultantmembernumber',$cardyeji["'".$employeedata['employee_id']."'"]["'".$inrrscard['lookupcard_id']."'"]);
		$totalconsultantmembernumber["'".$inrrscard['lookupcard_id']."'"]=$totalconsultantmembernumber["'".$inrrscard['lookupcard_id']."'"]+$cardyeji["'".$employeedata['employee_id']."'"]["'".$inrrscard['lookupcard_id']."'"];
...
while ($inrrscard = $inrscard -> FetchRow()) {	 
	  ...
	  if($inrrscard['type']=='1'){
		  ...
		  $cardnumber=$this -> dbObj -> GetRow("select count(*) as cardnumber FROM  `".WEB_ADMIN_TABPOX.'selldetailmonth'.$this->getUid()."` A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id left join  ".WEB_ADMIN_TABPOX."marketingcard C ON A.item_id=C.marketingcard_id  WHERE  B.status in(1,4,5,6) and A.item_type =3  and  B.creattime between '".$bgdate."' and '".$enddate."' and C.inheritedid in(".$inrrscard['cardidlist'].") and A.agencyid =".$_SESSION['currentorgan']);
		...
		// $t -> set_var('totalconsultantmembernumber',$cardnumber['cardnumber']);
			 ...
			 $cardjine=$this -> dbObj -> GetRow("select sum(A.amount) as cardjine FROM `".WEB_ADMIN_TABPOX.'selldetailmonth'.$this->getUid()."` A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id left join  ".WEB_ADMIN_TABPOX."marketingcard C ON A.item_id=C.marketingcard_id  WHERE  B.status in(1,4,5,6) and A.item_type =3 and   B.creattime between '".$bgdate."' and '".$enddate."' and C.inheritedid in(".$inrrscard['cardidlist'].") and A.agencyid =".$_SESSION['currentorgan']);
			// echo "select sum(A.amount) as cardjine FROM  ".WEB_ADMIN_TABPOX."sellcarddetail A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id left join  ".WEB_ADMIN_TABPOX."marketingcard C ON A.item_id=C.marketingcard_id  WHERE  B.status in(1,4,5,6) and A.item_type =3 and   B.creattime between '".$bgdate."' and '".$enddate."' and C.inheritedid in(".$inrrscard['cardidlist'].") and A.agencyid =".$_SESSION['currentorgan'];
			  $cardjine['cardjine']=$cardjine['cardjine']==0?'':$cardjine['cardjine'];
			 // $t -> set_var('totalconsultantmembernumber',$cardjine['cardjine']);
			   
		 }	
		  $t -> set_var('totalconsultantmembernumber',$totalconsultantmembernumber["'".$inrrscard['lookupcard_id']."'"]);
		$t -> parse('tcm','totalconsultantmemberlist',true);
	...
	$t -> parse('c','consultant',true);	
		...
		$t -> set_var('totaldingjin',sprintf ("%01.2f",$totaldingjin));
		$t -> set_var('totalyufu',sprintf ("%01.2f",$totalyufu));
		$t -> set_var('totalyufup',sprintf ("%01.2f",$totalyufup));
		$t -> set_var('totaladdupdingjin',sprintf ("%01.2f",$totaladdupdingjin));
		$t -> set_var('totaladdupyufup',sprintf ("%01.2f",$totaladdupyufup));
		$t -> set_var('totaladdupyufu',sprintf ("%01.2f",$totaladdupyufu));
		$t -> set_var('totaladdupconsultantproduce',sprintf ("%01.2f",$totaladdupconsultantproduce));
...
$beautyproduce=$this->beautyproduce($bgdate,$enddate);//美容师本月累计销售产品
...
while ($beautydata = $beauty -> FetchRow()) {//美容师业绩
 	...
 	if(!$consumenumber["'".$beautydata['employee_id']."'"]){
		...
		$t -> set_var('consumenumber',sprintf ("%01.2f",$consumenumber["'".$beautydata['employee_id']."'"]));
		...
		$t -> set_var('beautyitemtoday',sprintf ("%01.2f",$beautyitemtoday["'".$beautydata['employee_id']."'"]));//美容师 当天疗程卡
...
while ($inrrscard = $inrscard -> FetchRow()) {	 
	$t -> set_var('beautymembernumber',$cardyeji["'".$beautydata['employee_id']."'"]["'".$inrrscard['lookupcard_id']."'"]);
 	$totalbeautymembernumber["'".$inrrscard['lookupcard_id']."'"] =$totalbeautymembernumber["'".$inrrscard['lookupcard_id']."'"] + $cardyeji["'".$beautydata['employee_id']."'"]["'".$inrrscard['lookupcard_id']."'"];
	...
	$t -> set_var('beautyitem',sprintf ("%01.2f",$beautyitem["'".$beautydata['employee_id']."'"]));
...
/*	  if($inrrscard['type']=='1'){
		  //	$totalbeautymembernumber=$this -> dbObj -> GetRow("select count(*) as cardnumber FROM  ".WEB_ADMIN_TABPOX."sellcarddetail A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id left join  ".WEB_ADMIN_TABPOX."marketingcard C ON A.item_id=C.marketingcard_id  WHERE  B.status in(1,4,5,6) and A.item_type =3  and  B.creattime between '".$bgdate."' and '".$enddate."' and C.inheritedid in(".$inrrscard['cardidlist'].") and A.agencyid =".$_SESSION['currentorgan']);
	   ...
	   //$t -> set_var('totalbeautymembernumber',$totalbeautymembernumber["cardnumber"]);
 		...
 		//$totalbeautymembernumber=$this -> dbObj -> GetRow("select sum(A.amount) as cardjine FROM  ".WEB_ADMIN_TABPOX."sellcarddetail A INNER JOIN   ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id left join  ".WEB_ADMIN_TABPOX."marketingcard C ON A.item_id=C.marketingcard_id  WHERE  B.status in(1,4,5,6) and A.item_type =3 and  B.creattime between '".$bgdate."' and '".$enddate."' and C.inheritedid in(".$inrrscard['cardidlist'].") and A.agencyid =".$_SESSION['currentorgan']);
	   ...
	   $t -> set_var('totalbeautymembernumber',$totalbeautymembernumber["'".$inrrscard['lookupcard_id']."'"]);
	  	$t -> parse('tbm','totalbeautymemberlist',true);	
...
/*
 ...
 function consultanttodayyeji($bgtodate,$endtodate,$cmldiscount){//顾问当天业绩
	...
	$consultantperformancetoday1=$this->perObj->main2($bgtodate,$endtodate);
/*	
	$sqlstr1="select * from  ".WEB_ADMIN_TABPOX."selldetail";
	$sqlstr2="select * from  ".WEB_ADMIN_TABPOX."sellcarddetail ";
	$sqlstr3="select * from  ".WEB_ADMIN_TABPOX."sellconsumedetail  ";
...
*/ 
 return $consultantperformancetoday1;	
  ...
  function consultantaddupyeji($bgdate,$enddate){//顾问累计业绩
	$this->perObj=new performancehistory();

	$consultantperformance=$this->perObj->main2($bgdate,$enddate);
 
 	return $consultantperformance;	
 }
 function shougongfei($bgdate,$enddate){
	$this->perObj1=new performancehistory();
	 $beautymanual1=$this->perObj1->main2($bgdate,$enddate);
...
/*	$sqlstr2="select * from  ".WEB_ADMIN_TABPOX."sellcarddetail ";
	...
	$sqlstr5="select * from  ".WEB_ADMIN_TABPOX."sellotherdetail ";*/
	$sql=$sqlstr1;
 	$inrstoday1=$this -> dbObj -> Execute("select * FROM  `".WEB_ADMIN_TABPOX.'selldetail'.$this->getUid()."` A  WHERE A.item_type =1 AND  A.agencyid =".$_SESSION['currentorgan']); 	
	while ($inrrstoday1 = $inrstoday1 -> FetchRow()) {
//=====顾问=============================		 
		if($inrrstoday1["employee_id"]<>'' && $inrrstoday1["employee_id"]<>0){
		$employee_id=explode(";",$inrrstoday1["employee_id"]);
 ...
 return $consultantproducetoday1;	
  ...
  function consultantproduce($bgdate,$enddate){//顾问累计销售业绩
...
/*	$sqlstr2="select * from  ".WEB_ADMIN_TABPOX."sellcarddetail ";
	...
	$sqlstr5="select * from  ".WEB_ADMIN_TABPOX."sellotherdetail ";*/
	$sql=$sqlstr1;
 	$inrs=$this -> dbObj -> Execute("select A.* FROM  `".WEB_ADMIN_TABPOX.'selldetail'.$this->getUid()."` A INNER JOIN  ".WEB_ADMIN_TABPOX."sell B  ON A.sell_id=B.sell_id WHERE  A.item_type=1  AND B.status in(1,4,5,6)  and   A.agencyid =".$_SESSION['currentorgan']); 	
	// echo "select * FROM  (".$sql.") A INNER JOIN  ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id WHERE  B.creattime between '".$bgdate."' and '".$enddate."' AND A.item_type=1 AND B.status in(1,4,5,6)   and    A.agencyid =".$_SESSION['currentorgan'];
	while ($inrrs = $inrs -> FetchRow()) {
    //=====顾问=============================		 
		...
		$consultantproduce["'".$employee_id[$i]."'"]=$consultantproduce["'".$employee_id[$i]."'"]+$inrrs['amount']/count($employee_id);
		}

		}
	}
 return $consultantproduce;	
 }
...
function consultantitemtoday($bgtodate,$endtodate){//顾问当天 疗程卡(项目卡itemcard)业绩
...
/*	$sqlstr2="select * from  ".WEB_ADMIN_TABPOX."sellcarddetail ";
	...
	$sqlstr5="select * from  ".WEB_ADMIN_TABPOX."sellotherdetail ";*/
	$sql=$sqlstr1;
 	$inrstoday1=$this -> dbObj -> Execute("select * FROM  `".WEB_ADMIN_TABPOX.'selldetail'.$this->getUid()."` A  WHERE A.item_type=3 AND  A.agencyid =".$_SESSION['currentorgan']); 
...
function beautyconsumenumbertoday($sellidtoday){//美容师当天 项次 业绩
	...
	$sql=$sqlstr3." union ".$sqlstr4;
 	$inrstoday1=$this -> dbObj -> Execute("select * FROM  (".$sql.") A  WHERE A.sell_id in (".$sellidtoday.") AND A.discount>0 AND A.item_type in(0,2,4) AND  A.agencyid =".$_SESSION['currentorgan']); 
	while ($inrrstoday1 = $inrstoday1 -> FetchRow()) {
		if($inrrstoday1["beauty_id"]<>'' && $inrrstoday1["beauty_id"]<>0){
		$beauty_id=explode(";",$inrrstoday1["beauty_id"]);
		for ($i=0;$i<count($beauty_id);$i++){	 
		 
		$beautyconsumenumbertoday["'".$beauty_id[$i]."'"]=$beautyconsumenumbertoday["'".$beauty_id[$i]."'"]+1*$inrrstoday1["number"]/count($beauty_id);
		 }
 ...
 function beautyproducetoday($sellidtoday){//美容师当天 产品销售
	...
	while ($inrrstoday1 = $inrstoday1 -> FetchRow()) {
		...
		$beautyproducetoday["'".$beauty_id[$i]."'"]=$beautyproducetoday["'".$beauty_id[$i]."'"]+$inrrstoday1['value']*$inrrstoday1['number']/count($beauty_id);
		 }
 ...
 function beautyproduce($bgdate,$enddate){//美容师本月累计业绩
	...
	$sql=$sqlstr3 ;
 	//$inrstoday1=$this -> dbObj -> Execute("select * FROM  (".$sql.") A  WHERE A.sell_id in (".$sellidtoday.") AND A.discount>0 AND A.item_type in(1) AND  A.agencyid =".$_SESSION['currentorgan']); 
 	$inrs=$this -> dbObj -> Execute("select * FROM  (".$sql.") A INNER JOIN  ".WEB_ADMIN_TABPOX."sell B  on A.sell_id=B.sell_id WHERE  B.creattime between '".$bgdate."' and '".$enddate."' AND A.item_type=1 and   A.agencyid =".$_SESSION['currentorgan']); 	
	 
	while ($inrrs = $inrs -> FetchRow()) {
		if($inrrs["beauty_id"]<>'' && $inrrs["beauty_id"]<>0){
		$beauty_id=explode(";",$inrrs["beauty_id"]);
		 
		for ($i=0;$i<count($beauty_id);$i++){	 
		
		$beautyproduce["'".$beauty_id[$i]."'"]=$beautyproduce["'".$beauty_id[$i]."'"]+$inrrs['amount']/count($beauty_id);
...
function newyufunumber($bgdate,$enddate){//预定散客人数
 
	$sellidtoday=$sellidtoday==''?0:$sellidtoday;
 
 	$newyufu=$this -> dbObj -> Execute("select count(*) as newyufunumber FROM ".WEB_ADMIN_TABPOX."sellotherdetail A LEFT JOIN  ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id   WHERE  B.creattime between '".$bgdate."' and '".$enddate."'  AND A.item_type=5 AND A.item_id=0  and (B.membercard_no='' or B.membercard_no is NULL) AND  B.agencyid =".$_SESSION['currentorgan'].' GROUP BY B.customer_id '); 
...
function deal($bgdate,$enddate){//成交数 
	$sellid=$sellid==''?0:$sellid;
 	$inrs1=$this -> dbObj -> Execute("select * FROM ".WEB_ADMIN_TABPOX."sellcarddetail  A LEFT JOIN  ".WEB_ADMIN_TABPOX."sell B ON A.sell_id=B.sell_id   WHERE  B.creattime between '".$bgdate."' and '".$enddate."'  and A.sell_id in (".$sellid.") AND A.item_type=3   AND A.discount>0 and A.value>0 AND B.agencyid =".$_SESSION['currentorgan'].' GROUP BY B.customer_id'); 
 	$deal=0;	
	while ($inrrs1 = $inrs1 -> FetchRow()) {	 
...
function totalbeautymanual($bgdate,$enddate,$cmldiscount){//本月手工
	 ...
	 $sql5="select * from ".WEB_ADMIN_TABPOX."sellotherdetail where agencyid =".$agencyid;
      $sql7="select * from ".WEB_ADMIN_TABPOX."sellotherdetail where agencyid =".$agencyid." and item_type=5 and item_id=0 ";//预收款

	 

$totalbeautymanual=0;
//$totalbeautymanual=$this->dbObj->GetOne("select  sum(A.cmlaccount) as cmlaccount from (".$sql4." union ".$sql1." union ".$sql7.") A INNER JOIN ".WEB_ADMIN_TABPOX."sell B ON A.sell_id= B.sell_id where  B.creattime between '".$bgdate."' and '".$enddate."' and B.status in(1,4,5,6) and A.agencyid =".$agencyid);//服务类金额+预收款，预付计算为手工费！
$totalbeautymanual=$this->dbObj->GetOne("select  sum(A.cmlaccount) as cmlaccount from `".WEB_ADMIN_TABPOX.'selldetailmonth'.$this->getUid()."` A INNER JOIN ".WEB_ADMIN_TABPOX."sell B ON A.sell_id= B.sell_id where  A.item_type in (0,2,4) or (A.item_type in (5) and A.item_id=0) and B.status in(1,4,5,6) and A.agencyid =".$agencyid);
//echo "select  sum(A.cmlaccount) as cmlaccount from (".$sql4." union ".$sql1." union ".$sql7.") A INNER JOIN ".WEB_ADMIN_TABPOX."sell B ON A.sell_id= B.sell_id where  B.creattime between '".$bgdate."' and '".$enddate."' and B.status in(1,4,5,6) and A.agencyid =".$agencyid;
//echo "select  sum(A.cmlaccount) as cmlaccount from (".$sql4." union ".$sql1." union ".$sql7.") A INNER JOIN ".WEB_ADMIN_TABPOX."sell B ON A.sell_id= B.sell_id where  B.creattime between '".$bgdate."' and '".$enddate."' and A.agencyid =".$agencyid;
//$this->perObj2=new performancehistory();


//$totalbeautymanual=$this->perObj2->main2($bgdate,$enddtae);
//$sellid=explode(",",$sellid);
//for ($i=0;$i<count($sellid);$i++)	{
//$totalbeautymanual=$totalbeautymanual+$this->perObj2->main1($bgdate,$enddtae);
//}
/*
	$sellid=$sellid==''?0:$sellid;
	$sqlstr3="select * from  ".WEB_ADMIN_TABPOX."sellconsumedetail  ";
 ...
 function totalown($bgdate,$enddate){//本月欠款
 	$totalown=0;
	$sellid=$sellid==''?0:$sellid;
	 
 	$inrs=$this -> dbObj -> Execute("select sellown,payable1,realpay FROM   ".WEB_ADMIN_TABPOX."sell WHERE  creattime between '".$bgdate."' and '".$enddate."' and status in (1,4,5,6)  AND  agencyid =".$_SESSION['currentorgan']); 
 
	while ($inrrs = $inrs -> FetchRow()) {		 
		//$totalown=$totalown+$inrrs["payable1"]-$inrrs["realpay"];
		$totalown=$totalown+$inrrs["sellown"];
		}
	 
 
 return sprintf("%01.2f",$totalown);	
 } 
...
function yejisellid($sellid){//算出sellid平均业绩
	...
	$sqlstr5="select * from  ".WEB_ADMIN_TABPOX."sellotherdetail ";
	$sql=$sqlstr1." union ".$sqlstr2." union ".$sqlstr3." union ".$sqlstr4." union ".$sqlstr5;
 	$inrs=$this -> dbObj -> Execute("select * FROM  (".$sql.") A  WHERE A.sell_id in (".$sellid.") AND  A.agencyid =".$_SESSION['currentorgan']);
	$i=$inrs-> RecordCount();
	while ($inrrs = $inrs -> FetchRow()) {		 
 ...
 function cardyeji($bgdate,$enddate){//算出sellid平均业绩
 $inrscard=$this -> dbObj -> Execute("select * FROM   ".WEB_ADMIN_TABPOX."lookupcard     order by orderid");
 
	while ($inrrscard = $inrscard -> FetchRow()) {	//循环统计项
	  if($inrrscard['type']=='1'){//如果是统计张数
  
			$inrs=$this -> dbObj -> Execute("select sum(number) as cardnumber,employee_id FROM  ".WEB_ADMIN_TABPOX."cardyeji A INNER JOIN   ".WEB_ADMIN_TABPOX."marketingcard C ON A.cardid=C.marketingcard_id  WHERE  A.status in(0,3) and A.item_type=3  and  A.payalldate between '".date('Y-m-d',strtotime($bgdate))."' and '".date('Y-m-d',strtotime($enddate))."' and C.inheritedid in(".$inrrscard['cardidlist'].") group by employee_id"); 
 
			while ($inrrs= $inrs-> FetchRow()) {//循环员工	
			$cardyeji["'".$inrrs['employee_id']."'"]["'".$inrrscard['lookupcard_id']."'"]=$inrrs['cardnumber'] ;//某员工某卡业绩
			 
			}
		  
	  }else if($inrrscard['type']=='0'){//统计金额
		 	 $inrs=$this -> dbObj -> Execute("select sum(A.value) as cardvalue,employee_id FROM  ".WEB_ADMIN_TABPOX."cardyeji A INNER JOIN   ".WEB_ADMIN_TABPOX."marketingcard C ON A.cardid=C.marketingcard_id  WHERE  A.status in(0,3) and A.item_type=3  and  A.payalldate between '".date('Y-m-d',strtotime($bgdate))."' and '".date('Y-m-d',strtotime($enddate))."' and C.inheritedid in(".$inrrscard['cardidlist'].") group by employee_id"); 
		   
	  		while ($inrrs= $inrs-> FetchRow()) {	
			$cardyeji["'".$inrrs['employee_id']."'"]["'".$inrrscard['lookupcard_id']."'"]=$inrrs['cardvalue'];
			}
	  }
 ...
 function totalcardyeji($bgdate,$enddate){//算出sellid平均业绩
 $inrscard=$this -> dbObj -> Execute("select * FROM   ".WEB_ADMIN_TABPOX."lookupcard     order by orderid");
 
	while ($inrrscard = $inrscard -> FetchRow()) {	//循环统计项
	  if($inrrscard['type']=='1'){//如果是统计张数
  
			$inrs=$this -> dbObj -> Execute("select sum(number) as cardnumber,employee_id FROM  ".WEB_ADMIN_TABPOX."cardyeji A INNER JOIN   ".WEB_ADMIN_TABPOX."marketingcard C ON A.cardid=C.marketingcard_id  WHERE  A.status in(0,3) and A.item_type=3  and  A.payalldate between '".date('Y-m-d',strtotime($bgdate))."' and '".date('Y-m-d',strtotime($enddate))."' and C.inheritedid in(".$inrrscard['cardidlist'].") and A.agencyid =".$_SESSION['currentorgan']); 
 
			 
			$cardyeji["'".$inrrscard['lookupcard_id']."'"]=$inrrs['cardnumber'] ;//某员工某卡业绩
			 
		  
	  }else if($inrrscard['type']=='0'){//统计金额
		 	 $inrs=$this -> dbObj -> Execute("select sum(A.value) as cardvalue,employee_id FROM  ".WEB_ADMIN_TABPOX."cardyeji A INNER JOIN   ".WEB_ADMIN_TABPOX."marketingcard C ON A.cardid=C.marketingcard_id  WHERE  A.status in(0,3) and A.item_type=3  and  A.payalldate between '".date('Y-m-d',strtotime($bgdate))."' and '".date('Y-m-d',strtotime($enddate))."' and C.inheritedid in(".$inrrscard['cardidlist'].") and A.agencyid =".$_SESSION['currentorgan']); 
		   
	  		 
			$cardyeji["'".$inrrscard['lookupcard_id']."'"]=$inrrs['cardvalue'];
