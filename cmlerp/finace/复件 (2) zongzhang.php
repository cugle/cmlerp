...
//分页		
			...
			$sqlbg='select sum(A.lend) as bglend ,sum(A.loan) as bgloan from '.WEB_ADMIN_TABPOX.'transfervoucherhistory A INNER JOIN '.WEB_ADMIN_TABPOX.'accounttitle B ON A.accounttitle_id=B.accounttitle_id   where  A.agencyid ='.$_SESSION["currentorgan"].' and '.$condition1 ;
...
/*			if(strpos('0,1,4',$nostr)){
			...
			$bgbalance=($bgdata['bglend']-$bgdata['bgloan']);
		...
		$t -> set_var('bglend',$bgdata['bglend']);
		$t -> set_var('bgloan',$bgdata['bgloan']);
		$totaldata = &$this -> dbObj -> GetRow($sql1);	
		$t -> set_var('totallend',$totaldata['totallend']+$bgdata['bglend']);
		$t -> set_var('totalloan',$totaldata['totalloan']+$bgdata['bgloan']);
		$totalbalance=$bgbalance+$balance;
