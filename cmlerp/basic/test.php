...
function disp(){
	...
	function disp1(){
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'%"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			$inrs -> Close();	
			$t -> set_var('t');
			$inrs1 = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'account');
			while ($inrrs1 = &$inrs1 -> FetchRow()) {
				$t -> set_var($inrrs1);
			
			   	$t -> set_var('delete',$this -> getDelStr('',$inrrs1['account_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs1['account_id']));
				$t -> parse('t','test',true);
			}
			$inrs1 -> Close();		
