...
//分页		
			...
			$sql='select * from '.WEB_ADMIN_TABPOX.'procatalog  r INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on r.category_id  =f.category_id  where f.roomgroup_name like '%".$keywords."%' and  r.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'procatalog   where  agencyid ='.$_SESSION["currentorgan"];
			 
			}
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  category_id DESC  LIMIT ".$offset." , ".$psize);
			...
			//echo $sql." ORDER BY  category_id DESC  LIMIT ".$offset." , ".$psize;
			
			$t -> set_var('ml');
			
			//$inrs = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'procatalog  where agencyid ='.$_SESSION["currentorgan"]);
			 //echo 'select * from '.WEB_ADMIN_TABPOX.'roomgroup  where agencyid ='.$_SESSION["currentorgan"];
	     	while ($inrrs = &$inrs -> FetchRow()) {
				$t -> set_var($inrrs);
			   		$t -> set_var('delete',$this -> getDelStr('',$inrrs['category_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['category_id']));
	...
	function goDispAppend(){
		...
		$t = new Template('../template/basic');
		$t -> set_file('f','procatalog_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		...
		$t -> set_var('userid',$this->getUid());		
			...
			$t -> set_var($this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'procatalog WHERE category_id = '.$updid));			
			$t -> set_var('error',"");
			$t -> set_var('showeditdiv',"");
			$t -> set_var('updid',$updid);
			$t -> set_var('action','upd');
			$t -> set_var('actionName','修改');
			$inrs = &$this -> dbObj -> Execute('SELECT * FROM '.WEB_ADMIN_TABPOX.'procatalog WHERE category_id = '.$updid);	
			while ($inrrs = &$inrs -> FetchRow()) {
				$t -> set_var($inrrs);
				if($inrrs['nodiscount']==1){
				$t -> set_var('nodiscount',"checked");
				}else{
				$t -> set_var('nodiscount',"");
				}
			}
	...
	function goDelete(){
        ...
        $this -> dbObj -> Execute('DELETE FROM '.WEB_ADMIN_TABPOX.'procatalog WHERE category_id in('.$delid.')');
		$info='删除';
		//echo 'DELETE FROM '.WEB_ADMIN_TABPOX.'procatalog WHERE category_id in('.$delid.')';
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '增加';	
			if($_POST["nodiscount"]=='on'){$_POST["nodiscount"]=1;}else{$_POST["nodiscount"]=0;}
			//echo "INSERT INTO `".WEB_ADMIN_TABPOX."procatalog` (`category_name` ,`memo` ,`agencyid` )VALUES ( '".$_POST["category_name"]."','".$_POST["memo"]."','".$_SESSION["currentorgan"]."')";
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."procatalog` (`category_name` ,nodiscount,`memo` ,`agencyid` )VALUES ( '".$_POST["category_name"]."','".$_POST["nodiscount"]."','".$_POST["memo"]."','".$_SESSION["currentorgan"]."')");
			...
			$id = $_POST[MODIFY.'id'];
			if($_POST["nodiscount"]=='on'){$_POST["nodiscount"]=1;}else{$_POST["nodiscount"]=0;}
			$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."procatalog SET category_name='".$_POST["category_name"]."',nodiscount='".$_POST["nodiscount"]."', memo='".$_POST["memo"]."' WHERE category_id =".$id);
//echo 'UPDATE '.WEB_ADMIN_TABPOX."procatalog SET category_name='".$_POST["category_name"]."', memo='".$_POST["memo"]."' WHERE category_id =".$id;
		}
//$this -> quit($info.'成功！');
		if(mysql_affected_rows())
		$this -> quit($info.'成功！');
	...
	function quit($info){
		exit("<script>alert('$info');location.href='procatalog.php';</script>");
	}
