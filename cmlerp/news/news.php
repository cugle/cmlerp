...
class Pagecustomer extends admin {
    ...
    function view(){
		...
		$t -> set_var($data);
			...
			$fag=$this -> dbObj -> GetRow('select * from '.WEB_ADMIN_TABPOX.'usernews where user_id='.$this->getUid()." and news_id=".$id);
			if(!$fag){
	...
	function disp(){
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'%"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'news  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;

			}else if($ftable<>''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'news a INNER JOIN '.WEB_ADMIN_TABPOX."$ftable f on a.categoryid =f.category_id  where f.category_name like '%".$keywords."%' and  a.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'news  where  agencyid ='.$_SESSION["currentorgan"];
			 
			}
			 
			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  news_id DESC  LIMIT ".$offset." , ".$psize);
			
			$result = &$this -> dbObj -> Execute($sql);		
			$count=$result->RecordCount();
			$t -> set_var('pagelist',$this -> page("?category=".$category."&keywords=".urlencode($keywords)."&ftable=".$ftable,$count,$psize,$pageid));		

			//$inrs = &$this -> dbObj -> Execute('select * from '.WEB_ADMIN_TABPOX.'news  where agencyid ='.$_SESSION["currentorgan"]);
			//echo 'select * from '.WEB_ADMIN_TABPOX.'news  where agencyid ='.$_SESSION["currentorgan"];
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				$t -> set_var('employee_name',$this->dbObj->GetOne('SELECT employee_name FROM '.WEB_ADMIN_TABPOX.'employee WHERE employee_id = '.$inrrs['employee_id']));
			   	$t -> set_var('delete',$this -> getDelStr('',$inrrs['news_id']));
		        $t -> set_var('edit',$this -> getupdStr('',$inrrs['news_id']));
				$t -> set_var('view','<a href="?action=view&updid='.$inrrs['news_id'].'" >查看</a>');
				$t -> parse('a','news',true);
			}
			$inrs -> Close();	
	...
	function goDispAppend(){
		...
		if($this -> isAppend){
			...
			$t -> set_var('error',"");	
			$t -> set_var('showeditdiv',"");		
			$t -> set_var('news_title',"");

			$t -> set_var('createtime',"");
			$t -> set_var('news_content',"");
			$t -> set_var('createtime',date("Y-m-d H:i:s"));
			$t -> set_var('userid',$this->getUid());
			$employee=$this -> dbObj -> GetRow('select * from '.WEB_ADMIN_TABPOX.'user A INNER JOIN '.WEB_ADMIN_TABPOX.'employee B ON A.employee_id=B.employee_id  where A.userid = '.$this->getUid());
			$t -> set_var('employee_id',$employee['employee_id']);
			$t -> set_var('employee_name',$employee['employee_name']);
			$t -> set_var('newstype',$this->selectlist('newstype','newstype_id','newstype_name'));
		}else{
			$updid = $_GET[MODIFY.'id'] + 0 ;	
			$data=$this->dbObj->GetRow('SELECT * FROM '.WEB_ADMIN_TABPOX.'news WHERE news_id = '.$updid);
			$t -> set_var($data);
			...
			while ($inrrs = &$inrs -> FetchRow()) {
				...
				$t -> set_var($inrrs);
				$t -> set_var('newstype',$this->selectlist('newstype','newstype_id','newstype_name',$inrrs['type']));
	...
	function goAppend(){
		...
		if($this -> isAppend){
			$info = '增加';	
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."news` ( `news_title`, `news_content`, `type`,`agencyid`)VALUES ( '".$_POST["news_title"]."', '".$_POST["news_content"]."',  '".$_POST["type"]."','".$_SESSION["currentorgan"]."')");
			...
			$this -> dbObj -> Execute('UPDATE '.WEB_ADMIN_TABPOX."news SET news_title='".$_POST["news_title"]."', news_content='".$_POST["news_content"]."', type='".$_POST["type"]."' WHERE news_id =".$id);
