...
function turnback(){
	 $this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."lingyong` SET `status`=0  WHERE lingyong_id =".$_GET['id']);//5已经审核4表示被反冲3反冲2未审核1未提交
	 exit("<script>alert('撤回成功');window.location.href='lingyonghistory.php';</script>");	
	}		
function audit(){
	$employee_id=$this -> dbObj -> GetOne('select employee_id from '.WEB_ADMIN_TABPOX.'user   where userid  ='.$this -> getUid());
	 
	 $this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."lingyong` SET `status`=5 ,auditor_id=".$employee_id." WHERE lingyong_id =".$_GET['id']);//5已经审核4表示被反冲3反冲2未审核1未提交
	 exit("<script>alert('审核成功');window.location.href='lingyonghistory.php';</script>");	
	}	
function recoil(){
	$this -> dbObj -> Execute("START TRANSACTION");//事务开始。
	$this->prodaybooksObj=new prodaybooks();//商品流水账
	
	$condition =' lingyong_id='.$_GET['id'];
	$sql='select * from '.WEB_ADMIN_TABPOX.'lingyong  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;	
	
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  lingyong_id DESC  LIMIT 0 ,1");
		     	while ($inrrs = &$inrs -> FetchRow()) {

			//$man=$this -> dbObj -> getone('select username from '.WEB_ADMIN_TABPOX.'user where userid = '.$this->getUid());
			//插入进货记录
			$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."lingyong` (`lingyong_no`,`warehouse_id`,`employee_id`,`acount`,`creattime`,`memo`,`lingyong_time`,`status`,`man`,`agencyid`) VALUES ('" .$inrrs["lingyong_no"]."', '".$inrrs["warehouse_id"]."', '".$inrrs["employee_id"]."', '" .(-$inrrs["acount"])."','".$inrrs['creattime']."', '".$inrrs["memo"]."', '".$inrrs['lingyong_time']."','3','".$inrrs['man']."','".$inrrs["agencyid"]."')");
			 // echo "INSERT INTO `".WEB_ADMIN_TABPOX."lingyong` (`lingyong_no`,`warehouse_id`,`employee_id`,`acount`,`creattime`,`memo`,`lingyong_time`,`status`,`man`,`agencyid`) VALUES ('" .$inrrs["lingyong_no"]."', '".$inrrs["warehouse_id"]."', '".$inrrs["employee_id"]."', '" .(-$inrrs["acount"])."','".$inrrs['creattime']."', '".$inrrs["memo"]."', '".$inrrs['lingyong_time']."','3','".$inrrs['man']."','".$inrrs["agencyid"]."')";
			$id = $this -> dbObj -> Insert_ID();
		$warehouse_id=$inrrs["warehouse_id"];	
		$sqlstr='select * from '.WEB_ADMIN_TABPOX.'lingyongdetail  where  agencyid ='.$_SESSION["currentorgan"].' and lingyong_id='.$_GET['id'];	
		
		//插入进货明细
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."lingyongdetail` (lingyong_id,produce_id,number,lingyongprice,totalacount,memo,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".(-$inrrs1["number"])."','".$inrrs1["lingyongprice"]."','".(-$inrrs1["totalacount"])."','".$inrrs1["memo"]."','".$inrrs1["agencyid"]."')");
				 	// echo "INSERT INTO `".WEB_ADMIN_TABPOX."lingyongdetail` (lingyong_id,produce_id,number,lingyongprice,totalacount,memo,agencyid) VALUES ('".$id."','".$inrrs1["produce_id"]."','".(-$inrrs1["number"])."','".$inrrs1["lingyongprice"]."','".$inrrs1["totalacount"]."','".$inrrs1["memo"]."','".$inrrs1["agencyid"]."')";
				 $res3=$this->prodaybooksObj->main($inrrs1['produce_id'],$inrrs["warehouse_id"],$inrrs1['number'],$inrrs1['totalacount'],7,$id,$inrrs['man'],"被反冲领用单; ".$inrrs['memo'],$_SESSION["currentorgan"]);//商品流水账		
					}
			
				}
	   $this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."lingyong` SET `status`=2  WHERE lingyong_id =".$_GET['id']);//4表示被反冲3反冲2未审核1未提交	
	  	//更新库存
		$this->stockObj=new stock();
		
		$res1=$this->stockObj->lingyongtostock($id,$warehouse_id,$_SESSION["currentorgan"]); 	   
	   
		if($res1&&$res3){
		
		$this -> dbObj -> Execute("COMMIT");
		  exit("<script>alert('反冲成功');window.location.href='lingyonghistory.php';</script>");	
		}else{
		$this -> dbObj -> Execute("ROLLBACK");
		 exit("<script>alert('发生错误，审核失败，数据已经回滚。');window.location.href='lingyonghistory.php';</script>");
		}
	}
function  print1(){
		//定义模板
		$t = new Template('../template/stock');
		$t -> set_file('f','lingyonghistory_detail_print.html');
		$t->unknowns = "move";
        ...
        //$t -> set_block('f','consumables','cs');	
	       ...
	       $status_name=array("未完成","未提交","<font color=red>红字冲销</font>","<font color=red>被红字冲销</font>","<font color=green>已提交</font>","已审核");	
		...
		$acount = &$this -> dbObj -> GetOne('select count(*) from '.WEB_ADMIN_TABPOX.'lingyongdetail  where lingyong_id  ='.$updid);
...
function  viewbill(){
		//定义模板
		$t = new Template('../template/stock');
		$t -> set_file('f','lingyonghistory_detail_viewbill.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
...
function disp(){
		//定义模板
		$t = new Template('../template/stock');
		$t -> set_file('f','lingyonghistory.html');
		$t->unknowns = "remove";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		$t -> set_block('f','mainlist','ml');		
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'lingyong where  agencyid ='.$_SESSION["currentorgan"].' and status>1 and '.$condition ;

			}else if($ftable<>''){
			$sql="select * from ".WEB_ADMIN_TABPOX."lingyong s INNER JOIN ".WEB_ADMIN_TABPOX."$ftable f on s.".$ftable."_id =f.".$ftable."_id  where f.".$category." like '%".$keywords."%'  and s.status>1 and  s.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'lingyong WHERE  agencyid ='.$_SESSION["currentorgan"].' and status>1 ';
			 
			}

			$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  lingyong_no desc  LIMIT ".$offset." , ".$psize);
			...
			$t -> set_var('statuslist',$liststr);
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				$t -> set_var('view','<a href="?action=upd&updid='.$inrrs['lingyong_id'].'">查看</a>');
				$t -> set_var('print','<a href="?action=print&id='.$inrrs['lingyong_id'].'" target="_blank">打印</a>');				
				$t -> parse('ml','mainlist',true);
			}
			$inrs -> Close();	
	...
	function goDispAppend(){
		...
		$t = new Template('../template/stock');
		$t -> set_file('f','lingyonghistory_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
...
if($_POST['cproduce_id']!=''&&$_POST['cnumber']!=''){			
			...
			echo "INSERT INTO `".WEB_ADMIN_TABPOX."lingyongdetail` (lingyong_id,produce_id,number,memo,agencyid) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cmemo"]."','".$_SESSION["currentorgan"]."')";
			}
		...
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."lingyong` SET `acount` =".$acount." where lingyong_id=".$id) ;	
			...
			exit("<script>alert('增加成功');window.location.href='lingyong.php?action=upd&updid=".$id."';</script>");
		}else{
			$info = '修改';
			$id = $_POST[MODIFY.'id'];
			$man=$this -> dbObj -> getone('select username from '.WEB_ADMIN_TABPOX.'user where userid = '.$this->getUid());
 ...
 if($_POST["con_act"]=='upd'){
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."lingyongdetail` SET `produce_id` = '".$_POST["cproduce_id"]."',`number` = '".$_POST["cnumber"]."',`memo` = '".$_POST["cmemo"]."'  WHERE lingyongdetail_id  =".$_POST['proupdid']);	
		...
		if($_POST['issave']=='1'){
		$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."lingyong` SET `status`=1  WHERE lingyong_id =".$id);//0未提交，1已提交2已收部分3全部收完
