...
//修改单据状态。
		...
		$auditor_id=$this -> dbObj -> GetOne("select employee_id from`".WEB_ADMIN_TABPOX."user`  WHERE `userid`=".$this->getUid());
		$res= $this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."moveproduce` SET `status`=5 ,auditor_id=".$auditor_id." WHERE moveproduce_id =".$_GET['id']);	
		if($res){
		$this -> dbObj -> Execute("COMMIT");
			 exit("<script>alert('审核成功');window.location.href='moveproducehistory.php';</script>");	
		}else{
		$this -> dbObj -> Execute("ROLLBACK");
		 exit("<script>alert('发生错误，审核失败，数据已经回滚。');history.go(-1);</script>");
		} 
...
$condition =' moveproduce_id='.$_GET['id'];
	$updid=$_GET['id'];
	$sql='select * from '.WEB_ADMIN_TABPOX.'moveproduce  where  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;	
	$auditor_id=$this -> dbObj -> GetOne("select employee_id from`".WEB_ADMIN_TABPOX."user`  WHERE `userid`=".$this->getUid());
	$inrs = &$this -> dbObj -> Execute($sql." ORDER BY  moveproduce_id DESC  LIMIT 0 ,1");
		     	while ($inrrs = &$inrs -> FetchRow()) {

			//$man=$this -> dbObj -> getone('select username from '.WEB_ADMIN_TABPOX.'user where userid = '.$this->getUid());
			//插入调拨记录
			$res1=$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."moveproduce` ( `moveproduce_no`, `fromwarehouse`,`towarehouse`, `date`, `man`, `employee_id`,  `memo`,auditor_id ,status, `agencyid`) VALUES ('" .$inrrs["moveproduce_no"]."', '".$inrrs["fromwarehouse"]."','".$inrrs["towarehouse"]."', '".$inrrs["date"]."','".$inrrs["man"]."', '" .$inrrs["employee_id"]."','".$inrrs["memo"]."','".$auditor_id."',2,'".$inrrs["agencyid"]."')");
			//echo "INSERT INTO `".WEB_ADMIN_TABPOX."moveproduce` ( `moveproduce_no`, `fromwarehouse`,`towarehouse`, `date`, `man`, `employee_id`,  `memo`,auditor_id ,status, `agencyid`) VALUES ('" .$inrrs["moveproduce_no"]."', '".$inrrs["fromwarehouse"]."','".$inrrs["towarehouse"]."', '".$inrrs["date"]."','".$inrrs["man"]."', '" .$inrrs["employee_id"]."','".$inrrs["memo"]."','".$auditor_id."',2,'".$inrrs["agencyid"]."')";
			//$res1=$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."moveproduce` (`moveproduce_no`,`frowarehouse`, `towarehouse`,`employee_id`,`acount`,`creattime`,`memo`,`purchase_time`,`status`,`man`,`agencyid`) VALUES ('" .$inrrs["purchase_no"]."', '".$inrrs["order_id"]."','".$inrrs["warehouse_id"]."', '" .$inrrs["suppliers_id"]."','".$inrrs["employee_id"]."', '" .(-$inrrs["acount"])."','".$inrrs['creattime']."', '".$inrrs["memo"]."', '".$inrrs['purchase_time']."','2','".$inrrs['man']."','".$inrrs["agencyid"]."')");
			...
			$fromwarehouse=$inrrs["fromwarehouse"];
			$towarehouse=$inrrs["towarehouse"];
			$man=$inrrs["man"];
			$id = $this -> dbObj -> Insert_ID();
			
		$sqlstr='select * from '.WEB_ADMIN_TABPOX.'moveproducedetail  where  agencyid ='.$_SESSION["currentorgan"].' and moveproduce_id='.$_GET['id'];	
		
		//插入进货明细
		$inrs1 = &$this -> dbObj -> Execute($sqlstr);
				while ($inrrs1 = &$inrs1 -> FetchRow()) {
					$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."moveproducedetail` (`moveproduce_id`, `produce_id`, `number`, `price`,`moveproduceprice`, `account`, `agencyid`) VALUES ('".$id."','".$inrrs1["produce_id"]."','".-$inrrs1["number"]."','".-$inrrs1["price"]."','".$inrrs1["moveproduceprice"]."','".-$inrrs1["account"]."','".$inrrs1["agencyid"]."')");	
		...
		/*$res3=$this->prodaybooksObj->main($inrrs1['produce_id'],$fromwarehouse,-$inrrs1["number"],-$inrrs1["totalacount"],4,$id,$inrrs['man'],"反冲的调拨; ".$inrrs['memo'],$_SESSION["currentorgan"]);//商品流水账	
		$res33=$this->prodaybooksObj->main($inrrs1['produce_id'],$warehouse_id,$inrrs1["number"],$inrrs1["totalacount"],4,$id,$inrrs['man'],"反冲的调拨; ".$inrrs['memo'],$_SESSION["currentorgan"]);//商品流水账	*/
		
		$res2=$this->stockObj->main($inrrs1["produce_id"],-$inrrs1['number'],-$inrrs1['account'],$towarehouse,$inrrs1["agencyid"]);//增加目的库存
		
		$res3=$this->prodaybooksObj->main($inrrs1['produce_id'],$towarehouse,-$inrrs1['number'],-$inrrs1['account'],9,$id,$man,"反冲的调拨单; ".$inrrs['memo'],$inrrs1["agencyid"]);//商品流水账
		
		 $res11=$this->stockObj->main($inrrs1["produce_id"],$inrrs1['number'],$inrrs1['account'],$fromwarehouse,$inrrs1['agencyid']);//减少源仓库库存
		 $res33=$this->prodaybooksObj->main($inrrs1['produce_id'],$fromwarehouse,$inrrs1['number'],$inrrs1['account'],9,$id,$$inrrs['man'],"反冲的调拨单; ".$inrrs['memo'],$inrrs1["agencyid"]);//商品流水账
					}
			
				}
	    $st1=$this -> dbObj -> Execute("UPDATE `".WEB_ADMIN_TABPOX."moveproduce` SET `status`=3  WHERE moveproduce_id =".$_GET['id']);//4表示被反冲3反冲2正常1未提交	
	  	//更新库存
		...
		if($res1&&$res2&&$res33&&$res11&&$res3){
		
		$this -> dbObj -> Execute("COMMIT");
		exit("<script>alert('反冲成功');window.location.href='moveproducehistory.php';</script>");	
		}else{
		$this -> dbObj -> Execute("ROLLBACK");
		exit("<script>alert('发生错误，提交失败，数据已经回滚。');history.go(-1);</script>");
		}		
...
function  addorder(){
	...
	function disp(){
		//定义模板
		$t = new Template('../template/stock');
		$t -> set_file('f','moveproducehistory.html');
		$t->unknowns = "remove";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]		
		$t -> set_block('f','mainlist','ml');		
		...
		if($ftable==''){$condition=$category.' like "%'.$keywords.'"';}else{$condition=$category.' like "%'.$keywords.'%"';}
			...
			if($condition<>''&&$ftable==''){
			$sql='select * from '.WEB_ADMIN_TABPOX.'moveproduce  where status in(2,3,4,5,6) and  agencyid ='.$_SESSION["currentorgan"].' and '.$condition ;
 
			}else if($ftable<>''){
			$sql="select * from ".WEB_ADMIN_TABPOX."moveproduce s INNER JOIN ".WEB_ADMIN_TABPOX."$ftable f on s.categoryid =f.category_id  where f.category_name like '%".$keywords."%' and s.status in(2,3,4,5,6) and   s.agencyid =".$_SESSION["currentorgan"] ;
			
			}else{
			$sql='select * from '.WEB_ADMIN_TABPOX.'moveproduce  where  agencyid ='.$_SESSION["currentorgan"].' and status in(2,3,4,5,6) ';
	    	...
	    	$t -> set_var('recordcount',$count);	
	     	...
	     	while ($inrrs = &$inrs -> FetchRow()) {
				...
				$t -> set_var('twarehouse_name',$this -> dbObj -> getone('select warehouse_name from '.WEB_ADMIN_TABPOX.'warehouse  where warehouse_id ='.$inrrs["towarehouse"]));
				 ...
				 $t -> set_var('audit_name',$this->dbObj->GetOne('SELECT employee_name FROM '.WEB_ADMIN_TABPOX.'employee WHERE employee_id = '.$inrrs[auditor_id]));
	...
	function goDispAppend(){
		...
		$t = new Template('../template/stock');
		$t -> set_file('f','moveproducehistory_detail.html');
		$t->unknowns = "move";
		$t->left_delimiter = "[#"; //修改左边界符为[#
        $t->right_delimiter = "#]"; //修改右边界符#]	
		$t -> set_block('f','mainlist','ml');	
		...
		$t -> set_var('userid',$this->getUid());		
			...
			$t -> set_var('tostorageidlist',$this ->selectlist('warehouse','warehouse_id','warehouse_name',$data['towarehouse']));
			 $t -> set_var('audit_name',$this->dbObj->GetOne('SELECT employee_name FROM '.WEB_ADMIN_TABPOX.'employee WHERE employee_id = '.$data[auditor_id]));
			$t -> set_var('cstocknumber',"");	
			$t -> set_var('updid',$updid);
			$t -> set_var('action','upd');			
			$t -> set_var($data);			
...
if($_POST['subsave']==1){			
				$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."moveproducedetail` (`moveproduce_id`, `produce_id`, `number`, `price`,`moveproduceprice`, `account`, `agencyid`) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cprice"]."','".$_POST["cmoveproduceprice"]."','".$_POST["caccount"]."','".$_SESSION["currentorgan"]."')");	
			// echo "INSERT INTO `".WEB_ADMIN_TABPOX."moveproducedetail` (`moveproduce_id`, `produce_id`, `number`, `price`,`moveproduceprice`, `account`, `agencyid`) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cprice"]."','".$_POST["cmoveproduceprice"]."','".$_POST["caccount"]."','".$_SESSION["currentorgan"]."')";
 ...
 if($_POST["con_act"]=='upd'){
        ...
        if($_POST["con_act"]=='add'&&$_POST['subsave']!=''){
		$this -> dbObj -> Execute("INSERT INTO `".WEB_ADMIN_TABPOX."moveproducedetail` (`moveproduce_id`, `produce_id`, `number`, `price`,`moveproduceprice`, `account`, `agencyid`) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cprice"]."','".$_POST["cmoveproduceprice"]."','".$_POST["caccount"]."','".$_SESSION["currentorgan"]."')");	
		//echo "INSERT INTO `".WEB_ADMIN_TABPOX."moveproducedetail` (`moveproduce_id`, `produce_id`, `number`, `price`, `moveproduceprice`,`account`, `agencyid`) VALUES ('".$id."','".$_POST["cproduce_id"]."','".$_POST["cnumber"]."','".$_POST["cprice"]."','".$_POST["cmoveproduceprice"]."','".$_POST["caccount"]."','".$_SESSION["currentorgan"]."')";
