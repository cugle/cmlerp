...
class pageMain {
	...
	function disp(){
		$this -> forAjax();
		$t = new Template('template/mainStyle/style2/');
		...
		//echo "select a.agency_easyname from ".WEB_ADMIN_TABPOX."agency a inner join  ".WEB_ADMIN_TABPOX."user u on a.agency_id =u.agencyidwhere u.userid =".$this -> po -> loginObj->getUid();
		$_SESSION["currentorganname"] = $this -> po -> dbObj -> GetOne("select a.agency_easyname from ".WEB_ADMIN_TABPOX."agency a inner join  ".WEB_ADMIN_TABPOX."user u on a.agency_id=u.agencyid  where u.userid =".$this -> po -> loginObj->getUid());	
	    }
		//设置主菜单
		...
		//设置当前机构信息
		 $t -> set_var('currentorganname',$_SESSION["currentorgan"].$_SESSION["currentorganname"]); 		
...
//		  <{optname} {attrib}='{subusername}'>{subusername}</{optname}>
		...
		//资源路径
		$t -> set_var('path',WEB_ADMIN_HTTPPATH.'/template/mainStyle/style2/');
		$t -> parse('out','f');
		$t -> p('out');
	}
	function forAjax(){
